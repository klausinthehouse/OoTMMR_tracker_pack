...
--[[local CUSTOM_EVENT_ITEMS = {
    ...
    function search()
        ...
        while current ~= nil do
            OOTMM_RUNTIME_STATE["_check_rule_events_used"] = {} -- FIXME: This is leaking all over the place now, and i hate it.
            local events_used = {}
            ...
            if current.age_change then
                if current.child and not current.adult then
                    if event('TIME_TRAVEL_AT_WILL') then
                        current.adult = current.child
                    else
                        -- Make sure we get back here once we can time travel at will.
                        events_used["TIME_TRAVEL_AT_WILL"] = true
                    end
                elseif current.adult and not current.child then
                    if event('TIME_TRAVEL_AT_WILL') then
                        current.child = current.adult
                    else
                        events_used["TIME_TRAVEL_AT_WILL"] = true
                    end
                end
                    ...
                    if place_logic and (saved_place == nil or update_node_if_better(saved_place, activated_current)) then
                        for new_type, new_rules in pairs(place_logic) do
                            ...
                            for new_name, new_rule in pairs(new_rules) do
                                ...
                                node.age_change = place_logic["age_change"]
	...
	function bombchu_license_bag_first()
		return license(BOMBCHU_5) or license(BOMBCHU_10) or license(BOMBCHU_20) or license(SHARED_BOMBCHU) or license(SHARED_BOMBCHU_5) or license(SHARED_BOMBCHU_10) or license(SHARED_BOMBCHU_20)
	end
	...
	function bombchu_license_bag()
		return setting('bombchuBehaviorOot', 'bagSeparate') and bombchu_license_bag_separate() or (setting('bombchuBehaviorOot', 'bagFirst') and bombchu_license_bag_first())
	end

	function bombchu_license()
		return (setting('bombchuBehaviorOot', 'free') or setting('bombchuBehaviorOot', 'bombBag')) and has_bomb_bag() or bombchu_license_bag()
	end

	function bombchu_drops()
		return bombchu_license_bag() and (event('BOMBS_OR_BOMBCHU') or (setting('sharedBombchu') and event('MM_BOMBS_OR_BOMBCHU')))
	end

	function bombchu_source()
		return event('BOMBCHU') or bombchu_drops() or renewable(BOMBCHU_5) or renewable(BOMBCHU_10) or renewable(BOMBCHU_20) or (setting('sharedBombchu') and (event('MM_BOMBCHU') or renewable(SHARED_BOMBCHU) or renewable(SHARED_BOMBCHU_5) or renewable(SHARED_BOMBCHU_10) or renewable(SHARED_BOMBCHU_20)))
	end
	...
	function can_use_slingshot()
		return has('SLINGSHOT') and age_slingshot() and (event('SEEDS') or renewable(DEKU_SEEDS_30))
	end
	...
	function can_hammer()
		return age_hammer() and has_hammer_raw()
	end
	...
	function age_slingshot()
		return is_child() or setting('agelessSlingshot') or glitch_equip_swap()
	end

	function age_bow()
		return is_adult() or setting('agelessBow') or glitch_equip_swap()
	end

	function starts_with_master_sword()
		return setting('startingAge', 'adult') and (not setting('swordlessAdult') or (not setting('shuffleMasterSword') and setting('timeTravelSword')))
	end
	...
	function has_sword_biggoron()
		return cond(setting('progressiveSwordsOot', 'progressive'), cond(starts_with_master_sword(), has('SWORD', 3), has('SWORD', 4)), cond(setting('progressiveSwordsOot', 'goron'), has('SWORD_GORON', 2), has('SWORD_BIGGORON')))
	end
	...
	function can_use_sword_biggoron()
		return age_sword_adult() and has_sword_biggoron()
	end
	...
	function can_use_sword_or_sticks()
		return can_use_sword() or can_use_sticks()
	end

	function has_shield_deku()
		return age_shield_child() and (renewable(SHIELD_DEKU) or renewable(SHARED_SHIELD_DEKU) or event('SHIELD_DEKU') or has('SHIELD', 1) or has('SHARED_SHIELD', 1))
	end

	function has_shield_hylian()
		return renewable(SHARED_SHIELD_HYLIAN) or renewable(SHIELD_HYLIAN) or has('SHIELD', 2) or has('SHARED_SHIELD', 2)
	end

	function has_shield()
		return has_shield_hylian() or has_mirror_shield() or has_shield_deku()
	end

	function has_shield_for_scrubs()
		return is_adult() and has_shield_hylian() or has_shield_deku()
	end
	...
	function stone_of_agony()
		return cond(setting('sharedStoneAgony'), has('SHARED_STONE_OF_AGONY'), has('STONE_OF_AGONY')) or trick('OOT_HIDDEN_GROTTOS')
	end
	...
	function has_spin_upgrade()
		return cond(setting('spinUpgradeOot'), cond(setting('sharedSpinUpgrade'), has('SHARED_SPIN_UPGRADE'), has('SPIN_UPGRADE')), true)
	end
	...
	function can_swim_or_sink()
		return can_swim() or has_iron_boots()
	end
	...
	function can_ride_bean(x)
		return is_adult() and (setting('ootPreplantedBeans') or event(x))
	end
	...
	function can_cut_grass()
		return can_cut_grass_no_c_button() or can_boomerang() or has_explosives() or can_hammer() or glitch_broken_stick()
	end

	function can_cut_grass_for_bugs()
		return can_use_sword() or can_use_sticks() or can_use_mask_blast() or can_boomerang() or has_explosives() or can_hammer()
	end
	...
	function can_rescue_carpenter()
		return small_keys_hideout_all() and (can_use_sword() or ((can_boomerang() or has_nuts()) and can_use_sticks())) and soul_carpenters() and soul_thief_fighter()
	end
	...
	function get_past_skultullas()
		return has_nuts() or can_use_sword_or_sticks() or has_ranged_weapon() or can_use_din() or has_explosives() or can_hammer() or can_use_mask_blast() or can_use_mask_stone()
	end

	function collect_gs_on_walls()
		return gs() and (can_collect_distance() or (has_ranged_weapon() and climb_anywhere()))
	end
	...
	function can_move_mido_reqs()
		return is_child() and has_sword_kokiri() and has_shield_deku()
	end
	...
	function merchant_price(id)
		return wallet_price(OOT_MERCHANTS, id)
	end
	...
	function glitch_broken_stick()
		return trick('GLITCH_OOT_BROKEN_STICK') and can_use_sticks()
	end
    ...
    function rusty_key(x)
		return not setting('RustyKeysOot') or has(x)
	end
	...
	function break_icicle()
		return can_use_sword_or_sticks() or has_explosives_or_hammer()
	end

	function break_icicle_ice_cavern()
    ...
    ["Bottom of the Well Main"] = {
        ...
        ["locations"] = {
            ["Bottom of the Well Coffin"] = function () return can_swim() or can_play_zelda() or (has_iron_boots() and hookshot_anywhere()) end,
            ...
            ["Bottom of the Well Underwater"] = function () return can_play_zelda() or has_iron_boots() end,
            ...
            ["Bottom of the Well Underwater 2"] = function () return can_play_zelda() or has_iron_boots() end,
            ...
            ["Bottom of the Well Pot Main Room Underwater"] = function () return can_play_zelda() or ((can_swim() or has_iron_boots()) and (has_ranged_weapon() or has_bombchu())) end,
            ...
            ["Bottom of the Well Heart 1"] = function () return can_swim() or can_play_zelda() or (has_iron_boots() and hookshot_anywhere()) end,
            ["Bottom of the Well Heart 2"] = function () return can_swim() or can_play_zelda() or (has_iron_boots() and hookshot_anywhere()) end,
            ["Bottom of the Well Basement Big Fairy"] = function () return can_play_sun() end,
            ["Bottom of the Well Basement Boulders"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Deku Tree Basement"] = {
        ...
        ["exits"] = {
            ["Deku Tree Lobby"] = function () return true end,
            ["Deku Tree Basement Back Room"] = function () return can_hit_triggers_distance() and event('DEKU_BURN_WEB') and (can_swim() or longshot_anywhere() or (has_iron_boots() and hookshot_anywhere())) end,
    ...
    ["Deku Tree Basement Ledge"] = {
        ["exits"] = {
            ...
            ["Deku Tree Before Boss"] = function () return has_fire_or_sticks() and (can_swim() or has_iron_boots()) end,
        },
        ["age_change"] = true,
    },
    ...
    ["Dodongo Cavern Main"] = {
        ["exits"] = {
            ...
            ["Dodongo Cavern Pre-Boss Lobby"] = function () return event('DC_BOMB_EYES') end,
        },
        ["locations"] = {
    ...
    ["Forest Temple Garden West"] = {
        ["events"] = {
            ["STICKS"] = function () return soul_deku_baba() and (can_hookshot() or can_hammer() or can_boomerang() or (has_nuts() and can_use_sword())) end,
            ["NUTS"] = function () return soul_deku_baba() and (can_use_sword() or has_explosives() or can_use_slingshot()) end,
        },
        ["exits"] = {
    ...
    ["Ganon Castle Light End"] = {
        ...
        ["exits"] = {
            ["Ganon Castle Light Wallmaster"] = function () return soul_wallmaster() end,
        },
        ["locations"] = {
    ...
    ["Ganon Castle Water"] = {
        ...
        ["locations"] = {
            ...
            ["Ganon Castle Icicle Water 01"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 02"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 03"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 04"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 05"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 06"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 07"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 08"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 09"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 10"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 11"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 12"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 13"] = function () return break_icicle() end,
            ["Ganon Castle Icicle Water 14"] = function () return break_icicle() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Ganon Castle Shadow"] = {
        ["exits"] = {
            ["Ganon Castle Shadow End"] = function () return can_hammer() and ((can_longshot() and soul_like_like() or has_fire_arrows()) and (has_hover_boots() or has_fire()) and (has_lens() or (can_longshot() and has_hover_boots())) or (climb_anywhere() and (has_hover_boots() or hookshot_anywhere() or has_lens()))) end,
        },
        ["locations"] = {
            ["Ganon Castle Shadow Chest 1"] = function () return can_play_time() or can_hookshot() or has_hover_boots() or has_fire_arrows() end,
            ["Ganon Castle Shadow Chest 2"] = function () return (can_longshot() and soul_like_like() or has_fire_arrows()) and (has_hover_boots() or has_fire()) or (climb_anywhere() and (has_hover_boots() or hookshot_anywhere() or has_lens())) end,
            ["Ganon Castle Pot Shadow 1"] = function () return (can_longshot() and soul_like_like() or has_fire_arrows()) and (has_hover_boots() or has_fire()) or (climb_anywhere() and (has_hover_boots() or hookshot_anywhere() or (has_lens() and has_fire_arrows()))) end,
            ["Ganon Castle Pot Shadow 2"] = function () return (can_longshot() and soul_like_like() or has_fire_arrows()) and (has_hover_boots() or has_fire()) or (climb_anywhere() and (has_hover_boots() or hookshot_anywhere() or (has_lens() and has_fire_arrows()))) end,
            ["Ganon Castle Heart Shadow 1"] = function () return ((can_longshot() and soul_like_like() or has_fire_arrows()) and (has_hover_boots() or has_fire()) or climb_anywhere()) and has_lens() end,
            ["Ganon Castle Heart Shadow 2"] = function () return ((can_longshot() and soul_like_like() or has_fire_arrows()) and (has_hover_boots() or has_fire()) or climb_anywhere()) and has_lens() end,
            ["Ganon Castle Heart Shadow 3"] = function () return ((can_longshot() and soul_like_like() or has_fire_arrows()) and (has_hover_boots() or has_fire()) or climb_anywhere()) and has_lens() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Ganon Castle Tower"] = {
        ["exits"] = {
            ["Ganon Castle Stairs"] = function () return true end,
            ["Ganon Castle Tower Lobby"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower Lobby"] = {
        ["exits"] = {
            ...
            ["Ganon Castle Tower Dinalfos Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower Dinalfos Room"] = {
        ["events"] = {
            ["GANON_TOWER_LIZALFOS_CLEAR"] = function () return soul_lizalfos_dinalfos() and (can_use_sticks() or has_weapon() or can_use_slingshot()) end,
        },
        ["exits"] = {
            ["Ganon Castle Tower Lobby"] = function () return event('GANON_TOWER_LIZALFOS_CLEAR') end,
            ["Ganon Castle Tower First Transitional Staircase"] = function () return event('GANON_TOWER_LIZALFOS_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower First Transitional Staircase"] = {
        ["exits"] = {
            ["Ganon Castle Tower Dinalfos Room"] = function () return true end,
            ["Ganon Castle Tower Stalfos Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower Stalfos Room"] = {
        ["events"] = {
            ["GANON_TOWER_STALFOS_CLEAR"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() end,
        },
        ["exits"] = {
            ["Ganon Castle Tower First Transitional Staircase"] = function () return event('GANON_TOWER_STALFOS_CLEAR') end,
            ["Ganon Castle Tower Second Transitional Staircase"] = function () return event('GANON_TOWER_STALFOS_CLEAR') end,
        },
        ["locations"] = {
            ["Ganon Castle Boss Key"] = function () return event('GANON_TOWER_STALFOS_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower Second Transitional Staircase"] = {
        ["exits"] = {
            ["Ganon Castle Tower Stalfos Room"] = function () return true end,
            ["Ganon Castle Tower Iron Knuckle Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower Iron Knuckle Room"] = {
        ["events"] = {
            ["GANON_TOWER_IRON_KNUCKLE_CLEAR"] = function () return soul_iron_knuckle() and has_weapon() end,
        },
        ["exits"] = {
            ["Ganon Castle Tower Second Transitional Staircase"] = function () return event('GANON_TOWER_IRON_KNUCKLE_CLEAR') end,
            ["Ganon Castle Tower Staircase Before Boss Door"] = function () return event('GANON_TOWER_IRON_KNUCKLE_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower Staircase Before Boss Door"] = {
        ["exits"] = {
            ["Ganon Castle Tower Iron Knuckle Room"] = function () return true end,
            ["Ganon Castle Tower Lower Arena"] = function () return setting('ganonBossKey', 'removed') or has('BOSS_KEY_GANON') or (setting('ganonBossKey', 'custom') and special(GANON_BK)) end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Tower Lower Arena"] = {
        ...
        ["exits"] = {
            ["Ganon Castle Tower Staircase Before Boss Door"] = function () return false end,
            ["Ganon Castle Tower Final Staircase"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Ganon Castle Tower Final Staircase"] = {
        ["exits"] = {
            ["Ganon Castle Tower Lower Arena"] = function () return false end,
            ["Ganon Castle Tower Boss"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ...
    ["Gerudo Fortress Carpenter 1 Left"] = {
        ...
        ["locations"] = {
            ["Gerudo Fortress Jail 1"] = function () return (can_use_sword() or ((can_boomerang() or has_nuts()) and can_use_sticks())) and soul_carpenters() and soul_thief_fighter() end,
    ...
    ["Gerudo Fortress Carpenter 2 Bottom"] = {
        ...
        ["locations"] = {
            ["Gerudo Fortress Jail 2"] = function () return (can_use_sword() or ((can_boomerang() or has_nuts()) and can_use_sticks())) and soul_carpenters() and soul_thief_fighter() end,
            ["Gerudo Member Card"] = function () return carpenters_rescued() end,
    ...
    ["Gerudo Fortress Carpenter 4 Top"] = {
        ...
        ["locations"] = {
            ["Gerudo Fortress Jail 4"] = function () return (can_use_sword() or ((can_boomerang() or has_nuts()) and can_use_sticks())) and soul_carpenters() and soul_thief_fighter() end,
            ["Gerudo Member Card"] = function () return carpenters_rescued() end,
    ...
    ["Gerudo Fortress Carpenter 3"] = {
        ...
        ["locations"] = {
            ["Gerudo Fortress Jail 3"] = function () return (can_use_sword() or ((can_boomerang() or has_nuts()) and can_use_sticks())) and soul_carpenters() and soul_thief_fighter() end,
            ["Gerudo Member Card"] = function () return carpenters_rescued() end,
    ...
    ["Gerudo Training Grounds"] = {
        ...
        ["locations"] = {
            ...
            ["Gerudo Training Grounds Wonder Item Beamos Room"] = function () return can_use_bow() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Gerudo Training Grounds Upper"] = {
        ...
        ["locations"] = {
            ...
            ["Gerudo Training Grounds Wonder Item Eye Statue Room"] = function () return has_hover_boots() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Gerudo Training Grounds Underwater"] = {
        ["locations"] = {
            ...
            ["Gerudo Training Grounds SR Water 2"] = function () return can_swim() or can_hookshot() end,
            ["Gerudo Training Grounds SR Water 3"] = function () return true end,
            ["Gerudo Training Grounds SR Water 4"] = function () return can_swim() or can_hookshot() end,
            ["Gerudo Training Grounds SR Water 5"] = function () return can_swim() or hookshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Hammer"] = {
        ...
        ["locations"] = {
            ...
            ["Gerudo Training Grounds SR Lava Back Right"] = function () return soul_keese() and soul_enemy(SOUL_ENEMY_TORCH_SLUG) and (can_use_sword() or can_use_sticks() or can_hammer() or can_use_bow()) and can_hookshot() end,
    ...
    ["Ice Cavern"] = {
        ...
        ["locations"] = {
            ...
            ["Ice Cavern Icicle Before Entrance 1"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Entrance 2"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Entrance 3"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Entrance 1"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Entrance 2"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Entrance 3"] = function () return break_icicle_ice_cavern() end,
    ...
    ["Ice Cavern Scythe"] = {
        ...
        ["locations"] = {
            ...
            ["Ice Cavern Icicle Before Scythe 1"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Scythe 2"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Scythe 3"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Scythe 4"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Scythe 1"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Scythe 2"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Scythe 3"] = function () return break_icicle_ice_cavern() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern HP Room"] = {
        ...
        ["locations"] = {
            ...
            ["Ice Cavern Icicle Compass 01"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 02"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 03"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 04"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 05"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 06"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 07"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 08"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 09"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 10"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 11"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 12"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Compass 13"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Red Ice Compass 1"] = function () return has_blue_fire() end,
            ["Ice Cavern Red Ice Compass 2"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern First Fire Room"] = {
        ...
        ["locations"] = {
            ...
            ["Ice Cavern Icicle Before Map 1"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Map 2"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Map 3"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Red Ice Map 1"] = function () return has_blue_fire() end,
            ["Ice Cavern Red Ice Map 2"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Block Room"] = {
        ...
        ["locations"] = {
            ...
            ["Ice Cavern Icicle Before Block 1"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Block 2"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Block 3"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Block 4"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle Before Block 5"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Red Ice Block"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern End"] = {
        ...
        ["locations"] = {
            ...
            ["Ice Cavern Icicle After Block 1"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle After Block 2"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle After Block 3"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle After Block 4"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle After Block 5"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle After Block 6"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle After Block 7"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Icicle After Block 8"] = function () return break_icicle_ice_cavern() end,
            ["Ice Cavern Red Ice After Block 1"] = function () return has_blue_fire() end,
            ["Ice Cavern Red Ice After Block 2"] = function () return has_blue_fire() end,
            ["Ice Cavern Red Ice After Block 3"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu"] = {
    ...
    ["Jabu-Jabu Elevator"] = {
        ["exits"] = {
            ...
            ["Jabu-Jabu Pre-Boss Room"] = function () return event('BIG_OCTO') or (has_hover_boots() and (trick('OOT_JABU_BOSS_HOVER') or can_play_elegy())) or climb_anywhere() or (glitch_megaflip() and (trick('OOT_JJB_BOXLESS') and can_jump_slash() or can_play_elegy())) end,
            ["Jabu-Jabu Hole Room"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Jabu-Jabu Hole Room"] = {
        ["exits"] = {
            ...
            ["Jabu-Jabu Parasite Fork"] = function () return true end,
            ["Jabu-Jabu Basement"] = function () return true end,
            ["Jabu-Jabu Before Big Octo"] = function () return event('PARASITE') end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Parasite Fork"] = {
        ["events"] = {
            ["PARASITE"] = function () return event('RED_PARASITE') and event('BLUE_PARASITE') and event('GREEN_PARASITE') end,
        },
        ["exits"] = {
            ["Jabu-Jabu Hole Room"] = function () return true end,
            ["Jabu-Jabu Boomerang Room"] = function () return true end,
            ["Jabu-Jabu Map Room"] = function () return soul_ruto() or can_play_elegy() end,
            ["Jabu-Jabu Compass Room"] = function () return event('RED_PARASITE') end,
            ["Jabu-Jabu Blue Parasite"] = function () return event('RED_PARASITE') end,
            ["Jabu-Jabu Green Parasite"] = function () return event('BLUE_PARASITE') end,
        },
        ["age_change"] = true,
    },
    ...
    ["Jabu-Jabu Map Room"] = {
        ["events"] = {
            ["RED_PARASITE"] = function () return soul_enemy(SOUL_ENEMY_PARASITE) and can_boomerang() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Parasite Fork"] = function () return true end,
        },
        ["locations"] = {
            ["Jabu-Jabu Map Chest"] = function () return event('RED_PARASITE') end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Compass Room"] = {
        ["exits"] = {
            ["Jabu-Jabu Parasite Fork"] = function () return soul_enemy(SOUL_ENEMY_SHABOM) end,
        },
        ["locations"] = {
            ["Jabu-Jabu Compass Chest"] = function () return soul_enemy(SOUL_ENEMY_SHABOM) end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Blue Parasite"] = {
        ["events"] = {
            ["BLUE_PARASITE"] = function () return soul_enemy(SOUL_ENEMY_PARASITE) and can_boomerang() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Parasite Fork"] = function () return event('BLUE_PARASITE') end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Green Parasite"] = {
        ["events"] = {
            ["GREEN_PARASITE"] = function () return soul_enemy(SOUL_ENEMY_PARASITE) and can_boomerang() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Parasite Fork"] = function () return event('GREEN_PARASITE') and soul_enemy(SOUL_ENEMY_BIRI_BARI) end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Basement"] = {
        ["exits"] = {
            ["Jabu-Jabu Hole Room"] = function () return true end,
            ["Jabu-Jabu Muscle Block Room"] = function () return true end,
            ["Jabu-Jabu Water Switch Room"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Jabu-Jabu Before Big Octo"] = {
        ["exits"] = {
            ["Jabu-Jabu Basement"] = function () return true end,
            ["Jabu-Jabu Big Octo"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Big Octo"] = {
        ...
        ["exits"] = {
            ["Jabu-Jabu Elevator"] = function () return event('BIG_OCTO') or climb_anywhere() end,
        },
        ["locations"] = {
    ...
    ["Kokiri Forest Child"] = {
        ["locations"] = {
            ["Kokiri Forest Kokiri Sword Chest"] = function () return true end,
            ["Kokiri Forest GS Night Child"] = function () return gs_night() and can_damage_skull() end,
            ["Kokiri Forest Rupee Child 1"] = function () return true end,
            ["Kokiri Forest Rupee Child 2"] = function () return true end,
            ["Kokiri Forest Rupee Crawl 1"] = function () return true end,
            ["Kokiri Forest Rupee Crawl 2"] = function () return true end,
            ["Kokiri Forest Heart 1"] = function () return true end,
            ["Kokiri Forest Heart 2"] = function () return true end,
            ["Kokiri Forest Heart 3"] = function () return true end,
            ["Kokiri Forest Wonder Item Invisible Rupee 1"] = function () return true end,
            ["Kokiri Forest Wonder Item Invisible Rupee 2"] = function () return true end,
            ["Kokiri Forest Wonder Item Invisible Rupee 3"] = function () return true end,
            ["Kokiri Forest Wonder Item Invisible Rupee 4"] = function () return true end,
            ["Kokiri Forest Wonder Item Midair 1"] = function () return true end,
            ["Kokiri Forest Wonder Item Midair 2"] = function () return true end,
            ["Kokiri Forest Wonder Item Midair 3"] = function () return true end,
            ["Kokiri Forest Wonder Item Crawl 1"] = function () return true end,
    ...
    ["Kokiri Forest Grass Child"] = {
        ["locations"] = {
            ["Kokiri Forest Grass Child Near Crawl 1"] = function () return true end,
            ["Kokiri Forest Grass Child Near Crawl 2"] = function () return true end,
            ["Kokiri Forest Grass Child Near Crawl 3"] = function () return true end,
            ["Kokiri Forest Grass Child Near Crawl 4"] = function () return true end,
            ["Kokiri Forest Grass Child Near Crawl 5"] = function () return true end,
            ["Kokiri Forest Grass Child Near Crawl 6"] = function () return true end,
            ["Kokiri Forest Grass Child Near Crawl 7"] = function () return true end,
            ["Kokiri Forest Grass Child Near Crawl 8"] = function () return true end,
    ...
    ["Kokiri Forest Grass Adult"] = {
        ["locations"] = {
            ...
            ["Kokiri Forest Grass Adult Near Crawl 1"] = function () return true end,
            ["Kokiri Forest Grass Adult Near Crawl 2"] = function () return true end,
            ["Kokiri Forest Grass Adult Near Crawl 3"] = function () return true end,
            ["Kokiri Forest Grass Adult Near Crawl 4"] = function () return true end,
            ["Kokiri Forest Grass Adult Near Crawl 5"] = function () return true end,
            ["Kokiri Forest Grass Adult Near Crawl 6"] = function () return true end,
            ["Kokiri Forest Grass Adult Near Crawl 7"] = function () return true end,
            ["Kokiri Forest Grass Adult Near Crawl 8"] = function () return true end,
        },
        ["age_change"] = false,
    },
    ...
    ["Kokiri Shop Items"] = {
        ["locations"] = {
            ...
            ["Kokiri Shop Wonder Item"] = function () return true end,
        },
        ["age_change"] = false,
    },
    ["Mido's House"] = {
    ...
    ["Hyrule Field"] = {
        ...
        ["locations"] = {
            ...
            ["Hyrule Field Boulders"] = function () return has_explosives_or_hammer() end,
            ["Hyrule Field Silver Boulder"] = function () return can_lift_silver() end,
    ...
    ["Hyrule Field Red Boulders"] = {
        ["locations"] = {
            ["Hyrule Field Red Boulder 1"] = function () return true end,
            ["Hyrule Field Red Boulder 2"] = function () return true end,
            ["Hyrule Field Red Boulder 3"] = function () return true end,
            ["Hyrule Field Red Boulder 4"] = function () return true end,
        },
        ["age_change"] = false,
    },
    ...
    ["Market"] = {
        ...
        ["locations"] = {
            ...
            ["Market Wonder Item Day 2"] = function () return is_child() and is_day() end,
            ["Market Wonder Item Day 3"] = function () return is_child() and is_day() end,
            ["Market Wonder Item Day 4"] = function () return is_child() and is_day() end,
    ...
    ["Lon Lon Ranch House"] = {
        ...
        ["locations"] = {
            ...
            ["Lon Lon Ranch Talon House Pot 1"] = function () return rusty_key(RUSTY_MALON) end,
            ["Lon Lon Ranch Talon House Pot 2"] = function () return rusty_key(RUSTY_MALON) end,
            ["Lon Lon Ranch Talon House Pot 3"] = function () return rusty_key(RUSTY_MALON) end,
        },
        ["age_change"] = true,
    },
    ...
    ["Hyrule Castle"] = {
        ...
        ["locations"] = {
            ...
            ["Hyrule Castle Boulder"] = function () return has_explosives_or_hammer() end,
            ["Hyrule Castle Wonder Item Torch 1"] = function () return can_use_slingshot() and can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Torch 2"] = function () return can_use_slingshot() and can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Moat 01"] = function () return can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Moat 02"] = function () return can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Moat 03"] = function () return can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Moat 04"] = function () return can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Moat 05"] = function () return can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Moat 06"] = function () return can_swim_or_sink() end,
            ["Hyrule Castle Wonder Item Moat 07"] = function () return can_swim_or_sink() end,
    ...
    ["Ganon Castle Exterior"] = {
        ...
        ["locations"] = {
            ["Ganon Castle Exterior GS"] = function () return gs() and (has_ranged_weapon() or can_use_sword()) end,
            ["Ganon Castle Exterior Red Boulder 1"] = function () return can_hammer() end,
    ...
    ["Lost Woods"] = {
        ...
        ["exits"] = {
            ...
            ["Lost Woods Lost West"] = function () return setting('alterLostWoodsExits') end,
        },
        ["locations"] = {
            ["Lost Woods Target"] = function () return is_child() and (can_use_slingshot() or can_use_bow()) end,
            ["Lost Woods Skull Kid"] = function () return is_child() and can_play_saria() end,
            ["Lost Woods Sell Skull Mask"] = function () return is_child() and can_play_saria() and has('MASK_SKULL') end,
            ...
            ["Lost Woods Wonder Item 1"] = function () return is_child() end,
            ["Lost Woods Wonder Item 2"] = function () return is_child() end,
            ["Lost Woods Wonder Item 3"] = function () return is_child() end,
            ["Lost Woods Boulders"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Sacred Meadow"] = {
        ...
        ["locations"] = {
            ...
            ["Sacred Meadow Wonder Item Maze 1"] = function () return true end,
            ["Sacred Meadow Wonder Item Maze 2"] = function () return true end,
    ...
    ["Kakariko"] = {
        ...
        ["locations"] = {
            ...
            ["Kakariko Sell Keaton Mask"] = function () return soul_npc(SOUL_NPC_HYLIAN_GUARD) and is_child() and (setting('kakarikoGate', 'open') or event('KAKARIKO_GATE_OPEN')) and has_mask_keaton() end,
            ...
            ["Kakariko Silver Boulder"] = function () return can_lift_silver() and is_adult() end,
        },
        ["age_change"] = true,
    },
    ["Kakariko Balcony"] = {
    ...
    ["Dampe Grave"] = {
        ...
        ["locations"] = {
            ...
            ["Graveyard Dampe Tomb Wonder Item 01"] = function () return true end,
            ["Graveyard Dampe Tomb Wonder Item 02"] = function () return true end,
            ["Graveyard Dampe Tomb Wonder Item 03"] = function () return true end,
            ["Graveyard Dampe Tomb Wonder Item 04"] = function () return true end,
            ["Graveyard Dampe Tomb Wonder Item 05"] = function () return true end,
            ["Graveyard Dampe Tomb Wonder Item 06"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 07"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 08"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 09"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 10"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 11"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 12"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 13"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 14"] = function () return soul_dampe() end,
            ["Graveyard Dampe Tomb Wonder Item 15"] = function () return soul_dampe() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Death Mountain"] = {
        ...
        ["locations"] = {
            ...
            ["Death Mountain Trail Child Boulder"] = function () return is_child() and has_explosives_or_hammer() end,
            ["Death Mountain Trail Boulder"] = function () return has_explosives_or_hammer() end,
            ["Death Mountain Trail Big Fairy"] = function () return can_play_sun() end,
        },
        ["age_change"] = true,
    },
    ["Death Mountain Trail Red Boulders Lower"] = {
        ["locations"] = {
            ["Death Mountain Trail Red Boulder Lower 1"] = function () return true end,
            ["Death Mountain Trail Red Boulder Lower 2"] = function () return true end,
            ["Death Mountain Trail Red Boulder Lower 3"] = function () return true end,
            ["Death Mountain Trail Red Boulder Lower 4"] = function () return true end,
            ["Death Mountain Trail Red Boulder Lower 5"] = function () return true end,
            ["Death Mountain Trail Red Boulder Lower 6"] = function () return true end,
            ["Death Mountain Trail Red Boulder Lower 7"] = function () return true end,
            ["Death Mountain Trail Red Boulder Near Lower GS"] = function () return true end,
        },
        ["age_change"] = false,
    },
    ...
    ["Death Mountain Trail Red Boulders Upper"] = {
        ["locations"] = {
            ["Death Mountain Trail Red Boulder Near Upper GS"] = function () return true end,
            ["Death Mountain Trail Red Boulder Upper 1"] = function () return true end,
            ["Death Mountain Trail Red Boulder Upper 2"] = function () return true end,
        },
        ["age_change"] = false,
    },
    ...
    ["Goron City"] = {
        ...
        ["locations"] = {
            ...
            ["Goron City Maze Boulders"] = function () return has_explosives_or_hammer() end,
            ["Goron City Silver Boulders"] = function () return can_lift_silver() end,
            ["Goron City Shortcut Boulders"] = function () return has_explosives_or_hammer() or has_bombflowers end,
            ...
            ["Goron City Medigoron Giant Knife"] = function () return is_adult() and (has_bombflowers() or can_hammer() or has_blue_fire_arrows_mudwall()) and merchant_price(0) and soul_medigoron() end,
    ...
    ["Goron City Red Boulders"] = {
        ["locations"] = {
            ["Goron City Red Boulder 1"] = function () return true end,
            ["Goron City Red Boulder 2"] = function () return true end,
            ["Goron City Red Boulder 3"] = function () return true end,
            ["Goron City Red Boulder 4"] = function () return true end,
            ["Goron City Red Boulder 5"] = function () return true end,
        },
        ["age_change"] = false,
    },
    ["Darunia Chamber"] = {
    ...
    ["Zora River Front"] = {
        ...
        ["locations"] = {
            ...
            ["Zora River Wonder Item Front 1"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Front 2"] = function () return is_child() and can_swim_or_sink() end,
    ...
    ["Zora River"] = {
        ...
        ["locations"] = {
            ...
            ["Zora River Frogs Zeldas Lullaby"] = function () return is_child() and can_play_zelda() end,
            ["Zora River Frogs Eponas Song"] = function () return is_child() and can_play_epona() end,
            ["Zora River Frogs Sarias Song"] = function () return is_child() and can_play_saria() end,
            ["Zora River Frogs Suns Song"] = function () return is_child() and can_play_sun() end,
            ["Zora River Frogs Song of Time"] = function () return is_child() and can_play_time() end,
            ...
            ["Zora River Rupee 1"] = function () return is_adult() and can_swim_or_sink() end,
            ["Zora River Rupee 2"] = function () return is_adult() and can_swim_or_sink() end,
            ["Zora River Rupee 3"] = function () return is_adult() and can_swim_or_sink() end,
            ["Zora River Rupee 4"] = function () return is_adult() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 01"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 02"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 03"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 04"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 05"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 06"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 07"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 08"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 09"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 10"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 11"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 12"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 13"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 14"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 15"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 16"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 17"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 18"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 19"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 20"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 21"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 22"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 23"] = function () return is_child() and can_swim_or_sink() end,
            ["Zora River Wonder Item Back 24"] = function () return is_child() and can_swim_or_sink() end,
            ...
            ["Zora River Grotto Boulder"] = function () return has_explosives_or_hammer() end,
    ...
    ["Zora Domain"] = {
        ...
        ["locations"] = {
            ...
            ["Zora Domain Diving Game"] = function () return is_child() and can_use_wallet(1) and soul_zora() and can_swim_or_sink() end,
            ["Zora Domain Diving Game Green Rupee"] = function () return is_child() and can_use_wallet(1) and soul_zora() and can_swim_or_sink() end,
            ["Zora Domain Diving Game Blue Rupee"] = function () return is_child() and can_use_wallet(1) and soul_zora() and can_swim_or_sink() end,
            ["Zora Domain Diving Game Red Rupee"] = function () return is_child() and can_use_wallet(1) and soul_zora() and can_swim_or_sink() end,
            ["Zora Domain Diving Game Purple Rupee"] = function () return is_child() and can_use_wallet(1) and soul_zora() and can_swim_or_sink() end,
            ["Zora Domain Diving Game Huge Rupee"] = function () return is_child() and can_use_wallet(1) and soul_zora() and can_swim_or_sink() end,
    ...
    ["Zora Domain Underwater"] = {
        ["exits"] = {
            ["Zora Domain"] = function () return true end,
            ["Zora Domain Shortcut"] = function () return has_iron_boots() end,
        },
        ["age_change"] = true,
    },
    ["Zora Domain Shortcut"] = {
        ["exits"] = {
            ["Zora Domain Underwater"] = function () return is_adult() or can_swim() end,
            ["Lake Hylia Near Shortcut"] = function () return can_swim() end,
        },
        ["age_change"] = true,
    },
    ["Zora Domain Back"] = {
    ...
    ["Lake Hylia Near Shortcut"] = {
        ["exits"] = {
            ["Lake Hylia"] = function () return is_adult() or can_swim() end,
            ["Zora Domain Shortcut"] = function () return is_adult() or can_swim() end,
        },
        ["age_change"] = false,
    },
    ...
    ["Lake Hylia Near Water Temple"] = {
        ["exits"] = {
            ["Lake Hylia"] = function () return can_swim() end,
        },
        ["age_change"] = true,
    },
    ["Laboratory"] = {
    ...
    ["Zora Fountain"] = {
        ...
        ["locations"] = {
            ...
            ["Zora Fountain Boulder"] = function () return has_explosives_or_hammer() end,
            ["Zora Fountain submerged Boulder"] = function () return has_explosives_or_hammer() and can_lift_silver() end,
            ["Zora Fountain Silver Boulder"] = function () return can_lift_silver() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Death Mountain Crater Top"] = {
        ...
        ["locations"] = {
            ...
            ["Death Mountain Crater Boulder"] = function () return has_explosives_or_hammer() end,
    ...
    ["Death Mountain Crater Red Boulders"] = {
        ["locations"] = {
            ["Death Mountain Crater Red Boulder Grotto"] = function () return true end,
            ["Death Mountain Crater Red Boulder Great Fairy 1"] = function () return true end,
            ["Death Mountain Crater Red Boulder Great Fairy 2"] = function () return true end,
            ["Death Mountain Crater Red Boulder Shortcut"] = function () return true end,
        },
        ["age_change"] = false,
    },
    ...
    ["Death Mountain Crater Warp"] = {
        ...
        ["exits"] = {
            ["Death Mountain Crater Near Temple"] = function () return has_tunic_goron() and (is_adult() or setting('openDungeonsOot', 'fireChild') or has_hover_boots() or climb_anywhere() or hookshot_anywhere()) end,
    ...
    ["Gerudo Valley"] = {
        ...
        ["locations"] = {
            ...
            ["Gerudo Valley Rocks in Water"] = function () return is_adult or has_bombchu end,
            ...
            ["Gerudo Valley Red Boulder Before Bridge 1"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Red Boulder Before Bridge 2"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Boulder Before Bridge"] = function () return has_explosives_or_hammer end,
            ["Gerudo Valley Silver Boulder"] = function () return can_lift_silver() end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Valley After Bridge"] = {
        ...
        ["locations"] = {
            ...
            ["Gerudo Valley Red Boulder After Bridge 1"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Red Boulder After Bridge 2"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Red Boulder Around Chest 1"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Red Boulder Around Chest 2"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Red Boulder Around Chest 3"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Red Boulder Around Chest 4"] = function () return is_adult() and can_hammer() end,
            ["Gerudo Valley Boulder After Bridge"] = function () return has_explosives_or_hammer end,
        },
        ["age_change"] = true,
    },
    ...
    ["Octorok Grotto"] = {
        ...
        ["locations"] = {
            ["Gerudo Valley Octorok Grotto Rupee 1"] = function () return can_swim_or_sink() or can_boomerang() end,
            ["Gerudo Valley Octorok Grotto Rupee 2"] = function () return can_swim_or_sink() or can_boomerang() end,
            ["Gerudo Valley Octorok Grotto Rupee 3"] = function () return can_swim_or_sink() or can_boomerang() end,
            ["Gerudo Valley Octorok Grotto Rupee 4"] = function () return can_swim_or_sink() or can_boomerang() end,
            ["Gerudo Valley Octorok Grotto Rupee 5"] = function () return can_swim_or_sink() or can_boomerang() end,
    ...
    ["Gerudo Fortress Exterior"] = {
        ["events"] = {
            ["OPEN_FORTRESS_GATE"] = function () return soul_thief_guard() and has('GERUDO_CARD') and is_adult() end,
        },
        ["exits"] = {
            ["Gerudo Fortress Carpenter 1 Left"] = function () return true end,
            ["Gerudo Fortress Carpenter 1 Right"] = function () return true end,
    ...
    ["Gerudo Fortress Upper-Right Ledge"] = {
        ...
        ["locations"] = {
            ["Gerudo Fortress GS Wall"] = function () return is_adult() and gs_night() and (can_collect_distance() or has_bombs() or can_use_bow() or can_use_slingshot() or (climb_anywhere() and can_damage())) end,
        },
        ["age_change"] = true,
    },
    ...
    ["Haunted Wasteland Structure"] = {
        ...
        ["locations"] = {
            ["Haunted Wasteland Carpet Merchant"] = function () return soul_carpet_man() and merchant_price(1) end,
    ...
    ["Desert Colossus"] = {
        ...
        ["locations"] = {
            ...
            ["Desert Colossus Silver Boulder"] = function () return can_lift_silver() end,
            ...
            ["Desert Colossus Wonder Item Fountain 1"] = function () return can_use_bow() or can_use_slingshot() end,
            ["Desert Colossus Wonder Item Fountain 2"] = function () return can_use_bow() or can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Desert Colossus Oasis"] = {
        ["locations"] = {
            ["Desert Colossus Oasis Fairy 1"] = function () return can_swim_or_sink() end,
            ["Desert Colossus Oasis Fairy 2"] = function () return can_swim_or_sink() end,
            ["Desert Colossus Oasis Fairy 3"] = function () return can_swim_or_sink() end,
            ["Desert Colossus Oasis Fairy 4"] = function () return can_swim_or_sink() end,
            ["Desert Colossus Oasis Fairy 5"] = function () return can_swim_or_sink() end,
            ["Desert Colossus Oasis Fairy 6"] = function () return can_swim_or_sink() end,
            ["Desert Colossus Oasis Fairy 7"] = function () return can_swim_or_sink() end,
            ["Desert Colossus Oasis Fairy 8"] = function () return can_swim_or_sink() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Shadow Temple After Boat"] = {
        ...
        ["exits"] = {
            ["Shadow Temple Pre-Boss"] = function () return small_keys_shadow(5) and (can_use_bow() or scarecrow_longshot() or climb_anywhere() or event('SHADOW_PILLAR')) and (has_hover_boots() or climb_anywhere() or hookshot_anywhere() or glitch_megaflip()) end,
        },
        ["locations"] = {
            ...
            ["Shadow Temple Wonder Item"] = function () return can_use_bow() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Spirit Temple Child Back Left"] = {
        ["events"] = {
            ["SHIELD_DEKU"] = function () return setting('restoreBrokenActors') end,
        },
        ["exits"] = {
    ...
    ["Spirit Temple Child Climb"] = {
        ["exits"] = {
            ["Spirit Temple Statue"] = function () return has_explosives() or has_sunlight_arrows() end,
    ...
    ["Water Temple"] = {
        ["exits"] = {
            ["Lake Hylia Near Water Temple"] = function () return true end,
    ...
    ["Water Temple Main"] = {
        ["exits"] = {
            ...
            ["Water Temple Antichamber Room"] = function () return can_longshot() and event('WATER_LEVEL_RESET') or climb_anywhere() end,
    ...
    ["Bottom of the Well"] = {
        ["exits"] = {
            ["Kakariko Well"] = function () return true end,
            ["Bottom of the Well Entrance"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ...
    ["Bottom of the Well Main"] = {
        ["events"] = {
            ["BOTW_MQ_CENTER_GATES_LOWERED"] = function () return can_play_zelda() end,
            ["BOTW_MQ_DRAIN_WATER"] = function () return has_ranged_weapon() or has_explosives_or_hammer() or can_use_sticks() or can_use_sword() end,
            ["BOTW_MQ_EYE_GATE"] = function () return has_explosives_or_hammer() and can_hit_triggers_distance() end,
        },
        ["exits"] = {
            ["Bottom of the Well Entrance"] = function () return is_child() end,
            ["Bottom of the Well Main Room Center"] = function () return event('BOTW_MQ_CENTER_GATES_LOWERED') end,
            ["Bottom of the Well Main Room Center Switch Corner"] = function () return has_explosives_or_hammer() or climb_anywhere() or hookshot_anywhere() end,
            ["Bottom of the Well Basement"] = function () return true end,
            ["Bottom of the Well Coffin Room"] = function () return (event('BOTW_MQ_DRAIN_WATER') or can_dive_small() or can_hookshot()) and small_keys_botw(2) end,
            ["Bottom of the Well Before Dead Hand"] = function () return event('BOTW_MQ_DRAIN_WATER') and is_child() end,
            ["Bottom of the Well Main Side Room Before Pit"] = function () return is_child() end,
        },
        ["locations"] = {
            ["MQ Bottom of the Well Pot Lobby Alcove"] = function () return event('BOTW_MQ_EYE_GATE') end,
            ["MQ Bottom of the Well Alcove Boulders"] = function () return has_explosives_or_hammer() end,
            ...
            ["MQ Bottom of the Well Wonder Item Main Room Left 1"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Main Room Left 2"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Main Room Left 3"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Main Room Left 4"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Main Room Right 1"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Main Room Right 2"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Main Room Right 3"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Main Room Right 4"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Main Room Center Switch Corner"] = {
        ["events"] = {
            ["BOTW_CENTER_WEST_TORCH"] = function () return true end,
        },
        ["exits"] = {
            ["Bottom of the Well Main"] = function () return has_explosives_or_hammer() or climb_anywhere() or hookshot_anywhere() end,
            ["Bottom of the Well Main Room Center"] = function () return has_hover_boots() or trick('OOT_BOTW_MQ_BACKFLIP') or hookshot_anywhere() or climb_anywhere() end,
            ["Bottom of the Well Basement"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Main Room Center"] = {
        ["events"] = {
            ["BOTW_CENTER_EAST_TORCH"] = function () return can_hit_triggers_distance() or can_hookshot() or has_explosives() or can_use_mask_blast() end,
        },
        ["exits"] = {
            ["Bottom of the Well Wallmaster Main"] = function () return soul_wallmaster() end,
            ["Bottom of the Well Main Room Center Switch Corner"] = function () return has_hover_boots() or trick('OOT_BOTW_MQ_BACKFLIP') or hookshot_anywhere() or climb_anywhere() end,
            ["Bottom of the Well West Center Room"] = function () return event('BOTW_CENTER_WEST_TORCH') end,
            ["Bottom of the Well East Center Room"] = function () return event('BOTW_CENTER_EAST_TORCH') end,
            ["Bottom of the Well Basement Switch Ledge"] = function () return event('BOTW_MQ_GATE_FROM_SWITCH') end,
            ["Bottom of the Well Basement"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Bottom of the Well Map Chest"] = function () return true end,
            ["MQ Bottom of the Well Pot Lobby Cage 1"] = function () return true end,
            ["MQ Bottom of the Well Pot Lobby Cage 2"] = function () return true end,
            ["MQ Bottom of the Well Pot Lobby Cage 3"] = function () return true end,
            ["MQ Bottom of the Well Lobby Cage Big Fairy"] = function () return can_play_sun() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Basement"] = {
        ["exits"] = {
            ["Bottom of the Well Main"] = function () return true end,
            ["Bottom of the Well Basement Switch Ledge"] = function () return climb_anywhere() or hookshot_anywhere() end,
            ["Bottom of the Well Wallmaster Basement"] = function () return soul_wallmaster() end,
        },
        ["locations"] = {
            ["MQ Bottom of the Well Lens Chest"] = function () return event('BOTW_MQ_REDEAD_CHEST') and (has_weapon() or can_use_sticks() or can_use_din() or has_iron_boots() or can_play_sun()) end,
    ...
    ["Bottom of the Well West Center Room"] = {
        ["exits"] = {
            ["Bottom of the Well Main Room Center"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Bottom of the Well GS West Middle Room"] = function () return gs() and can_damage_skull() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well East Center Room"] = {
        ...
        ["locations"] = {
            ["MQ Bottom of the Well East Middle Room Key"] = function () return true end,
            ["MQ Bottom of the Well Pot Side Room 1"] = function () return true end,
            ["MQ Bottom of the Well Pot Side Room 2"] = function () return true end,
            ["MQ Bottom of the Well Pot Side Room 3"] = function () return true end,
            ["MQ Bottom of the Well Wonder Item Side Room 1"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Side Room 2"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Side Room 3"] = function () return can_use_slingshot() end,
            ["MQ Bottom of the Well Wonder Item Side Room 4"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Basement Switch Ledge"] = {
        ["events"] = {
            ["BOTW_MQ_REDEAD_CHEST"] = function () return true end,
        },
        ["exits"] = {
            ["Bottom of the Well Basement"] = function () return true end,
            ["Bottom of the Well Main Room Center"] = function () return event('BOTW_MQ_GATE_FROM_SWITCH') and climb_anywhere() and hookshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Coffin Room"] = {
        ["exits"] = {
            ["Bottom of the Well Main"] = function () return (event('BOTW_MQ_DRAIN_WATER') or can_dive_small() or can_hookshot()) and small_keys_botw(2) end,
        },
        ["locations"] = {
            ["MQ Bottom of the Well GS Coffin Room"] = function () return gs() and can_damage_skull() end,
            ["MQ Bottom of the Well Heart Coffin 1"] = function () return has_fire_or_sticks() end,
            ["MQ Bottom of the Well Heart Coffin 2"] = function () return has_fire_or_sticks() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Before Dead Hand"] = {
        ["exits"] = {
            ["Bottom of the Well Main"] = function () return event('BOTW_MQ_DRAIN_WATER') and is_child() end,
            ["Bottom of the Well Dead Hand Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Dead Hand Room"] = {
        ["exits"] = {
            ["Bottom of the Well Before Dead Hand"] = function () return soul_enemy(SOUL_ENEMY_DEAD_HAND) and (has_weapon() or (can_use_sticks() and trick('OOT_DEAD_HAND_STICKS'))) end,
        },
        ["locations"] = {
            ["MQ Bottom of the Well Compass Chest"] = function () return soul_enemy(SOUL_ENEMY_DEAD_HAND) and (has_weapon() or (can_use_sticks() and trick('OOT_DEAD_HAND_STICKS'))) end,
            ["MQ Bottom of the Well Dead Hand Key"] = function () return has_explosives() end,
            ["MQ Bottom of the Well Grass Dead-Hand 1"] = function () return can_cut_grass() end,
            ["MQ Bottom of the Well Grass Dead-Hand 2"] = function () return can_cut_grass() end,
            ["MQ Bottom of the Well Grass Dead-Hand 3"] = function () return can_cut_grass() end,
            ["MQ Bottom of the Well Grass Dead-Hand 4"] = function () return can_cut_grass() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Main Side Room Before Pit"] = {
        ["exits"] = {
            ["Bottom of the Well Main"] = function () return is_child() end,
            ["Bottom of the Well Pit Before Cage"] = function () return can_hit_triggers_distance() or can_boomerang() or has_explosives() or can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Pit Before Cage"] = {
        ["exits"] = {
            ["Bottom of the Well Main Side Room Before Pit"] = function () return true end,
            ["Bottom of the Well Cage"] = function () return small_keys_botw(2) end,
            ["Bottom of the Well Wallmaster Pit"] = function () return soul_wallmaster() end,
        },
        ["age_change"] = true,
    },
    ["Bottom of the Well Cage"] = {
        ["events"] = {
            ["BOTW_MQ_GATE_FROM_SWITCH"] = function () return has_explosives() end,
        },
        ["exits"] = {
            ["Bottom of the Well Pit Before Cage"] = function () return small_keys_botw(2) end,
        },
        ["age_change"] = true,
    },
    ["Deku Tree"] = {
        ["exits"] = {
            ["Kokiri Forest Near Deku Tree"] = function () return true end,
            ["MQ Deku Tree Lobby"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Lobby"] = {
        ...
        ["exits"] = {
            ["Deku Tree"] = function () return true end,
            ["MQ Deku Tree 2nd Floor"] = function () return true end,
            ["MQ Deku Tree Basement"] = function () return has_fire() or (can_use_sticks() and event('DEKU_MQ_MAIN_TORCH')) end,
        },
        ["locations"] = {
    ...
    ["MQ Deku Tree 2nd Floor"] = {
        ["exits"] = {
            ["MQ Deku Tree Lobby"] = function () return true end,
            ["MQ Deku Tree 3rd Floor"] = function () return true end,
            ["MQ Deku Tree Room Before Compass Room Entry Door"] = function () return has_fire() or (can_use_sticks() and event('DEKU_MQ_MAIN_TORCH')) or (event('DEKU_MQ_MAIN_TORCH') and can_use_bow()) end,
        },
        ["locations"] = {
            ...
            ["MQ Deku Tree Main Room 2nd Floor Crate"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room Before Compass Room Entry Door"] = {
        ["exits"] = {
            ["MQ Deku Tree 2nd Floor"] = function () return true end,
            ["MQ Deku Tree Room Before Compass Room Door to 2nd Floor"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room Before Compass Room Door to 2nd Floor"] = {
        ["exits"] = {
            ["MQ Deku Tree Room Before Compass Room Entry Door"] = function () return true end,
            ["MQ Deku Tree Room Before Compass Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room Before Compass Room"] = {
        ["events"] = {
            ["DEKU_MQ_BEFORE_COMPASS_EYE_SWITCH"] = function () return can_use_slingshot() or can_use_bow() end,
        },
        ["exits"] = {
            ["MQ Deku Tree Room Before Compass Room Door to 2nd Floor"] = function () return true end,
            ["MQ Deku Tree Compass Room Entry Door"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Deku Tree Heart Before Compass"] = function () return true end,
            ["MQ Deku Tree Grass Room Before Compass 1"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Compass 2"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Compass 3"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Compass 4"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Compass 5"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Compass 6"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Compass 7"] = function () return can_cut_grass() end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Compass Room Entry Door"] = {
        ["exits"] = {
            ["MQ Deku Tree Room Before Compass Room"] = function () return true end,
            ["MQ Deku Tree Compass Room Exit Door"] = function () return event('DEKU_MQ_BEFORE_COMPASS_EYE_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Compass Room Exit Door"] = {
        ["exits"] = {
            ["MQ Deku Tree Compass Room Entry Door"] = function () return true end,
            ["MQ Deku Tree MQ Compass Room Entrance"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree MQ Compass Room Entrance"] = {
        ["exits"] = {
            ["MQ Deku Tree Compass Room Exit Door"] = function () return true end,
            ["MQ Deku Tree MQ Compass Room Chest Side"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree MQ Compass Room Chest Side"] = {
        ["exits"] = {
            ["MQ Deku Tree MQ Compass Room Entrance"] = function () return can_use_slingshot() or climb_anywhere() or hookshot_anywhere() end,
            ["MQ Deku Tree MQ Compass Room Chest Side Alcove"] = function () return has_bombchu() or has_mask_blast() or (can_play_time() and has_explosives_or_hammer()) or hookshot_anywhere() or climb_anywhere() end,
        },
        ["locations"] = {
    ...
    ["MQ Deku Tree MQ Compass Room Chest Side Alcove"] = {
        ["exits"] = {
            ["MQ Deku Tree MQ Compass Room Chest Side"] = function () return has_explosives_or_hammer() or hookshot_anywhere() or climb_anywhere() end,
        },
        ["locations"] = {
            ...
            ["MQ Deku Tree Boulders"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree 3rd Floor"] = {
        ["events"] = {
            ["DEKU_MQ_MAIN_TORCH"] = function () return true end,
        },
        ["exits"] = {
            ["MQ Deku Tree 2nd Floor"] = function () return true end,
            ["MQ Deku Tree Lobby"] = function () return true end,
            ["MQ Deku Tree Basement"] = function () return true end,
            ["MQ Deku Tree Slingshot Room Entry Door"] = function () return has_fire() or event('DEKU_MQ_MAIN_TORCH') end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Slingshot Room Entry Door"] = {
        ["exits"] = {
            ["MQ Deku Tree 3rd Floor"] = function () return true end,
            ["MQ Deku Tree Slingshot Room Exit Door"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Slingshot Room Exit Door"] = {
        ["exits"] = {
            ["MQ Deku Tree Slingshot Room Entry Door"] = function () return event('DEKU_MQ_SLINGSHOT_ENEMIES') end,
            ["MQ Deku Tree MQ Slingshot Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree MQ Slingshot Room"] = {
        ["events"] = {
            ["DEKU_MQ_SLINGSHOT_ENEMIES"] = function () return soul_deku_baba() and soul_enemy(SOUL_ENEMY_GOHMA_LARVA) and (has_weapon() or can_use_sticks() or has_ranged_weapon_child()) end,
            ["DEKU_MQ_SLINGSHOT_TORCH"] = function () return has_fire() or can_use_sticks() end,
        },
        ["exits"] = {
            ["MQ Deku Tree Slingshot Room Exit Door"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Deku Tree Slingshot Chest"] = function () return event('DEKU_MQ_SLINGSHOT_ENEMIES') end,
            ["MQ Deku Tree Slingshot Room Far Chest"] = function () return event('DEKU_MQ_SLINGSHOT_TORCH') end,
            ...
            ["MQ Deku Tree Slingshot Room Large Crate 1"] = function () return true end,
            ["MQ Deku Tree Slingshot Room Large Crate 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Basement"] = {
        ["events"] = {
            ["DEKU_MQ_WATER_PATH_TORCH1"] = function () return has_fire() or can_use_sticks() end,
            ["DEKU_MQ_BASEMENT_EYE_SWITCH"] = function () return can_use_bow() or can_use_slingshot() end,
        },
        ["exits"] = {
            ["MQ Deku Tree Lobby"] = function () return true end,
            ["MQ Deku Tree Basement Ledge"] = function () return is_adult() or event('DEKU_BLOCK') or trick('OOT_DEKU_SKIP') or has_hover_boots() or hookshot_anywhere() or climb_anywhere() end,
            ["MQ Deku Tree Room Before Water Room Entry Door"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["MQ Deku Tree Room Before Water Room Entry Door"] = {
        ["exits"] = {
            ["MQ Deku Tree Basement"] = function () return true end,
            ["MQ Deku Tree Room Before Water Room Door to Basement"] = function () return event('DEKU_MQ_BASEMENT_EYE_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room Before Water Room Door to Basement"] = {
        ["exits"] = {
            ["MQ Deku Tree Room Before Water Room Entry Door"] = function () return event('DEKU_MQ_ROOM_BEFORE_WATER_CLEAR') end,
            ["MQ Deku Tree Room Before Water Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room Before Water Room"] = {
        ["events"] = {
            ["DEKU_MQ_WATER_PATH_TORCH1"] = function () return has_fire() end,
            ["DEKU_MQ_ROOM_BEFORE_WATER_CLEAR"] = function () return soul_deku_scrub() and soul_enemy(SOUL_ENEMY_GOHMA_LARVA) and can_damage() end,
        },
        ["exits"] = {
            ["MQ Deku Tree Room Before Water Room Door to Basement"] = function () return true end,
            ["MQ Deku Tree Room Before Water Room Door to Water Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Deku Tree Grass Room Before Spike 1"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Spike 2"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Spike 3"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Room Before Spike 4"] = function () return can_cut_grass() end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room Before Water Room Door to Water Room"] = {
        ["exits"] = {
            ["MQ Deku Tree Room Before Water Room"] = function () return true end,
            ["MQ Deku Tree Water Room Door to Room Before Water Room"] = function () return event('DEKU_MQ_WATER_PATH_TORCH1') end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Water Room Door to Room Before Water Room"] = {
        ["exits"] = {
            ["MQ Deku Tree Room Before Water Room Door to Water Room"] = function () return true end,
            ["MQ Deku Tree Water Room Front"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Water Room Front"] = {
        ["events"] = {
            ["MQ_DEKU_WATER_TORCHES"] = function () return is_child() and has_shield() and can_use_sticks() or (can_use_bow() and can_play_elegy()) or (can_use_sticks() and trick('OOT_DEKU_MQ_WATER_ROOM_SPIKE_ROLL')) end,
        },
        ["exits"] = {
            ["MQ Deku Tree Water Room Door to Room Before Water Room"] = function () return true end,
            ["MQ Deku Tree Water Room Back"] = function () return is_child() and has_shield() or can_longshot() or (can_hookshot() and has_iron_boots()) or trick('OOT_DEKU_MQ_WATER_ROOM_SPIKE_ROLL') end,
        },
        ["locations"] = {
    ...
    ["MQ Deku Tree Water Room Back"] = {
        ...
        ["exits"] = {
            ["MQ Deku Tree Water Room Front"] = function () return true end,
            ["MQ Deku Tree Water Room Door to Room After Water Room"] = function () return climb_anywhere() or hookshot_anywhere() or can_damage() end,
        },
        ["locations"] = {
    ...
    ["MQ Deku Tree Water Room Door to Room After Water Room"] = {
        ["exits"] = {
            ["MQ Deku Tree Water Room Back"] = function () return true end,
            ["MQ Deku Tree Room After Water Room Door to Water Room"] = function () return event('MQ_DEKU_WATER_TORCHES') end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room After Water Room Door to Water Room"] = {
        ["exits"] = {
            ["MQ Deku Tree Water Room Door to Room After Water Room"] = function () return event('DEKU_MQ_ROOM_AFTER_WATER_CLEAR') end,
            ["MQ Deku Tree Room After Water Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Room After Water Room"] = {
        ["events"] = {
            ["DEKU_MQ_ROOM_AFTER_WATER_CLEAR"] = function () return soul_keese() and soul_deku_scrub() and soul_enemy(SOUL_ENEMY_GOHMA_LARVA) and (has_weapon() or has_ranged_weapon_child()) end,
        },
        ["exits"] = {
            ["MQ Deku Tree Room After Water Room Door to Water Room"] = function () return true end,
            ["MQ Deku Tree Room After Water Room Door to Gravestone Room"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["MQ Deku Tree Room After Water Room Door to Gravestone Room"] = {
        ["exits"] = {
            ["MQ Deku Tree Room After Water Room"] = function () return true end,
            ["MQ Deku Tree Gravestone Room Door to Room After Water Room"] = function () return event('DEKU_MQ_ROOM_AFTER_WATER_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Gravestone Room Door to Room After Water Room"] = {
        ["exits"] = {
            ["MQ Deku Tree Room After Water Room Door to Gravestone Room"] = function () return true end,
            ["MQ Deku Tree MQ Grave Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree MQ Grave Room"] = {
        ["events"] = {
            ["STICKS"] = function () return can_kill_baba_sticks() end,
            ["NUTS"] = function () return can_kill_baba_nuts() end,
            ["DEKU_MQ_GRAVE_ROOM_WEBS"] = function () return can_use_sticks() or has_fire() or can_use_bow() end,
        },
        ["exits"] = {
            ["MQ Deku Tree Gravestone Room Door to Room After Water Room"] = function () return true end,
            ["MQ Deku Tree Back Side Room Entry Door"] = function () return event('DEKU_MQ_GRAVE_ROOM_WEBS') end,
            ["MQ Deku Tree Basement Ledge"] = function () return is_child() and event('DEKU_MQ_GRAVE_ROOM_WEBS') end,
        },
        ["locations"] = {
            ...
            ["MQ Deku Tree Wonder Item 1"] = function () return can_use_sword_or_sticks() end,
            ["MQ Deku Tree Wonder Item 2"] = function () return can_use_sword_or_sticks() end,
            ["MQ Deku Tree Wonder Item 3"] = function () return can_use_sword_or_sticks() end,
            ["MQ Deku Tree Wonder Item 4"] = function () return can_use_sword_or_sticks() end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Back Side Room Entry Door"] = {
        ["exits"] = {
            ["MQ Deku Tree MQ Grave Room"] = function () return true end,
            ["MQ Deku Tree Back Side Room Exit Door"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Back Side Room Exit Door"] = {
        ["exits"] = {
            ["MQ Deku Tree Back Side Room Entry Door"] = function () return true end,
            ["MQ Deku Tree Back Side Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Back Side Room"] = {
        ...
        ["locations"] = {
            ["MQ Deku Tree GS Back Room"] = function () return gs() and can_collect_distance() end,
            ["MQ Deku Tree Grass Back Room 1"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Back Room 2"] = function () return can_cut_grass() end,
            ["MQ Deku Tree Grass Back Room 3"] = function () return can_cut_grass() end,
        },
        ["age_change"] = true,
    },
    ["MQ Deku Tree Basement Ledge"] = {
        ...
        ["exits"] = {
            ["MQ Deku Tree Basement"] = function () return true end,
            ["Deku Tree Before Boss"] = function () return can_use_sticks() and (is_adult() or (is_child() and (has_hover_boots() or event('DEKU_BLOCK')))) or has_fire() end,
            ["MQ Deku Tree MQ Grave Room"] = function () return is_child() end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Entrance"] = {
        ["exits"] = {
            ["Dodongo Cavern"] = function () return true end,
            ["Dodongo Cavern Main Room Lower"] = function () return has_bombflowers() or can_hammer() or has_blue_fire_arrows_mudwall() end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Main Room Lower"] = {
        ["events"] = {
            ["MQ_DC_PILLAR_RAISE"] = function () return has_bombflowers() or can_hammer() end,
            ["MQ_DC_OPEN_SKULL"] = function () return (hookshot_anywhere() or climb_anywhere()) and has_explosives() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Entrance"] = function () return has_explosives_or_hammer() or (is_adult() and has_bombflowers()) or has_blue_fire_arrows_mudwall() end,
            ["Dodongo Cavern Main Room Inside Skull"] = function () return event('MQ_DC_OPEN_SKULL') end,
            ["Dodongo Cavern Staircase Room Lower"] = function () return event('MQ_DC_STAIRCASE_SWITCH') end,
            ["Dodongo Cavern Main Room Upper Skull Side"] = function () return event('MQ_DC_PILLAR_RAISE') or climb_anywhere() or hookshot_anywhere() end,
            ["Dodongo Cavern Main Room Upper Entrance Side"] = function () return climb_anywhere() or longshot_anywhere() end,
            ["Dodongo Cavern Lower Tunnel"] = function () return has_explosives_or_hammer() or (event('DC_MQ_SHORTCUT') and has_goron_bracelet()) or has_blue_fire_arrows_mudwall() end,
            ["Dodongo Cavern Bomb Bag Ledge"] = function () return is_adult() or climb_anywhere() or can_hookshot() end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Main Room Upper Skull Side"] = {
        ["events"] = {
            ["MQ_DC_OPEN_SKULL"] = function () return has_explosives() end,
            ["MQ_DC_STAIRCASE_SWITCH"] = function () return true end,
            ["DC_MQ_SHORTCUT"] = function () return has_explosives_or_hammer() or can_use_din() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Main Room Lower"] = function () return true end,
            ["Dodongo Cavern Vanilla Bomb Bag Room Upper"] = function () return event('DC_MQ_SHORTCUT') or hookshot_anywhere() or (climb_anywhere() and can_jump_slash()) end,
            ["Dodongo Cavern Main Room Upper Entrance Side"] = function () return longshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Staircase Room Lower"] = {
        ["events"] = {
            ["STICKS"] = function () return soul_deku_baba() and ((has_bombflowers() or can_hammer() or has_blue_fire_arrows_mudwall()) and (has_weapon() or can_boomerang())) end,
            ["DC_MQ_STAIRCASE"] = function () return has_bombflowers() or can_use_din() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Main Room Lower"] = function () return true end,
            ["Dodongo Cavern Staircase Room Upper"] = function () return event('DC_MQ_STAIRCASE') or climb_anywhere() or hookshot_anywhere() end,
            ["Dodongo Cavern MQ Song of Time Block Room"] = function () return has_bombflowers() or can_hammer() or has_blue_fire_arrows_mudwall() end,
        },
        ["locations"] = {
            ...
            ["MQ Dodongo Cavern Staircase Room Lower Large Crate 1"] = function () return true end,
            ["MQ Dodongo Cavern Staircase Room Lower Large Crate 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ...
    ["Dodongo Cavern Staircase Room Upper"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Dodongo Cavern Staircase Room Upper Large Crate 1"] = function () return true end,
            ["MQ Dodongo Cavern Staircase Room Upper Large Crate 2"] = function () return true end,
            ["MQ Dodongo Cavern Staircase Room Upper Large Crate 3"] = function () return true end,
            ["MQ Dodongo Cavern Staircase Room Upper Large Crate 4"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Room After Upper Staircase"] = {
        ["events"] = {
            ["DC_MQ_DEFEAT_DONDONGOS_AFTER_STAIRCASE"] = function () return soul_dodongo() and can_damage() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Staircase Room Upper"] = function () return true end,
            ["Dodongo Cavern Main Room Upper Entrance Side"] = function () return event('DC_MQ_DEFEAT_DONDONGOS_AFTER_STAIRCASE') end,
        },
        ["locations"] = {
            ["MQ Dodongo Cavern Compass Chest"] = function () return event('DC_MQ_DEFEAT_DONDONGOS_AFTER_STAIRCASE') end,
    ...
    ["Dodongo Cavern Main Room Upper Entrance Side"] = {
        ["exits"] = {
            ["Dodongo Cavern Main Room Lower"] = function () return true end,
            ["Dodongo Cavern Room After Upper Staircase"] = function () return true end,
            ["Dodongo Cavern Main Room Upper Skull Side"] = function () return longshot_anywhere() end,
            ["Dodongo Cavern Vanilla Bomb Bag Room Lower"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Vanilla Bomb Bag Room Lower"] = {
        ["events"] = {
            ["OPEN_MQ_DC_LARVAE_ROOM"] = function () return can_use_sticks() or has_fire() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Main Room Upper Entrance Side"] = function () return true end,
            ["Dodongo Cavern Room Before Upper Lizalfos"] = function () return can_use_sticks() and event('OPEN_MQ_DC_LARVAE_ROOM') or has_fire() end,
            ["Dodongo Cavern Vanilla Bomb Bag Room Upper"] = function () return can_hookshot() or has_hover_boots() or (is_adult() and trick('OOT_DC_JUMP')) or climb_anywhere() end,
            ["Dodongo Cavern MQ Larvae Room"] = function () return event('OPEN_MQ_DC_LARVAE_ROOM') end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Vanilla Bomb Bag Room Upper"] = {
        ["events"] = {
            ["DC_MQ_SHORTCUT"] = function () return has_bombflowers() or can_use_din() or can_use_bow() or has_explosives_or_hammer() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Vanilla Bomb Bag Room Lower"] = function () return true end,
            ["Dodongo Cavern Main Room Upper Skull Side"] = function () return event('DC_MQ_SHORTCUT') or climb_anywhere() end,
    ...
    ["Dodongo Cavern MQ Larvae Room"] = {
        ["events"] = {
            ["MQ_DC_CLEAR_LARVAE_ROOM"] = function () return soul_enemy(SOUL_ENEMY_GOHMA_LARVA) and can_damage() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Vanilla Bomb Bag Room Lower"] = function () return event('MQ_DC_CLEAR_LARVAE_ROOM') end,
        },
        ["locations"] = {
            ["MQ Dodongo Cavern Larvae Room Chest"] = function () return event('MQ_DC_CLEAR_LARVAE_ROOM') end,
            ["MQ Dodongo Cavern GS Larve Room"] = function () return gs() and can_damage_skull() end,
            ["MQ Dodongo Cavern Larve Room Large Crate 1"] = function () return true end,
            ["MQ Dodongo Cavern Larve Room Large Crate 2"] = function () return true end,
            ["MQ Dodongo Cavern Larve Room Large Crate 3"] = function () return true end,
            ["MQ Dodongo Cavern Larve Room Large Crate 4"] = function () return true end,
            ["MQ Dodongo Cavern Larve Room Large Crate 5"] = function () return true end,
            ["MQ Dodongo Cavern Larve Room Large Crate 6"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Room Before Upper Lizalfos"] = {
        ["events"] = {
            ["MQ_DC_ROOM_BEFORE_UPPER_LIZALFOS_GOLD_TORCH"] = function () return can_use_sticks() and event('OPEN_MQ_DC_LARVAE_ROOM') or has_fire() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Upper Lizalfos"] = function () return event('MQ_DC_ROOM_BEFORE_UPPER_LIZALFOS_GOLD_TORCH') and can_use_sticks() and has_goron_bracelet() or has_explosives_or_hammer() or has_blue_fire_arrows_mudwall() end,
            ["Dodongo Cavern Vanilla Bomb Bag Room Lower"] = function () return has_fire() end,
        },
        ["locations"] = {
            ...
            ["MQ Dodonge Cavern Boulder before Miniboss"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Room After Upper Lizalfos"] = {
        ["exits"] = {
            ...
            ["Dodongo Cavern Vanilla Bomb Bag Room Upper"] = function () return has_goron_bracelet() and (has_weapon() or can_use_sticks() or has_ranged_weapon()) or has_explosives_or_hammer() or is_adult() or climb_anywhere() or hookshot_anywhere() end,
        },
        ["locations"] = {
            ...
            ["MQ Dodongo Cavern Room After Upper Lizalfos Large Crate 1"] = function () return true end,
            ["MQ Dodongo Cavern Room After Upper Lizalfos Large Crate 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Upper Lizalfos"] = {
        ["events"] = {
            ["MQ_DC_UPPER_LIZALFOS_CLEAR_FROM_UPPER"] = function () return soul_lizalfos_dinalfos() and (can_use_sticks() or has_weapon() or can_use_slingshot()) end,
            ["MQ_DC_LOWER_LIZALFOS_CLEAR_FROM_UPPER"] = function () return (time_travel_at_will() and (has_explosives_or_hammer() or hookshot_anywhere() or climb_anywhere())) and soul_lizalfos_dinalfos() and (can_use_sticks() or has_weapon() or can_use_slingshot()) end,
        },
        ["exits"] = {
            ["Dodongo Cavern Room Before Upper Lizalfos"] = function () return event('MQ_DC_UPPER_LIZALFOS_CLEAR_FROM_UPPER') or event('MQ_DC_UPPER_LIZALFOS_CLEAR_FROM_LOWER') end,
            ["Dodongo Cavern Room After Upper Lizalfos"] = function () return event('MQ_DC_UPPER_LIZALFOS_CLEAR_FROM_UPPER') or event('MQ_DC_UPPER_LIZALFOS_CLEAR_FROM_LOWER') end,
        },
        ["locations"] = {
            ["MQ Dodongo Cavern GS Upper Lizalfos"] = function () return gs() and (has_explosives_or_hammer() or (climb_anywhere() and can_damage_skull()) or longshot_anywhere()) end,
            ...
            ["MQ Dodongo Cavern Heart Lizalfos Room"] = function () return has_explosives_or_hammer() or hookshot_anywhere() or climb_anywhere() end,
            ["MQ Dodonge Cavern Miniboss Boulders"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Lower Tunnel"] = {
        ["events"] = {
            ["MQ_DC_LOWER_TUNNEL_EYE_SWITCH"] = function () return can_use_bow() or ((has_bombflowers() or can_use_din()) and can_use_slingshot()) end,
        },
        ["exits"] = {
            ["Dodongo Cavern Corridor Before Lower Lizalfos"] = function () return event('MQ_DC_LOWER_TUNNEL_EYE_SWITCH') end,
            ["Dodongo Cavern Main Room Lower"] = function () return true end,
            ["Dodongo Cavern Lower Tunnel Side Room"] = function () return has_bombflowers() or has_explosives_or_hammer() or has_blue_fire_arrows_mudwall() end,
        },
        ["locations"] = {
            ...
            ["MQ Dodonge Cavern Corridor Boulders"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Lower Tunnel Side Room"] = {
        ["exits"] = {
            ["Dodongo Cavern Lower Tunnel"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Lower Lizalfos"] = {
        ["events"] = {
            ["MQ_DC_LOWER_LIZALFOS_CLEAR_FROM_LOWER"] = function () return soul_lizalfos_dinalfos() and (can_use_sticks() or has_weapon() or can_use_slingshot()) end,
            ["MQ_DC_UPPER_LIZALFOS_CLEAR_FROM_LOWER"] = function () return ((climb_anywhere() or longshot_anywhere()) and time_travel_at_will()) and soul_lizalfos_dinalfos() and (can_use_sticks() or has_weapon() or can_use_slingshot()) end,
        },
        ["exits"] = {
            ["Dodongo Cavern Corridor Before Lower Lizalfos"] = function () return event('MQ_DC_LOWER_LIZALFOS_CLEAR_FROM_UPPER') or event('MQ_DC_LOWER_LIZALFOS_CLEAR_FROM_LOWER') end,
            ["Dodongo Cavern Poe Room"] = function () return event('MQ_DC_LOWER_LIZALFOS_CLEAR_FROM_UPPER') or event('MQ_DC_LOWER_LIZALFOS_CLEAR_FROM_LOWER') end,
        },
        ["locations"] = {
            ["MQ Dodongo Cavern Heart Lizalfos Room"] = function () return true end,
            ["MQ Dodongo Cavern GS Upper Lizalfos"] = function () return gs() and (longshot_anywhere() or (climb_anywhere() and can_damage_skull())) end,
            ["MQ Dodongo Cavern Pot Miniboss 1"] = function () return climb_anywhere() or longshot_anywhere() end,
            ["MQ Dodongo Cavern Pot Miniboss 2"] = function () return climb_anywhere() or longshot_anywhere() end,
            ["MQ Dodongo Cavern Pot Miniboss 3"] = function () return climb_anywhere() or longshot_anywhere() end,
            ["MQ Dodongo Cavern Pot Miniboss 4"] = function () return climb_anywhere() or longshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Poe Room"] = {
        ["events"] = {
            ["MQ_DC_POE_ROOM_BOMB_SWITCHES"] = function () return can_use_bow() or has_bombflowers() or can_use_din() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Lower Lizalfos"] = function () return true end,
            ["Dodongo Cavern Bomb Bag Ledge"] = function () return event('MQ_DC_POE_ROOM_BOMB_SWITCHES') end,
            ["Dodongo Cavern MQ Poe Room Side Room"] = function () return event('MQ_DC_POE_ROOM_BOMB_SWITCHES') end,
        },
        ["locations"] = {
            ...
            ["MQ Dodongo Cavern Poe Room Large Crate 1"] = function () return true end,
            ["MQ Dodongo Cavern Poe Room Large Crate 2"] = function () return true end,
            ["MQ Dodongo Cavern Poe Room Large Crate 3"] = function () return true end,
            ["MQ Dodongo Cavern Poe Room Large Crate 4"] = function () return true end,
            ["MQ Dodongo Cavern Poe Room Large Crate 5"] = function () return true end,
            ["MQ Dodongo Cavern Poe Room Large Crate 6"] = function () return true end,
            ["MQ Dodongo Cavern Poe Room Large Crate 7"] = function () return true end,
            ["MQ Dodongo Cavern Poe Room Large Crate 8"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern MQ Poe Room Side Room"] = {
        ["exits"] = {
            ["Dodongo Cavern Poe Room"] = function () return soul_keese() and soul_deku_scrub() and can_damage() end,
        },
        ["locations"] = {
            ["MQ Dodongo Cavern GS Poe Room Side"] = function () return gs() and can_collect_distance() end,
            ["MQ Dodongo Cavern Grass Green Corridor Side Room 1"] = function () return can_cut_grass() end,
            ["MQ Dodongo Cavern Grass Green Corridor Side Room 2"] = function () return can_cut_grass() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Dodongo Cavern Main Room Inside Skull"] = {
        ["exits"] = {
            ["Dodongo Cavern Main Room Lower"] = function () return event('MQ_DC_OPEN_SKULL') end,
            ["Dodongo Cavern Pre-Boss Lobby"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Dodongo Cavern Pre-Boss Lobby"] = {
        ["exits"] = {
            ["Dodongo Cavern Main Room Inside Skull"] = function () return true end,
            ["Dodongo Cavern Boss"] = function () return event('MQ_DC_BOSS_SWITCH') end,
            ["Dodongo Cavern Boss Loop Start"] = function () return true end,
            ["Dodongo Cavern Boss Loop Ending Ledge"] = function () return is_adult() or climb_anywhere() or hookshot_anywhere() end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Boss Loop Start"] = {
        ["events"] = {
            ["MQ_DC_BOSS_LOOP_FIRE_WALL_CRYSTAL_BEFORE_FIRE"] = function () return has_bombs() or has_bombchu() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Pre-Boss Lobby"] = function () return true end,
            ["Dodongo Cavern Boss Loop After Fire Wall"] = function () return is_adult() or climb_anywhere() or hookshot_anywhere() or event('MQ_DC_BOSS_LOOP_FIRE_WALL_CRYSTAL_BEFORE_FIRE') end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Boss Loop After Fire Wall"] = {
        ["events"] = {
            ["MQ_DC_BOSS_LOOP_FIRE_WALL_CRYSTAL_AFTER_FIRE"] = function () return has_weapon() or has_bombs() or can_use_mask_blast() or has_bombchu() or can_use_bow() or can_use_slingshot() or can_hookshot() or can_use_sticks() or can_hammer() or can_boomerang() end,
        },
        ["exits"] = {
            ["Dodongo Cavern Boss Loop Side Room"] = function () return true end,
            ["Dodongo Cavern Boss Loop Start"] = function () return event('MQ_DC_BOSS_LOOP_FIRE_WALL_CRYSTAL_AFTER_FIRE') or climb_anywhere() or hookshot_anywhere() end,
            ["Dodongo Cavern Boss Loop After Armos Wall"] = function () return climb_anywhere() or hookshot_anywhere() or has_fire() or has_explosives() end,
        },
        ["locations"] = {
            ["MQ Dodongo Cavern GS Near Boss"] = function () return gs() and can_damage_skull() end,
    ...
    ["Dodongo Cavern Boss Loop Side Room"] = {
        ["exits"] = {
            ["Dodongo Cavern Boss Loop After Fire Wall"] = function () return can_damage() end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Boss Loop After Armos Wall"] = {
        ["exits"] = {
            ["Dodongo Cavern Boss Loop Ending Ledge"] = function () return true end,
            ["Dodongo Cavern Boss Loop After Fire Wall"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Dodongo Cavern Boss Loop Ending Ledge"] = {
        ["events"] = {
            ["MQ_DC_BOSS_SWITCH"] = function () return true end,
        },
        ["exits"] = {
            ["Dodongo Cavern Pre-Boss Lobby"] = function () return true end,
            ["Dodongo Cavern Boss Loop After Armos Wall"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple"] = {
    ...
    ["Fire Temple Entrance"] = {
        ["exits"] = {
            ["Fire Temple"] = function () return true end,
            ["Fire Temple Lower Lobby"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Lower Lobby"] = {
        ["exits"] = {
            ["Fire Temple Entrance"] = function () return true end,
            ["Fire Temple Upper Lobby"] = function () return is_adult() or (is_child() and (climb_anywhere() or hookshot_anywhere())) end,
            ["Fire Temple Hammer Loop Enemies Room"] = function () return small_keys_fire_mq(5) end,
            ["Fire Temple Hammer Loop Goron Cell Like Like Side"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Fire Temple Hammer Loop Goron Cell Like Like Side"] = {
        ["events"] = {
            ["FIRE_MQ_1ST_GORON_LIKELIKE"] = function () return (can_damage() or can_hammer()) and soul_like_like() end,
        },
        ["exits"] = {
            ["Fire Temple Lower Lobby"] = function () return event('FIRE_MQ_1ST_GORON_LIKELIKE') end,
            ["Fire Temple Hammer Loop Goron Cell Map Chest Side"] = function () return event('FIRE_MQ_MAP_CHEST_HAMMER_SWITCH') end,
        },
        ["locations"] = {
            ["MQ Fire Temple Early Lower Left Chest"] = function () return event('FIRE_MQ_1ST_GORON_LIKELIKE') end,
            ["MQ Fire Temple Map Chest"] = function () return event('FIRE_MQ_MAP_CHEST_HAMMER_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Upper Lobby"] = {
        ["exits"] = {
            ["Fire Temple Lower Lobby"] = function () return true end,
            ["Fire Temple Pre-Boss Room Entrance Side"] = function () return has_fire() end,
            ["Fire Temple 1F Lava Bridge Room Main"] = function () return can_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Pre-Boss Room Entrance Side"] = {
        ["exits"] = {
            ...
            ["Fire Temple Pre-Boss Room Cell Corner"] = function () return has_tunic_goron() end,
            ["Fire Temple Pre-Boss"] = function () return has_tunic_goron() and (event('FIRE_MQ_PRE_BOSS_PILLAR') and (is_adult() or has_hover_boots()) or hookshot_anywhere() or climb_anywhere()) end,
            ["Fire Temple Pre-Boss Room Tall Ledges Corner"] = function () return has_tunic_goron() and (climb_anywhere() or can_hookshot() or has_hover_boots()) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Pre-Boss Room Cell Corner"] = {
        ["exits"] = {
            ["Fire Temple Pre-Boss Room Entrance Side"] = function () return has_tunic_goron() end,
            ["Fire Temple Pre-Boss"] = function () return has_tunic_goron() and (has_hover_boots() or climb_anywhere() or hookshot_anywhere()) end,
        },
        ["locations"] = {
            ["MQ Fire Temple Pre-Boss Chest"] = function () return event('FIRE_MQ_PRE_BOSS_TORCHES') end,
            ["MQ Fire Temple Pre-Boss Room Near Cage Large Crate 1"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Pre-Boss Room Near Cage Large Crate 2"] = function () return has_tunic_goron() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Pre-Boss"] = {
        ["exits"] = {
            ["Fire Temple Pre-Boss Room Entrance Side"] = function () return has_tunic_goron() and (is_adult() or hookshot_anywhere() or climb_anywhere() or (event('FIRE_MQ_PRE_BOSS_PILLAR') and has_hover_boots())) end,
            ["Fire Temple Pre-Boss Room Cell Corner"] = function () return has_tunic_goron() and (is_adult() or (can_hookshot() or has_hover_boots() or climb_anywhere())) end,
            ["Fire Temple Pre-Boss Room Tall Ledges Corner"] = function () return has_tunic_goron() and (can_hookshot() or has_hover_boots() or climb_anywhere()) end,
            ["Fire Temple Boss"] = function () return boss_key(BOSS_KEY_FIRE) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Pre-Boss Room Tall Ledges Corner"] = {
        ["events"] = {
            ["FIRE_MQ_PRE_BOSS_TORCHES"] = function () return has_tunic_goron() and can_use_din() and (is_adult() or can_longshot() or has_hover_boots() or can_use_bow() or hookshot_anywhere() or climb_anywhere()) or (has_fire_arrows() and (has_tunic_goron_strict() and is_child() or (has_tunic_goron() and (can_longshot() or hookshot_anywhere() or (has_hover_boots() and (is_adult() or climb_anywhere())))))) end,
        },
        ["exits"] = {
            ["Fire Temple Pre-Boss Room Entrance Side"] = function () return has_tunic_goron() end,
            ["Fire Temple Pre-Boss"] = function () return has_tunic_goron() and (has_hover_boots() or climb_anywhere() or hookshot_anywhere()) end,
        ...
        ["locations"] = {
            ["MQ Fire Temple Pot Pre-Boss 1"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Pot Pre-Boss 2"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Pre-Boss Room Tall Ledge Low Large Crate 1"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Pre-Boss Room Tall Ledge Low Large Crate 2"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Pre-Boss Room Tall Ledge Middle Large Crate"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Pre-Boss Room Tall Ledge Top Large Crate"] = function () return has_tunic_goron() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Hammer Loop Enemies Room"] = {
        ["events"] = {
            ["FIRE_MQ_HAMMER_LOOP_FIRST_CLEAR"] = function () return has_weapon() and soul_keese() and soul_enemy(SOUL_ENEMY_STALFOS) end,
        },
        ["exits"] = {
            ["Fire Temple Lower Lobby"] = function () return small_keys_fire_mq(5) end,
            ["Fire Temple Hammer Loop Iron Knuckle Room"] = function () return event('FIRE_MQ_HAMMER_LOOP_FIRST_CLEAR') end,
        },
        ["locations"] = {
    ...
    ["Fire Temple Hammer Loop Iron Knuckle Room"] = {
        ["events"] = {
            ["FIRE_MQ_HAMMER_LOOP_KNUCKLE_CLEAR"] = function () return soul_iron_knuckle() and has_weapon() end,
        },
        ["exits"] = {
            ["Fire Temple Hammer Loop Enemies Room"] = function () return true end,
            ["Fire Temple Hammer Loop Flare Dancer Room"] = function () return event('FIRE_MQ_HAMMER_LOOP_KNUCKLE_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Fire Temple Pot Hammer Loop 1"] = function () return true end,
            ["MQ Fire Temple Pot Hammer Loop 2"] = function () return true end,
            ["MQ Fire Temple Pot Hammer Loop 3"] = function () return true end,
            ["MQ Fire Temple Pot Hammer Loop 4"] = function () return true end,
            ["MQ Fire Temple Pot Hammer Loop 5"] = function () return true end,
            ["MQ Fire Temple Pot Hammer Loop 6"] = function () return true end,
            ["MQ Fire Temple Pot Hammer Loop 7"] = function () return true end,
            ["MQ Fire Temple Pot Hammer Loop 8"] = function () return true end,
            ["MQ Fire Temple Hammer Loop Iron Knuckle Big Fairy"] = function () return can_play_sun() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Hammer Loop Flare Dancer Room"] = {
        ["events"] = {
            ["FIRE_MQ_HAMMER_LOOP_FLARE_DANCER_CLEAR"] = function () return soul_enemy(SOUL_ENEMY_FLARE_DANCER) and (has_bombs() or has_bombchu() or can_use_mask_blast() or can_hammer() or can_hookshot()) and (has_weapon() or has_sticks() or can_hammer()) end,
        },
        ["exits"] = {
            ["Fire Temple Hammer Loop Iron Knuckle Room"] = function () return true end,
            ["Fire Temple Hammer Loop Goron Cell Map Chest Side"] = function () return event('FIRE_MQ_HAMMER_LOOP_FLARE_DANCER_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Fire Temple Hammer Chest"] = function () return event('FIRE_MQ_HAMMER_LOOP_FLARE_DANCER_CLEAR') and (is_adult() or can_hookshot() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Hammer Loop Goron Cell Map Chest Side"] = {
        ["events"] = {
            ["FIRE_MQ_MAP_CHEST_HAMMER_SWITCH"] = function () return can_hammer() end,
        },
        ["exits"] = {
            ["Fire Temple Hammer Loop Flare Dancer Room"] = function () return true end,
            ["Fire Temple Hammer Loop Goron Cell Like Like Side"] = function () return event('FIRE_MQ_MAP_CHEST_HAMMER_SWITCH') end,
        },
        ["locations"] = {
            ["MQ Fire Temple Map Chest"] = function () return event('FIRE_MQ_MAP_CHEST_HAMMER_SWITCH') end,
        },
    ...
    ["Fire Temple 1F Lava Bridge Room Main"] = {
        ["exits"] = {
            ["Fire Temple Upper Lobby"] = function () return true end,
            ["Fire Temple Lava Bridge Room North Side Upper Ledge"] = function () return has_tunic_goron() and (event('FIRE_MQ_LAVA_BRIDGE_ROOM_HOOKSHOT_PLATFORMS') and can_hookshot() or hookshot_anywhere() or climb_anywhere()) end,
            ["Fire Temple Lava Bridge Room North Side Lower Room"] = function () return has_tunic_goron() end,
            ["Fire Temple 1F Lava Bridge Room High Alcove"] = function () return true end,
            ["Fire Temple Lava Bridge Room South Side Ledge"] = function () return has_tunic_goron() and can_hookshot() or (has_tunic_goron_strict() and climb_anywhere()) end,
            ["Fire Temple Rising Block Room"] = function () return has_tunic_goron() and small_keys_fire_mq(2) end,
        },
        ["locations"] = {
    ...
    ["Fire Temple 1F Lava Bridge Room High Alcove"] = {
        ["events"] = {
            ["FIRE_MQ_LAVA_BRIDGE_ROOM_TORCHES"] = function () return has_fire() end,
        },
        ["exits"] = {
            ["Fire Temple 1F Lava Bridge Room Main"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Fire Temple Lava Bridge Room North Side Upper Ledge"] = {
        ["exits"] = {
            ["Fire Temple 1F Lava Bridge Room Main"] = function () return has_tunic_goron() end,
            ["Fire Temple Boss Key Chest Room"] = function () return has_tunic_goron() and event('FIRE_MQ_LAVA_BRIDGE_ROOM_TORCHES') end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Boss Key Chest Room"] = {
        ["exits"] = {
            ["Fire Temple Lava Bridge Room North Side Upper Ledge"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Fire Temple Boss Key Chest"] = function () return can_hookshot() or climb_anywhere() end,
            ["MQ Fire Temple Pot Boss Key Room 1"] = function () return can_hookshot() or can_boomerang() or climb_anywhere() end,
            ["MQ Fire Temple Pot Boss Key Room 2"] = function () return can_hookshot() or can_boomerang() or climb_anywhere() end,
            ["MQ Fire Temple Wonder Item Boss Key Room Hookshot"] = function () return can_hookshot() end,
            ["MQ Fire Temple Wonder Item Boss Key Room Bow"] = function () return can_use_bow() and (can_boomerang() or can_hookshot() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Lava Bridge Room North Side Lower Room"] = {
        ...
        ["locations"] = {
            ["MQ Fire Temple GS 1f Lava Room"] = function () return gs() and can_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Lava Bridge Room South Side Ledge"] = {
        ["events"] = {
            ["FIRE_MQ_LAVA_BRIDGE_ROOM_HOOKSHOT_PLATFORMS"] = function () return has_fire() end,
        },
        ["exits"] = {
            ["Fire Temple 1F Lava Bridge Room Main"] = function () return has_tunic_goron() end,
            ["Fire Temple 1F Lava Bridge Room South Side Room"] = function () return has_tunic_goron() and has_explosives() end,
        },
        ["locations"] = {
            ["MQ Fire Temple Pot Lava Room Right"] = function () return has_tunic_goron() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple 1F Lava Bridge Room South Side Room"] = {
        ["exits"] = {
            ["Fire Temple Lava Bridge Room South Side Ledge"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Fire Temple 1f Lava Room Goron Chest"] = function () return has_fire() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Rising Block Room"] = {
        ["exits"] = {
            ["Fire Temple 1F Lava Bridge Room Main"] = function () return has_tunic_goron() and small_keys_fire_mq(2) end,
            ["Fire Temple Room Before Maze"] = function () return has_tunic_goron_strict() end,
        },
        ["locations"] = {
            ["MQ Fire Temple Heart 1"] = function () return has_tunic_goron_strict() end,
            ["MQ Fire Temple Heart 2"] = function () return has_tunic_goron_strict() end,
            ["MQ Fire Temple Heart 3"] = function () return has_tunic_goron_strict() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Room Before Maze"] = {
        ["events"] = {
            ["FIRE_MQ_ROOM_BEFORE_MAZE_HOOKSHOT_TARGET"] = function () return has_fire() end,
        },
        ["exits"] = {
            ["Fire Temple Rising Block Room"] = function () return true end,
            ["Fire Temple Maze Room Lower"] = function () return event('FIRE_MQ_ROOM_BEFORE_MAZE_HOOKSHOT_TARGET') and can_hookshot() or climb_anywhere() or hookshot_anywhere() end,
            ["Fire Temple Shortcut Cage Under Maze"] = function () return event('FIRE_MQ_MAZE_SHORTCUT_SWITCH') end,
        },
        ["locations"] = {
            ["MQ Fire Temple Wonder Item Shortcut Room 1"] = function () return can_hammer() end,
            ["MQ Fire Temple Wonder Item Shortcut Room 2"] = function () return can_hammer() end,
            ["MQ Fire Temple Wonder Item Shortcut Room 3"] = function () return can_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Shortcut Cage Under Maze"] = {
        ["events"] = {
            ["FIRE_MQ_MAZE_SHORTCUT_SWITCH"] = function () return can_hammer() end,
        },
        ["exits"] = {
            ["Fire Temple Room Before Maze"] = function () return event('FIRE_MQ_MAZE_SHORTCUT_SWITCH') end,
            ["Fire Temple Maze Room Upper"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Fire Temple Compass Chest"] = function () return can_hammer() end,
            ["MQ Fire Temple Cell Below Maze Large Crate 1"] = function () return true end,
            ["MQ Fire Temple Cell Below Maze Large Crate 2"] = function () return true end,
            ["MQ Fire Temple Cell Below Maze Large Crate 3"] = function () return true end,
            ["MQ Fire Temple Cell Below Maze Large Crate 4"] = function () return true end,
            ["MQ Fire Temple Cell Below Maze Large Crate 5"] = function () return true end,
            ["MQ Fire Temple Cell Below Maze Large Crate 6"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Maze Room Lower"] = {
        ["events"] = {
            ["FIRE_MQ_MAZE_ROOM_LOWER_CAGE_SWITCH"] = function () return can_jump_slash() or (has_weapon() and has_magic()) end,
            ["FIRE_MQ_MAZE_ROOM_LOWER_RUSTY_SWITCH"] = function () return trick('OOT_HAMMER_WALLS') and can_hammer() end,
        },
        ["exits"] = {
            ["Fire Temple Room Before Maze"] = function () return true end,
            ["Fire Temple Maze Room Upper"] = function () return event('FIRE_MQ_MAZE_ROOM_LOWER_RUSTY_SWITCH') and can_hookshot() or climb_anywhere() or hookshot_anywhere() end,
            ["Fire Temple Maze Room Upper Switch Cage Alcove"] = function () return can_hookshot() or (climb_anywhere() and (has_ranged_weapon() or has_explosives() or can_use_mask_blast())) end,
            ["Fire Temple Maze Room Rusty Switch Behind Bombable Wall"] = function () return has_explosives() end,
            ["Fire Temple Maze Room Blue Switch Behind Bombable Wall"] = function () return has_explosives() end,
            ["Fire Temple Tower Area Above Maze Room"] = function () return can_longshot() end,
        },
        ["locations"] = {
            ["MQ Fire Temple Maze Lower Chest"] = function () return event('FIRE_MQ_MAZE_ROOM_LOWER_CAGE_SWITCH') end,
            ["MQ Fire Temple Wonder Item East Maze"] = function () return can_hookshot() end,
            ["MQ Fire Temple Maze Room Lower Cage Large Crate 1"] = function () return event('FIRE_MQ_MAZE_ROOM_LOWER_CAGE_SWITCH') end,
            ["MQ Fire Temple Maze Room Lower Cage Large Crate 2"] = function () return event('FIRE_MQ_MAZE_ROOM_LOWER_CAGE_SWITCH') end,
            ["MQ Fire Temple Maze Room Lower Cage Large Crate 3"] = function () return event('FIRE_MQ_MAZE_ROOM_LOWER_CAGE_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Maze Room Rusty Switch Behind Bombable Wall"] = {
        ["events"] = {
            ["FIRE_MQ_MAZE_ROOM_LOWER_RUSTY_SWITCH"] = function () return can_hammer() end,
        },
        ["exits"] = {
            ["Fire Temple Maze Room Lower"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Maze Room Blue Switch Behind Bombable Wall"] = {
        ["events"] = {
            ["FIRE_MQ_MAZE_ROOM_BLUE_SWITCH"] = function () return can_play_elegy() end,
        },
        ["exits"] = {
            ["Fire Temple Maze Room Lower"] = function () return true end,
            ["Fire Temple Maze Room Side Room"] = function () return event('FIRE_MQ_MAZE_ROOM_BLUE_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Maze Room Upper Switch Cage Alcove"] = {
        ["events"] = {
            ["FIRE_MQ_MAZE_ROOM_BLUE_SWITCH"] = function () return has_explosives() end,
        },
        ["exits"] = {
            ["Fire Temple Maze Room Lower"] = function () return true end,
            ["Fire Temple Maze Room Upper"] = function () return true end,
        ...
        ["locations"] = {
            ...
            ["MQ Fire Temple Maze Room Upper Cage Large Crate 1"] = function () return true end,
            ["MQ Fire Temple Maze Room Upper Cage Large Crate 2"] = function () return true end,
            ["MQ Fire Temple Maze Room Upper Cage Large Crate 3"] = function () return true end,
            ["MQ Fire Temple Maze Room Upper Cage Small Crate 1"] = function () return true end,
            ["MQ Fire Temple Maze Room Upper Cage Small Crate 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Maze Room Side Room"] = {
        ["exits"] = {
            ["Fire Temple Maze Room Blue Switch Behind Bombable Wall"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Fire Temple Maze Room Upper"] = {
        ["exits"] = {
            ["Fire Temple Maze Room Lower"] = function () return true end,
            ["Fire Temple Maze Room Upper Switch Cage Alcove"] = function () return has_ranged_weapon() or has_explosives() or can_use_mask_blast() or can_jump_slash() or (has_spin_upgrade() and has_magic() and (can_use_sword_master() or can_use_sword_razor() or can_use_sword_biggoron())) end,
            ["Fire Temple Shortcut Cage Under Maze"] = function () return has_explosives() end,
            ["Fire Temple 3F Lava Floor Room After Maze"] = function () return small_keys_fire_mq(3) end,
            ["Fire Temple Tower Area Above Maze Room"] = function () return can_longshot() or (can_play_time() and can_hookshot()) or hookshot_anywhere() or climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower Area Above Maze Room"] = {
        ["exits"] = {
            ["Fire Temple Maze Room Upper"] = function () return true end,
            ["Fire Temple Maze Room Lower"] = function () return true end,
            ["Fire Temple Tower Above Maze First Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower Above Maze First Room"] = {
        ["exits"] = {
            ["Fire Temple Tower Area Above Maze Room"] = function () return true end,
            ["Fire Temple Tower Above Maze Burning Block Room"] = function () return can_hookshot() end,
        },
        ["locations"] = {
            ["MQ Fire Temple Wonder Item East Climb First 1"] = function () return can_hookshot() end,
            ["MQ Fire Temple Wonder Item East Climb First 2"] = function () return can_hookshot() end,
            ["MQ Fire Temple Wonder Item East Climb Second 1"] = function () return can_hookshot() end,
            ["MQ Fire Temple Wonder Item East Climb Second 2"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower Above Maze Burning Block Room"] = {
        ["events"] = {
            ["FIRE_MQ_EAST_TOWER_TOP_HOOKSHOT_TARGETS"] = function () return can_hammer() end,
        },
        ["exits"] = {
            ["Fire Temple Tower Above Maze First Room"] = function () return true end,
            ["Fire Temple Inside Maze Narrow Paths Room"] = function () return true end,
            ["Fire Temple 1F Lava Bridge Room Main"] = function () return has_tunic_goron() end,
        },
        ["locations"] = {
            ["MQ Fire Temple GS Burning Block"] = function () return gs() and (event('FIRE_MQ_EAST_TOWER_TOP_HOOKSHOT_TARGETS') and can_hookshot() or hookshot_anywhere() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple 3F Lava Floor Room After Maze"] = {
        ["events"] = {
            ["FIRE_MQ_3F_LAVA_ROOM_BLUE_SWITCH"] = function () return has_tunic_goron_strict() and (can_hookshot() or climb_anywhere()) end,
            ["FIRE_MQ_3F_LAVA_ROOM_TORCHES"] = function () return event('FIRE_MQ_3F_LAVA_ROOM_BLUE_SWITCH') and can_use_bow() or has_fire_arrows() end,
        },
        ["exits"] = {
            ["Fire Temple Maze Room Upper"] = function () return has_tunic_goron() and small_keys_fire_mq(3) end,
            ["Fire Temple 3F Lava Room Cage Area"] = function () return has_tunic_goron() and (is_adult() or hookshot_anywhere() or climb_anywhere()) or (has_tunic_goron_strict() and can_hookshot()) end,
            ["Fire Temple Inside Maze Narrow Paths Room"] = function () return has_tunic_goron() end,
            ["Fire Temple 3F Lava Room Ledge Before Fire Walls Hallway"] = function () return has_tunic_goron() and (is_adult() or can_hookshot() or climb_anywhere()) end,
        },
        ["locations"] = {
            ["MQ Fire Temple Pot Grids Above Lava 1"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Pot Grids Above Lava 2"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple Wonder Item Torch Room"] = function () return has_tunic_goron() and can_hookshot() end,
            ["MQ Fire Temple 3F Lava Room Lower Large Crate 1"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple 3F Lava Room Lower Large Crate 2"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple 3F Lava Room Lower Large Crate 3"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple 3F Lava Room Lower Small Crate 1"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple 3F Lava Room Lower Small Crate 2"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple 3F Lava Room Near Cage Large Crate"] = function () return has_tunic_goron() and (can_hookshot() or climb_anywhere()) end,
            ["MQ Fire Temple 3F Lava Room Near Cage Small Crate 1"] = function () return has_tunic_goron() and (can_hookshot() or climb_anywhere()) end,
            ["MQ Fire Temple 3F Lava Room Near Cage Small Crate 2"] = function () return has_tunic_goron() and (can_hookshot() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple 3F Lava Room Ledge Before Fire Walls Hallway"] = {
        ["exits"] = {
            ["Fire Temple 3F Lava Floor Room After Maze"] = function () return has_tunic_goron() end,
            ["Fire Temple 3F Hallway Before Fire Walls"] = function () return has_tunic_goron() and event('FIRE_MQ_3F_LAVA_ROOM_TORCHES') end,
        },
        ["locations"] = {
            ["MQ Fire Temple 3F Lava Room High Ledge Large Crate"] = function () return has_tunic_goron() end,
            ["MQ Fire Temple 3F Lava Room High Ledge Small Crate"] = function () return has_tunic_goron() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple 3F Lava Room Cage Area"] = {
        ["exits"] = {
            ["Fire Temple 3F Lava Floor Room After Maze"] = function () return can_hookshot() or climb_anywhere() end,
            ["Fire Temple Inside Maze Narrow Paths Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ...
    ["Fire Temple 3F Hallway Before Fire Walls"] = {
        ["exits"] = {
            ["Fire Temple 3F Lava Room Ledge Before Fire Walls Hallway"] = function () return true end,
            ["Fire Temple Fire Walls Room Pillar Side"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Fire Walls Room Pillar Side"] = {
        ["events"] = {
            ["FIRE_MQ_PRE_BOSS_PILLAR"] = function () return can_hammer() and (is_adult() or can_play_time() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
        },
        ["exits"] = {
            ["Fire Temple 3F Hallway Before Fire Walls"] = function () return true end,
            ["Fire Temple Pre-Boss Room Entrance Side"] = function () return has_tunic_goron() and event('FIRE_MQ_PRE_BOSS_PILLAR') and (is_adult() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["Fire Temple Pre-Boss"] = function () return has_tunic_goron() and event('FIRE_MQ_PRE_BOSS_PILLAR') and (is_adult() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["Fire Temple Pre-Boss Room Cell Corner"] = function () return has_tunic_goron() and event('FIRE_MQ_PRE_BOSS_PILLAR') and (can_hookshot() or climb_anywhere()) end,
            ["Fire Temple Pre-Boss Room Tall Ledges Corner"] = function () return has_tunic_goron() and event('FIRE_MQ_PRE_BOSS_PILLAR') and (can_longshot() or hookshot_anywhere() or climb_anywhere()) end,
            ["Fire Temple Fire Walls Room Outside Side Room"] = function () return has_hover_boots() or can_play_time() or hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Middle Hallway Lower"] = function () return true end,
            ["Fire Temple Fire Walls Middle Hallway Upper"] = function () return is_adult() or can_play_time() or has_hover_boots() or hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Room Large Fire Wall Side"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Room High Area After Staircase"] = function () return event('FIRE_MQ_HIGH_LEDGE_AFTER_STAIRCASE_PILLAR') and longshot_anywhere() end,
        },
        ["locations"] = {
    ...
    ["Fire Temple Fire Walls Room Outside Side Room"] = {
        ["exits"] = {
            ["Fire Temple Fire Walls Room Side Room"] = function () return true end,
            ["Fire Temple Fire Walls Room Pillar Side"] = function () return is_adult() and can_play_time() or hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Room Large Fire Wall Side"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Room Area After Large Fire Wall"] = function () return hookshot_anywhere() or climb_anywhere() end,
        },
        ["locations"] = {
            ["MQ Fire Temple Pot Fire Maze Room Right 1"] = function () return true end,
            ["MQ Fire Temple Pot Fire Maze Room Right 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Fire Walls Room Side Room"] = {
        ["exits"] = {
            ["Fire Temple Fire Walls Room Outside Side Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Fire Temple GS Fire Walls Side Room"] = function () return gs() and can_damage_skull() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Fire Walls Middle Hallway Lower"] = {
        ["exits"] = {
            ["Fire Temple Fire Walls Room Pillar Side"] = function () return true end,
            ["Fire Temple Fire Walls Room Large Fire Wall Side"] = function () return event('FIRE_MQ_FIRE_WALLS_MIDDLE_ROOM_RUSTY_SWITCH') end,
            ["Fire Temple Fire Walls Middle Hallway Upper"] = function () return hookshot_anywhere() or climb_anywhere() end,
        },
        ["locations"] = {
    ...
    ["Fire Temple Fire Walls Middle Hallway Upper"] = {
        ["events"] = {
            ["FIRE_MQ_FIRE_WALLS_MIDDLE_ROOM_RUSTY_SWITCH"] = function () return can_hammer() and (can_hookshot() or climb_anywhere()) end,
        },
        ["exits"] = {
            ["Fire Temple Fire Walls Middle Hallway Lower"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Room Pillar Side"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Fire Walls Room Large Fire Wall Side"] = {
        ["exits"] = {
            ["Fire Temple Fire Walls Room Pillar Side"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Room Outside Side Room"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Fire Walls Middle Hallway Lower"] = function () return true end,
            ["Fire Temple Fire Walls Room Area After Large Fire Wall"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Fire Temple Pot Fire Maze Room Right 1"] = function () return can_boomerang() end,
            ["MQ Fire Temple Pot Fire Maze Room Right 2"] = function () return can_boomerang() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Fire Walls Room Area After Large Fire Wall"] = {
        ["exits"] = {
            ["Fire Temple Fire Walls Room Large Fire Wall Side"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Fire Temple Flare Dancer Room After Fire Walls"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Fire Temple Pot Fire Maze Room Back Right 1"] = function () return true end,
            ["MQ Fire Temple Pot Fire Maze Room Back Right 2"] = function () return true end,
            ["MQ Fire Temple Wonder Item Fire Maze"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Flare Dancer Room After Fire Walls"] = {
        ["events"] = {
            ["FIRE_MQ_FLARE_DANCER_AFTER_FIRE_WALLS"] = function () return soul_enemy(SOUL_ENEMY_FLARE_DANCER) and (has_bombs() or has_bombchu() or can_use_mask_blast() or can_hammer() or can_hookshot()) and (has_weapon() or has_sticks() or can_hammer()) end,
        },
        ["exits"] = {
            ["Fire Temple Fire Walls Room Area After Large Fire Wall"] = function () return event('FIRE_MQ_FLARE_DANCER_AFTER_FIRE_WALLS') end,
            ["Fire Temple Tower After Flare Dancer First Room"] = function () return event('FIRE_MQ_FLARE_DANCER_AFTER_FIRE_WALLS') or (longshot_anywhere() and climb_anywhere()) end,
        },
        ["locations"] = {
            ["MQ Fire Temple Flare Dancer Key"] = function () return event('FIRE_MQ_FLARE_DANCER_AFTER_FIRE_WALLS') end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower After Flare Dancer First Room"] = {
        ["exits"] = {
            ["Fire Temple Flare Dancer Room After Fire Walls"] = function () return true end,
            ["Fire Temple Tower After Flare Dancer Pit Room"] = function () return small_keys_fire_mq(4) end,
        },
        ["locations"] = {
            ["MQ Fire Temple Wonder Item After Flare Dancer"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower After Flare Dancer Pit Room"] = {
        ["events"] = {
            ["FIRE_MQ_TOWER_TOP_HAMMER_SWITCH_BEFORE_STAIRCASE"] = function () return can_hammer() end,
            ["FIRE_MQ_TOWER_AFTER_FLARE_DANCER_HOOKSHOT_TARGETS"] = function () return has_weapon() or has_bombs() or can_use_mask_blast() or has_bombchu() or can_use_bow() or can_use_slingshot() or can_hookshot() or can_use_sticks() or can_hammer() or can_boomerang() end,
        },
        ["exits"] = {
            ["Fire Temple Tower After Flare Dancer First Room"] = function () return small_keys_fire_mq(4) end,
            ["Fire Temple Tower After Flare Dancer Area Before Staircase"] = function () return event('FIRE_MQ_TOWER_TOP_HAMMER_SWITCH_BEFORE_STAIRCASE') end,
            ["Fire Temple Fire Walls Middle Hallway Lower"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Fire Temple Topmost Chest"] = function () return is_adult() or (scarecrow_hookshot() or climb_anywhere() or hookshot_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower After Flare Dancer Area Before Staircase"] = {
        ["events"] = {
            ["FIRE_MQ_TOWER_AFTER_FLARE_DANCER_CRYSTAL_SWITCH_FROM_BELOW"] = function () return event('FIRE_MQ_TOWER_TOP_HAMMER_SWITCH_BEFORE_STAIRCASE') and (has_ranged_weapon() or has_bombchu()) end,
        },
        ["exits"] = {
            ["Fire Temple Tower After Flare Dancer Pit Room"] = function () return event('FIRE_MQ_TOWER_TOP_HAMMER_SWITCH_BEFORE_STAIRCASE') and (event('FIRE_MQ_TOWER_AFTER_FLARE_DANCER_CRYSTAL_SWITCH_FROM_BELOW') and can_hookshot() or hookshot_anywhere() or climb_anywhere()) end,
            ["Fire Temple Tower After Flare Dancer Staircase Upper"] = function () return small_keys_fire_mq(5) end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower After Flare Dancer Staircase Upper"] = {
        ["events"] = {
            ["FIRE_MQ_STAIRCASE_LOWERED"] = function () return can_hammer() end,
        },
        ["exits"] = {
            ["Fire Temple Tower After Flare Dancer Area Before Staircase"] = function () return small_keys_fire_mq(5) end,
            ["Fire Temple Tower After Flare Dancer Staircase Lower Alcove"] = function () return event('FIRE_MQ_STAIRCASE_LOWERED') end,
        },
        ["locations"] = {
            ["MQ Fire Temple Wonder Item Staircase"] = function () return event('FIRE_MQ_STAIRCASE_LOWERED') and can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Tower After Flare Dancer Staircase Lower Alcove"] = {
        ["exits"] = {
            ["Fire Temple Tower After Flare Dancer Staircase Upper"] = function () return event('FIRE_MQ_STAIRCASE_LOWERED') end,
            ["Fire Temple Fire Walls Room High Area After Staircase"] = function () return event('FIRE_MQ_STAIRCASE_LOWERED') and can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Fire Temple Fire Walls Room High Area After Staircase"] = {
        ["events"] = {
            ["FIRE_MQ_HIGH_LEDGE_AFTER_STAIRCASE_PILLAR"] = function () return can_hammer() end,
        },
        ["exits"] = {
            ["Fire Temple Fire Walls Room Pillar Side"] = function () return event('FIRE_MQ_HIGH_LEDGE_AFTER_STAIRCASE_PILLAR') end,
        },
        ["locations"] = {
            ["MQ Fire Temple GS Topmost"] = function () return gs() and (can_collect_distance() or ((has_explosives() or can_use_mask_blast() or has_weapon() or has_ranged_weapon() or has_sticks()) and climb_anywhere())) end,
        },
        ["age_change"] = true,
    },
    ["Forest Temple"] = {
    ...
    ["Forest Temple Bow Region"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Forest Temple Spinning Red Pool Room Small Crate 1"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and soul_wolfos() and has_weapon() and small_keys_forest(5) end,
            ["MQ Forest Temple Spinning Red Pool Room Small Crate 2"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and soul_wolfos() and has_weapon() and small_keys_forest(5) end,
            ["MQ Forest Temple Spinning Red Pool Room Small Crate 3"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and soul_wolfos() and has_weapon() and small_keys_forest(5) end,
        },
        ["age_change"] = true,
    },
    ...
    ["Ganon Castle"] = {
        ["events"] = {
            ["MQ_GANON_OPEN_MAIN"] = function () return soul_iron_knuckle() and soul_armos() and soul_bubble() and (can_use_sword_master() or can_use_sword_goron() or (can_use_sticks() and (has_ranged_weapon_child() or can_use_bow())) or has_explosives_or_hammer()) end,
        },
        ["exits"] = {
            ["Ganon Castle Exterior After Bridge"] = function () return true end,
            ["Ganon Castle Main Room"] = function () return event('MQ_GANON_OPEN_MAIN') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Spirit Wallmaster"] = {
    ...
    ["Ganon Castle Main Room"] = {
        ["exits"] = {
            ["Ganon Castle"] = function () return true end,
            ["Ganon Castle Light Trial Entrance"] = function () return can_lift_gold() end,
            ["Ganon Castle Forest Trial Entrance"] = function () return true end,
            ["Ganon Castle Fire Trial Entrance Ledge"] = function () return true end,
            ["Ganon Castle Water Trial Entrance"] = function () return true end,
            ["Ganon Castle Spirit Trial Entrance"] = function () return true end,
            ["Ganon Castle Shadow Trial Entrance Side"] = function () return true end,
    ...
    ["Ganon Castle Light Trial Entrance"] = {
        ["events"] = {
            ["GANON_MQ_LIGHT_ENEMIES"] = function () return soul_lizalfos_dinalfos() and soul_enemy(SOUL_ENEMY_TORCH_SLUG) and (has_weapon() or (can_hammer() and can_use_sticks())) end,
        },
        ["exits"] = {
            ["Ganon Castle Main Room"] = function () return true end,
            ["Ganon Castle Light Trial Lullaby Hallway"] = function () return event('GANON_MQ_LIGHT_ENEMIES') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Light Trial Lullaby Hallway"] = {
        ["exits"] = {
            ["Ganon Castle Light Trial Entrance"] = function () return true end,
            ["Ganon Castle Light Trial Round Boulder Room Before Flame Walls"] = function () return small_keys_ganon(2) end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Light Trial Chest"] = function () return can_play_zelda() end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Light Trial Round Boulder Room Before Flame Walls"] = {
        ["exits"] = {
            ["Ganon Castle Light Trial Lullaby Hallway"] = function () return small_keys_ganon(2) end,
            ["Ganon Castle Light Trial Round Boulder Room After Flame Walls"] = function () return can_hookshot() or climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Light Trial Round Boulder Room After Flame Walls"] = {
        ["exits"] = {
            ["Ganon Castle Light Trial Round Boulder Room Before Flame Walls"] = function () return can_hookshot() or climb_anywhere() end,
            ["Ganon Castle Light Trial Ending Room"] = function () return small_keys_ganon(3) end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Heart Light 1"] = function () return true end,
            ["MQ Ganon Castle Heart Light 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Light Trial Ending Room"] = {
        ["events"] = {
            ["GANON_TRIAL_LIGHT"] = function () return can_use_light_arrows() and has_lens() end,
        },
        ["exits"] = {
            ["Ganon Castle Light Trial Round Boulder Room After Flame Walls"] = function () return small_keys_ganon(3) end,
        },
        ["locations"] = {
            ["MQ Ganon Pot Light End 1"] = function () return true end,
            ["MQ Ganon Pot Light End 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Forest Trial Entrance"] = {
        ["events"] = {
            ["GANON_MQ_FOREST_ENEMIES"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() end,
        },
        ["exits"] = {
            ["Ganon Castle Main Room"] = function () return true end,
            ["Ganon Castle Forest Trial Wind Room Starting Side"] = function () return event('GANON_MQ_FOREST_ENEMIES') end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Forest Trial Key"] = function () return can_hookshot() or can_boomerang() or climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Forest Trial Wind Room Starting Side"] = {
        ["events"] = {
            ["GANON_CASTLE_MQ_FOREST_WIND_FROZEN_EYE"] = function () return has_fire_arrows() end,
            ["GANON_CASTLE_MQ_FOREST_WIND_OPEN_EYE"] = function () return can_hit_triggers_distance() end,
        },
        ["exits"] = {
            ["Ganon Castle Forest Trial Entrance"] = function () return true end,
            ["Ganon Castle Forest Trial Wind Room Ending Side"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Forest Trial First Chest"] = function () return event('GANON_CASTLE_MQ_FOREST_WIND_OPEN_EYE') end,
            ["MQ Ganon Castle Forest Trial Second Chest"] = function () return event('GANON_CASTLE_MQ_FOREST_WIND_FROZEN_EYE') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Forest Trial Wind Room Ending Side"] = {
        ["events"] = {
            ["GANON_CASTLE_MQ_FOREST_WIND_FROZEN_EYE"] = function () return has_fire() end,
            ["GANON_CASTLE_MQ_FOREST_WIND_OPEN_EYE"] = function () return can_hit_triggers_distance() end,
            ["GANON_MQ_FOREST_SWITCH"] = function () return can_play_time() or can_play_elegy() end,
        },
        ["exits"] = {
            ["Ganon Castle Forest Trial Wind Room Starting Side"] = function () return hookshot_anywhere() or (has_hover_boots() and climb_anywhere()) or (can_longshot() and event('GANON_CASTLE_MQ_FOREST_WIND_OPEN_EYE')) end,
            ["Ganon Castle Forest Trial Ending Room"] = function () return event('GANON_MQ_FOREST_SWITCH') end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Forest Trial Second Chest"] = function () return event('GANON_CASTLE_MQ_FOREST_WIND_FROZEN_EYE') end,
        },
    ...
    ["Ganon Castle Forest Trial Ending Room"] = {
        ...
        ["locations"] = {
            ["MQ Ganon Pot Forest End 1"] = function () return true end,
            ["MQ Ganon Pot Forest End 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Fire Trial Entrance Ledge"] = {
        ["events"] = {
            ["GANON_MQ_FIRE_MONOLITH"] = function () return has_tunic_goron() and can_lift_gold() end,
        },
        ["exits"] = {
            ["Ganon Castle Main Room"] = function () return true end,
    ...
    ["Ganon Castle Fire Trial Ending Ledge"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Ganon Castle SR Fire Center-Left"] = function () return longshot_anywhere() and has_tunic_goron_strict() end,
            ["MQ Ganon Castle SR Fire Back-Left"] = function () return longshot_anywhere() and has_tunic_goron_strict() end,
            ["MQ Ganon Castle SR Fire High Above Lava"] = function () return longshot_anywhere() and has_tunic_goron_strict() end,
            ["MQ Ganon Castle SR Fire Front-Left"] = function () return longshot_anywhere() and has_tunic_goron_strict() end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Fire Trial Ending Room"] = {
        ...
        ["exits"] = {
            ["Ganon Castle Fire Trial Ending Ledge"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ganon Pot Fire End 1"] = function () return true end,
            ["MQ Ganon Pot Fire End 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Water Trial Entrance"] = {
        ["events"] = {
            ["MQ_GANON_WATER_BF_SWITCH"] = function () return can_use_bow() or can_use_slingshot() or has_weapon() or can_use_sticks() or has_explosives_or_hammer() end,
            ["BLUE_FIRE"] = function () return has_bottle() and event('MQ_GANON_WATER_BF_SWITCH') end,
        },
        ["exits"] = {
            ["Ganon Castle Main Room"] = function () return true end,
            ["Ganon Castle Water Trial Entrance Room Behind Red Ice"] = function () return has_blue_fire() or hookshot_anywhere() end,
        },
        ["locations"] = {
            ...
            ["MQ Ganon Castle Icicle Water 1"] = function () return break_icicle() end,
            ["MQ Ganon Castle Icicle Water 2"] = function () return break_icicle() end,
            ["MQ Ganon Castle Icicle Water 3"] = function () return break_icicle() end,
            ["MQ Ganon Castle Icicle Water 4"] = function () return break_icicle() end,
            ["MQ Ganon Castle Icicle Water 5"] = function () return break_icicle() end,
            ["MQ Ganon Castle Icicle Water 6"] = function () return break_icicle() end,
            ["MQ Ganon Castle Icicle Water 7"] = function () return break_icicle() end,
            ["MQ Ganon Castle Icicle Water 8"] = function () return break_icicle() end,
            ["MQ Ganon Castle Red Ice Water 1"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Water 2"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Water 3"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Water 4"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Water 5"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Water 6"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Water 7"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Water Trial Entrance Room Behind Red Ice"] = {
        ["exits"] = {
            ["Ganon Castle Water Trial Entrance"] = function () return has_blue_fire() or climb_anywhere() end,
            ["Ganon Castle Water Trial Silver Rupee Room"] = function () return small_keys_ganon(3) end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Red Ice Water 1"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Water 2"] = function () return has_blue_fire() end,
    ...
    ["Ganon Castle Water Trial Silver Rupee Room"] = {
        ["exits"] = {
            ["Ganon Castle Water Trial Entrance Room Behind Red Ice"] = function () return small_keys_ganon(3) end,
            ["Ganon Castle Water Trial Silver Rupee Room Behind Red Ice"] = function () return is_adult() and has_blue_fire() or hookshot_anywhere() or climb_anywhere() end,
        },
        ["locations"] = {
            ["MQ Ganon Castle SR Water Shallow Hole"] = function () return true end,
            ["MQ Ganon Castle SR Water Above Ground"] = function () return is_adult() or climb_anywhere() or hookshot_anywhere() end,
            ["MQ Ganon Castle SR Water Alcove"] = function () return has_blue_fire() and (is_adult() or climb_anywhere() or hookshot_anywhere()) end,
            ...
            ["MQ Ganon Castle Red Ice Block Silver Rupee Room 2"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Block Silver Rupee Room 3"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Block Silver Rupee Room 4"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Block Silver Rupee Room 5"] = function () return has_blue_fire() end,
            ["MQ Ganon Castle Red Ice Block Silver Rupee Room 6"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Ganon Castle Water Trial Ending Room"] = {
        ...
        ["exits"] = {
            ["Ganon Castle Water Trial Silver Rupee Room Behind Red Ice"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ganon Pot Water End 1"] = function () return true end,
            ["MQ Ganon Pot Water End 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Spirit Trial Entrance"] = {
        ["events"] = {
            ["GANON_MQ_SPIRIT_HAMMER_SWITCH"] = function () return (can_use_bow() or can_use_slingshot()) and soul_iron_knuckle() and can_hammer() or (can_hammer() and trick('OOT_HAMMER_WALLS')) end,
        },
        ["exits"] = {
            ["Ganon Castle Main Room"] = function () return true end,
            ["Ganon Castle Spirit Trial Sun Room Entrance Side"] = function () return event('GANON_MQ_SPIRIT_HAMMER_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Spirit Trial Sun Room Entrance Side"] = {
        ["events"] = {
            ["GANON_MQ_SPIRIT_CRYSTAL"] = function () return has_bombchu() end,
        },
        ["exits"] = {
            ["Ganon Castle Spirit Trial Entrance"] = function () return true end,
            ["Ganon Castle Spirit Trial Sun Room Sun Side"] = function () return event('GANON_MQ_SPIRIT_CRYSTAL') or climb_anywhere() or hookshot_anywhere() end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Spirit Trial First Chest"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Spirit Trial Sun Room Sun Side"] = {
        ["events"] = {
            ["GANON_MQ_SPIRIT_CRYSTAL"] = function () return has_bombchu() or can_use_bow() or can_hookshot() or can_use_slingshot() or can_boomerang() or has_bombs() end,
            ["GANON_MQ_SPIRIT_ZOMBIES"] = function () return can_play_sun() or has_weapon() or can_use_din() or can_hammer() end,
            ["GANON_MQ_SPIRIT_ENDING_SUNS"] = function () return event('GANON_MQ_SPIRIT_ZOMBIES') and (has_sunlight_arrows() or (has_fire_arrows() and has_mirror_shield())) end,
        },
        ["exits"] = {
            ["Ganon Castle Spirit Trial Sun Room Entrance Side"] = function () return event('GANON_MQ_SPIRIT_CRYSTAL') or climb_anywhere() or hookshot_anywhere() end,
            ["Ganon Castle Spirit Trial Ending Room"] = function () return event('GANON_MQ_SPIRIT_ENDING_SUNS') end,
            ["Ganon Castle Spirit Wallmaster"] = function () return soul_wallmaster() and event('GANON_MQ_SPIRIT_ENDING_SUNS') end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Spirit Trial Second Chest"] = function () return has_lens() end,
            ["MQ Ganon Castle Spirit Trial Back Right Sun Chest"] = function () return event('GANON_MQ_SPIRIT_ENDING_SUNS') end,
            ["MQ Ganon Castle Spirit Trial Back Left Sun Chest"] = function () return event('GANON_MQ_SPIRIT_ENDING_SUNS') end,
            ["MQ Ganon Castle Spirit Trial Front Left Sun Chest"] = function () return event('GANON_MQ_SPIRIT_ENDING_SUNS') end,
            ["MQ Ganon Castle Spirit Trial Gold Gauntlets Chest"] = function () return event('GANON_MQ_SPIRIT_ENDING_SUNS') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Spirit Trial Ending Room"] = {
        ...
        ["exits"] = {
            ["Ganon Castle Spirit Trial Sun Room Sun Side"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ganon Pot Spirit End 1"] = function () return true end,
            ["MQ Ganon Pot Spirit End 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Shadow Trial Entrance Side"] = {
        ["events"] = {
            ["GANON_MQ_SHADOW_BOMBFLOWER_FROM_ENTRANCE"] = function () return can_use_bow() or ((longshot_anywhere() or (has_lens() and (is_adult() and has_hover_boots() or hookshot_anywhere()))) and (can_use_din() or has_bombflowers())) end,
            ["GANON_MQ_SHADOW_PROGRESSION_FROM_ENTRANCE"] = function () return longshot_anywhere() or (has_lens() and hookshot_anywhere()) or (has_lens() and (can_hookshot() and event('GANON_MQ_SHADOW_BOMBFLOWER_FROM_ENTRANCE') or (is_adult() and has_hover_boots()))) end,
        },
        ["exits"] = {
            ["Ganon Castle Shadow Trial Middle Beamos Area 1"] = function () return event('GANON_MQ_SHADOW_PROGRESSION_FROM_ENTRANCE') end,
            ["Ganon Castle Main Room"] = function () return true end,
            ["Ganon Castle Shadow Trial Ending Side"] = function () return climb_anywhere() end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Shadow Trial Bomb Flower Chest"] = function () return event('GANON_MQ_SHADOW_BOMBFLOWER_FROM_ENTRANCE') and (can_hookshot() or (is_adult() and has_hover_boots())) end,
            ["MQ Ganon Castle SR Shadow Front-Right"] = function () return event('GANON_MQ_SHADOW_PROGRESSION_FROM_ENTRANCE') end,
            ["MQ Ganon Castle SR Shadow Front-Center"] = function () return event('GANON_MQ_SHADOW_PROGRESSION_FROM_ENTRANCE') end,
            ["MQ Ganon Castle Wonder Item"] = function () return event('GANON_MQ_SHADOW_BOMBFLOWER_FROM_ENTRANCE') and event('GANON_MQ_SHADOW_PROGRESSION_FROM_ENTRANCE') end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Shadow Trial Middle Beamos Area 1"] = {
        ["events"] = {
            ["GANON_MQ_SHADOW_TORCH_ICE_BLOCK"] = function () return has_fire() end,
            ["GANON_MQ_SHADOW_PROGRESSION_FROM_BEAMOS_1"] = function () return has_lens() and (can_longshot() or hookshot_anywhere() or (has_hover_boots() and (is_adult() or climb_anywhere()))) or longshot_anywhere() end,
            ["GANON_MQ_SHADOW_BOMBFLOWER_FROM_BEAMOS_1"] = function () return can_use_bow() or ((can_use_din() or has_bombflowers()) and (has_lens() or longshot_anywhere())) end,
        },
        ["exits"] = {
            ["Ganon Castle Shadow Trial Entrance Side"] = function () return event('GANON_MQ_SHADOW_PROGRESSION_FROM_BEAMOS_1') end,
            ["Ganon Castle Shadow Trial Middle Beamos Area 2"] = function () return event('GANON_MQ_SHADOW_TORCH_ICE_BLOCK') or has_hover_boots() or hookshot_anywhere() or (event('GANON_MQ_SHADOW_EYE_CHEST') and can_hookshot()) end,
        },
        ["locations"] = {
            ["MQ Ganon Castle SR Shadow Middle"] = function () return true end,
            ["MQ Ganon Castle Shadow Trial Bomb Flower Chest"] = function () return event('GANON_MQ_SHADOW_BOMBFLOWER_FROM_BEAMOS_1') and (has_lens() and (is_adult() or has_hover_boots() or hookshot_anywhere())) end,
            ["MQ Ganon Castle Wonder Item"] = function () return event('GANON_MQ_SHADOW_BOMBFLOWER_FROM_BEAMOS_1') and (has_lens() or longshot_anywhere()) end,
            ["MQ Ganon Castle SR Shadow Front-Right"] = function () return has_lens() or longshot_anywhere() end,
            ["MQ Ganon Castle SR Shadow Front-Center"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Shadow Trial Middle Beamos Area 2"] = {
        ["events"] = {
            ["GANON_MQ_SHADOW_TORCH_ICE_BLOCK"] = function () return has_fire_arrows() end,
            ["GANON_MQ_SHADOW_EYE_CHEST"] = function () return can_use_bow() or can_use_slingshot() end,
        },
        ["exits"] = {
            ["Ganon Castle Shadow Trial Middle Beamos Area 1"] = function () return event('GANON_MQ_SHADOW_TORCH_ICE_BLOCK') or has_hover_boots() or hookshot_anywhere() end,
            ["Ganon Castle Shadow Trial Ending Side"] = function () return has_lens() or hookshot_anywhere() end,
        },
        ["locations"] = {
            ["MQ Ganon Castle Shadow Trial Switch Chest"] = function () return event('GANON_MQ_SHADOW_EYE_CHEST') end,
            ["MQ Ganon Castle SR Shadow Back-Left"] = function () return has_lens() or hookshot_anywhere() end,
            ["MQ Ganon Castle SR Shadow Back-Center"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ...
    ["Ganon Castle Shadow Trial Ending Room"] = {
        ...
        ["exits"] = {
            ["Ganon Castle Shadow Trial Ending Side"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ganon Pot Shadow End 1"] = function () return true end,
            ["MQ Ganon Pot Shadow End 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ganon Castle Stairs"] = {
    ...
    ["Gerudo Training Grounds Maze Entrance Fork"] = {
        ["exits"] = {
            ["Gerudo Training Grounds"] = function () return true end,
            ["Gerudo Training Grounds Maze Left Room 1"] = function () return true end,
            ["Gerudo Training Grounds Maze Right Room 1"] = function () return event('GTG_MQ_LAVA_HAMMER_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Left Room 1"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Entrance Fork"] = function () return true end,
            ["Gerudo Training Grounds Maze Left Room 2"] = function () return true end,
            ["Gerudo Training Grounds Maze Left Hidden Room"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Left Hidden Room"] = {
        ...
        ["locations"] = {
            ["MQ Gerudo Training Grounds Maze First Chest"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Left Room 2"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Left Room 1"] = function () return true end,
            ["Gerudo Training Grounds Maze Left Room 3"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Gerudo Training Grounds Maze Left Room 3"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Left Room 2"] = function () return true end,
            ["Gerudo Training Grounds Maze Left Room 4"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Left Room 4"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Left Room 3"] = function () return true end,
            ["Gerudo Training Grounds Maze Left Room 5"] = function () return small_keys_gtg(1) end,
        },
        ["locations"] = {
    ...
    ["Gerudo Training Grounds Maze Left Room 5"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Left Room 4"] = function () return small_keys_gtg(2) end,
            ["Gerudo Training Grounds Maze Left Room 6"] = function () return small_keys_gtg(2) end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Maze Fourth Chest"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Left Room 6"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Left Room 5"] = function () return small_keys_gtg(3) end,
            ["Gerudo Training Grounds Maze Center Room"] = function () return small_keys_gtg(3) end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Center Room"] = {
        ["events"] = {
            ["GTG_ICE_ARROWS_SWITCH"] = function () return can_hammer() end,
        },
        ["exits"] = {
            ["Gerudo Training Grounds Maze Left Room 6"] = function () return small_keys_gtg(3) end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Maze Center Room Large Crate"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Right Room 1"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Entrance Fork"] = function () return true end,
            ["Gerudo Training Grounds Maze Right Room 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Maze Right Room 2"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Maze Right Room 1"] = function () return true end,
            ["Gerudo Training Grounds Lava Room Maze Ledge"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Maze Right Side Middle Chest"] = function () return true end,
            ["MQ Gerudo Training Grounds Maze Right Side Right Chest"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Right Path First Room"] = {
        ["events"] = {
            ["GTG_MQ_RIGHT_FIRST_CLEAR"] = function () return soul_lizalfos_dinalfos() and soul_armos() and soul_dodongo() and (can_use_sword_master() or (can_use_sword_goron() and has('SWORD_BIGGORON'))) end,
        },
        ["exits"] = {
            ["Gerudo Training Grounds"] = function () return true end,
            ["Gerudo Training Grounds Lava Room Near Beginning"] = function () return event('GTG_MQ_RIGHT_FIRST_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Right Side Dinolfos Chest"] = function () return event('GTG_MQ_RIGHT_FIRST_CLEAR') end,
            ["MQ Gerudo Training Grounds Wonder Item Dodongo Room Wall"] = function () return can_use_bow() end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Lava Room Near Beginning"] = {
        ...
        ["exits"] = {
            ["Gerudo Training Grounds Right Path First Room"] = function () return true end,
            ["Gerudo Training Grounds Lava Room Ledge Before Water Room"] = function () return can_longshot() or event('GTG_MQ_LAVA_TORCH_ENTRANCE_SIDE') or (trick('OOT_MQ_GTG_FLAMES') and (can_hookshot() or has_hover_boots())) or hookshot_anywhere() or climb_anywhere() end,
            ["Gerudo Training Grounds Lava Room Maze Ledge"] = function () return event('GTG_MQ_LAVA_HAMMER_SWITCH') and can_longshot() or (event('GTG_MQ_LAVA_TORCH_ENTRANCE_SIDE') and event('GTG_MQ_LAVA_HAMMER_SWITCH') and can_hookshot()) or hookshot_anywhere() or climb_anywhere() end,
            ["Gerudo Training Grounds Lava Room Deep Side Ledge"] = function () return hookshot_anywhere() and trick('OOT_MQ_GTG_FLAMES') or (can_longshot() and has_hover_boots() and climb_anywhere() and event('GTG_LAVA_HOOK_TARGETS')) end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds SR Lava Back-Left"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_ENTRANCE') end,
            ["MQ Gerudo Training Grounds SR Lava Back-Right"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_ENTRANCE') end,
            ["MQ Gerudo Training Grounds SR Lava Center"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_ENTRANCE') end,
            ["MQ Gerudo Training Grounds SR Lava Front-Right"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_ENTRANCE') end,
            ["MQ Gerudo Training Grounds SR Lava Front-Left"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_ENTRANCE') end,
            ["MQ Gerudo Training Grounds SR Lava Front"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_ENTRANCE') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Lava Room Ledge Before Water Room"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Gerudo Training Grounds SR Lava Back-Right"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_WATER_SIDE') end,
            ["MQ Gerudo Training Grounds SR Lava Center"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_WATER_SIDE') end,
            ["MQ Gerudo Training Grounds SR Lava Front-Right"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_WATER_SIDE') end,
            ["MQ Gerudo Training Grounds SR Lava Front-Left"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_WATER_SIDE') end,
            ["MQ Gerudo Training Grounds SR Lava Front"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_WATER_SIDE') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Lava Room Maze Ledge"] = {
        ...
        ["exits"] = {
            ["Gerudo Training Grounds Lava Room Near Beginning"] = function () return can_hookshot() or climb_anywhere() or event('GTG_MQ_LAVA_TORCH_FAR') or (trick('OOT_MQ_GTG_FLAMES') and has_hover_boots()) end,
            ["Gerudo Training Grounds Lava Room Ledge Before Water Room"] = function () return can_longshot() or hookshot_anywhere() or event('GTG_MQ_LAVA_TORCH_FAR') or (can_hookshot() and event('GTG_LAVA_HOOK_TARGETS')) or (has_hover_boots() and event('GTG_LAVA_HOOK_TARGETS') and trick('OOT_MQ_GTG_FLAMES')) end,
            ["Gerudo Training Grounds Lava Room Deep Side Ledge"] = function () return hookshot_anywhere() and trick('OOT_MQ_GTG_FLAMES') or climb_anywhere() or longshot_anywhere() end,
            ["Gerudo Training Grounds Maze Right Room 2"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds SR Lava Back-Left"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
            ["MQ Gerudo Training Grounds SR Lava Back-Right"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
            ["MQ Gerudo Training Grounds SR Lava Center"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
            ["MQ Gerudo Training Grounds SR Lava Front-Right"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
            ["MQ Gerudo Training Grounds SR Lava Front-Left"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
            ["MQ Gerudo Training Grounds SR Lava Front"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Lava Room Deep Side Ledge"] = {
        ...
        ["exits"] = {
            ["Gerudo Training Grounds Lava Room Near Beginning"] = function () return hookshot_anywhere() or (event('GTG_MQ_LAVA_TORCH_FAR') and (is_adult() or (climb_anywhere() or has_hover_boots()))) end,
            ["Gerudo Training Grounds Lava Room Maze Ledge"] = function () return event('GTG_MQ_LAVA_HAMMER_SWITCH') and can_longshot() or (event('GTG_LAVA_HOOK_TARGETS') and event('GTG_MQ_LAVA_HAMMER_SWITCH') and trick('OOT_MQ_GTG_FLAMES') and can_hookshot()) or hookshot_anywhere() or climb_anywhere() end,
            ["Gerudo Training Grounds Lava Room Ledge Before Water Room"] = function () return can_longshot() or hookshot_anywhere() or climb_anywhere() or (event('GTG_MQ_LAVA_TORCH_FAR') and (can_hookshot() or has_hover_boots())) or (event('GTG_LAVA_HOOK_TARGETS') and (is_adult() or has_hover_boots()) and can_hookshot()) end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds SR Lava Back-Left"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
            ["MQ Gerudo Training Grounds SR Lava Back-Right"] = function () return event('GTG_MQ_LAVA_TORCH_COMMON_PLATFORMS_FAR') end,
    ...
    ["Gerudo Training Grounds Slopes"] = {
        ["events"] = {
            ["GTG_MQ_SLOPES_STALAGMITES"] = function () return break_icicle() or climb_anywhere() or hookshot_anywhere() end,
        },
        ["exits"] = {
            ["Gerudo Training Grounds Wallmaster"] = function () return soul_wallmaster() end,
        ...
        ["locations"] = {
            ...
            ["MQ Gerudo Training Grounds Icicle Slopes 1"] = function () return break_icicle() end,
            ["MQ Gerudo Training Grounds Icicle Slopes 2"] = function () return break_icicle() end,
            ["MQ Gerudo Training Grounds Icicle Slopes 3"] = function () return break_icicle() end,
            ["MQ Gerudo Training Grounds Icicle Slopes 4"] = function () return break_icicle() end,
            ["MQ Gerudo Training Grounds Icicle Slopes 5"] = function () return break_icicle() end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Stalfos Room"] = {
        ["events"] = {
            ["BLUE_FIRE"] = function () return has_bottle() end,
            ["GTG_MQ_STALFOS_CLEAR"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and soul_skulltula() and has_weapon() end,
            ["GTG_MQ_SILVER_BLOCK_PUSH"] = function () return can_lift_silver() end,
        },
        ["exits"] = {
            ["Gerudo Training Grounds Slopes"] = function () return true end,
            ["Gerudo Training Grounds Stalfos Room Ledge Before Red Ice"] = function () return has_lens() and (can_play_time() and is_adult() or climb_anywhere() or hookshot_anywhere()) end,
            ["Gerudo Training Grounds Stalfos Room After Silver Block"] = function () return event('GTG_MQ_SILVER_BLOCK_PUSH') end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Stalfos Room Chest"] = function () return event('GTG_MQ_STALFOS_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Stalfos Room After Silver Block"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Stalfos Room"] = function () return event('GTG_MQ_SILVER_BLOCK_PUSH') end,
            ["Gerudo Training Grounds Stalfos Room Side Room"] = function () return event('GTG_MQ_STALFOS_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Stalfos Room Side Room"] = {
        ["events"] = {
            ["GTG_MQ_STALFOS_SIDE_CLEAR"] = function () return soul_enemy(SOUL_ENEMY_SPIKE) and soul_freezard() and (can_use_sword_master() or can_use_sword_razor() or (has('SWORD_BIGGORON') and age_sword_adult())) end,
        },
        ["exits"] = {
            ["Gerudo Training Grounds Stalfos Room After Silver Block"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Silver Block Room Chest"] = function () return event('GTG_MQ_STALFOS_SIDE_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Stalfos Room Ledge Before Red Ice"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Stalfos Room Ledge After Red Ice"] = function () return has_blue_fire() end,
            ["Gerudo Training Grounds Stalfos Room"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Stalfos Room Ledge After Red Ice"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Stalfos Room Ledge Before Red Ice"] = function () return has_blue_fire() or trick('OOT_PASS_COLLISION') end,
            ["Gerudo Training Grounds Spinning Statue Room Upper"] = function () return event('GTG_MQ_STALFOS_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Spinning Statue Room Upper"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Stalfos Room Ledge After Red Ice"] = function () return true end,
            ["Gerudo Training Grounds Spinning Statue Room Upper Side Door"] = function () return event('GTG_MQ_SPINNING_STATUE_CRYSTAL') end,
            ["Gerudo Training Grounds Spinning Statue Room Lower"] = function () return true end,
            ["Gerudo Training Grounds Spinning Statue Room Statue Middle"] = function () return has_hover_boots() or can_longshot() or hookshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Spinning Statue Room Upper Side Door"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Spinning Statue Room Upper"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Ice Arrows Chest"] = function () return event('GTG_ICE_ARROWS_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Spinning Statue Room Lower"] = {
        ["events"] = {
            ["GTG_MQ_SPINNING_STATUE_EYES"] = function () return can_use_bow() end,
            ["GTG_MQ_SPINNING_STATUE_CRYSTAL"] = function () return can_use_bow() or can_use_slingshot() or can_hookshot() or has_explosives() or can_boomerang() end,
        },
        ["exits"] = {
            ["Gerudo Training Grounds Spinning Statue Room Upper"] = function () return can_longshot() or hookshot_anywhere() or climb_anywhere() end,
            ["Gerudo Training Grounds Spinning Statue Room Statue Middle"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Gerudo Training Grounds Room After Spinning Statue"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Spinning Statue Chest"] = function () return event('GTG_MQ_SPINNING_STATUE_EYES') end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Spinning Statue Room Statue Middle"] = {
        ["exits"] = {
            ["Gerudo Training Grounds Spinning Statue Room Upper"] = function () return can_hookshot() and (is_adult() or (is_child() and has_hover_boots())) end,
            ["Gerudo Training Grounds Spinning Statue Room Lower"] = function () return is_adult() or (has_hover_boots() or climb_anywhere()) or hookshot_anywhere() end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Wonder Item Eye Statue"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Gerudo Training Grounds Room After Spinning Statue"] = {
        ["events"] = {
            ["GTG_MQ_KNUCKLE_AND_SLUGS_CLEAR"] = function () return soul_iron_knuckle() and soul_enemy(SOUL_ENEMY_TORCH_SLUG) and has_weapon() end,
            ["GTG_MQ_KNUCKLE_AND_SLUGS_WALL_CRYSTAL"] = function () return has_ranged_weapon() or has_explosives() or (climb_anywhere() and (can_use_mask_blast() or can_jump_slash())) end,
        },
        ["exits"] = {
            ["Gerudo Training Grounds Spinning Statue Room Lower"] = function () return event('GTG_MQ_KNUCKLE_AND_SLUGS_CLEAR') end,
            ["Gerudo Training Grounds Lava Room Deep Side Ledge"] = function () return event('GTG_MQ_KNUCKLE_AND_SLUGS_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Gerudo Training Grounds Torch Slug Room Clear Chest"] = function () return event('GTG_MQ_KNUCKLE_AND_SLUGS_CLEAR') end,
            ["MQ Gerudo Training Grounds Torch Slug Room Switch Chest"] = function () return event('GTG_MQ_KNUCKLE_AND_SLUGS_WALL_CRYSTAL') end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern"] = {
        ["exits"] = {
            ["Zora Fountain Frozen"] = function () return true end,
            ["Ice Cavern Room After Entrance Boulder"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Ice Cavern Room After Entrance Boulder"] = {
        ["events"] = {
            ["ICE_MQ_FIRST_CRYSTAL"] = function () return has_ranged_weapon() or has_explosives() or (is_adult() and (has('SWORD_BIGGORON') or can_use_mask_blast())) end,
        },
        ["exits"] = {
            ["Ice Cavern"] = function () return true end,
            ["Ice Cavern Ledge After Shiek"] = function () return climb_anywhere() or hookshot_anywhere() end,
            ["Ice Cavern After Initial Ice Block"] = function () return event('ICE_MQ_FIRST_CRYSTAL') end,
        },
        ["locations"] = {
            ["MQ Ice Cavern Icicle Entrance 1"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Entrance 2"] = function () return break_icicle_ice_cavern() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern After Initial Ice Block"] = {
        ["exits"] = {
            ["Ice Cavern Room After Entrance Boulder"] = function () return event('ICE_MQ_FIRST_CRYSTAL') end,
            ["Ice Cavern Main Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ice Cavern Pot First Room 1"] = function () return true end,
            ["MQ Ice Cavern Pot First Room 2"] = function () return true end,
            ["MQ Ice Cavern Icicle Before Main Room 1"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Before Main Room 2"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Before Main Room 3"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Before Main Room 4"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Before Main Room 5"] = function () return break_icicle_ice_cavern() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Main Room"] = {
        ["events"] = {
            ["ICE_MQ_MAIN_ENEMIES_CLEAR"] = function () return soul_wolfos() and soul_freezard() and (has_weapon() or can_use_sticks() or has_explosives()) end,
        },
        ["exits"] = {
            ["Ice Cavern After Initial Ice Block"] = function () return true end,
            ["Ice Cavern Map Room"] = function () return event('ICE_MQ_MAIN_ENEMIES_CLEAR') end,
            ["Ice Cavern Compass Room"] = function () return has_blue_fire() and is_adult() or climb_anywhere() or hookshot_anywhere() end,
            ["Ice Cavern Vanilla Push Block Room"] = function () return has_blue_fire() or climb_anywhere() or hookshot_anywhere() end,
        },
        ["locations"] = {
            ...
            ["MQ Ice Cavern Icicle Main Room 1"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Main Room 2"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Main Room 3"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Main Room 4"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Main Room 5"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Red Ice Main Room 1"] = function () return has_blue_fire() end,
            ["MQ Ice Cavern Red Ice Main Room 2"] = function () return has_blue_fire() end,
            ["MQ Ice Cavern Red Ice Main Room 3"] = function () return has_blue_fire() end,
            ["MQ Ice Cavern Red Ice Main Room 4"] = function () return has_blue_fire() end,
            ["MQ Ice Cavern Red Ice Main Room 5"] = function () return has_blue_fire() end,
            ["MQ Ice Cavern Red Ice Main Room 6"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Map Room"] = {
        ["events"] = {
            ["BLUE_FIRE"] = function () return has_bottle() and event('ICE_MQ_MAP_STALAGMITES') end,
            ["ICE_MQ_MAP_STALAGMITES"] = function () return true end,
            ["ICE_MQ_MAP_SWITCH"] = function () return has_weapon() or has_bombs() or can_use_mask_blast() or has_bombchu() or can_use_bow() or can_use_slingshot() or can_hookshot() or can_use_sticks() or can_hammer() or can_boomerang() end,
        },
        ["exits"] = {
            ["Ice Cavern Main Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ice Cavern Map Chest"] = function () return event('ICE_MQ_MAP_SWITCH') and has_blue_fire() end,
            ["MQ Ice Cavern Icicle Before Map Room"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 1"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 2"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 3"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 4"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 5"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 6"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 7"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 8"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Map Room 9"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Red Ice Map Room"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Compass Room"] = {
        ...
        ["exits"] = {
            ["Ice Cavern Main Room"] = function () return has_blue_fire() or climb_anywhere() or hookshot_anywhere() end,
        },
        ["locations"] = {
            ["MQ Ice Cavern Compass Chest"] = function () return true end,
            ["MQ Ice Cavern Piece of Heart"] = function () return has_explosives() or (can_use_mask_blast() and (is_child() or (is_adult() and has_shield()))) end,
            ["MQ Ice Cavern GS Compass Room"] = function () return gs() and (can_play_time() and has_blue_fire() and can_damage_skull() or has_blue_fire_arrows() or can_boomerang()) end,
            ["MQ Ice Cavern Pot Compass Room 1"] = function () return true end,
            ["MQ Ice Cavern Pot Compass Room 2"] = function () return true end,
            ["MQ Ice Cavern Icicle Compass Room 1"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Compass Room 2"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Compass Room 3"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Icicle Compass Room 4"] = function () return break_icicle_ice_cavern() end,
            ["MQ Ice Cavern Red Ice Compass Room"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Vanilla Push Block Room"] = {
        ["events"] = {
            ["BLUE_FIRE"] = function () return has_bottle() and (climb_anywhere() or hookshot_anywhere() or can_play_time()) end,
            ["ICE_MQ_SECOND_CRYSTAL"] = function () return has_ranged_weapon() or has_explosives() or (is_adult() and (has('SWORD_BIGGORON') or can_use_mask_blast())) end,
        },
        ["exits"] = {
            ["Ice Cavern Main Room"] = function () return has_blue_fire() or hookshot_anywhere() end,
            ["Ice Cavern Vanilla Push Block Room Scarecrow Ledge"] = function () return climb_anywhere() or hookshot_anywhere() or scarecrow_hookshot() or (is_adult() and trick('OOT_MQ_ICE_SCARE_NOTHING')) or (has_hover_boots() and (can_longshot() or (is_adult() and trick('OOT_MQ_ICE_SCARE_HOVER')))) end,
            ["Ice Cavern Past Red Ice Before Shiek"] = function () return climb_anywhere() or hookshot_anywhere() or (has_blue_fire() and (is_adult() or (can_play_time() and has_hover_boots()))) end,
        },
        ["locations"] = {
            ["MQ Ice Cavern GS Clear Blocks"] = function () return gs() and event('ICE_MQ_SECOND_CRYSTAL') and can_damage_skull() end,
            ["MQ Ice Cavern Red Ice Block Room 1"] = function () return has_blue_fire() end,
            ["MQ Ice Cavern Red Ice Block Room 2"] = function () return has_blue_fire() end,
            ["MQ Ice Cavern Red Ice Block Room 3"] = function () return has_blue_fire() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Vanilla Push Block Room Scarecrow Ledge"] = {
        ["exits"] = {
            ["Ice Cavern Vanilla Push Block Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ice Cavern GS Scarecrow"] = function () return gs() and can_damage_skull() end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Past Red Ice Before Shiek"] = {
        ["exits"] = {
            ["Ice Cavern Vanilla Push Block Room"] = function () return has_blue_fire() or hookshot_anywhere() end,
            ["Ice Cavern Serenade Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Ice Cavern Pot Final Corridor 1"] = function () return true end,
            ["MQ Ice Cavern Pot Final Corridor 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Serenade Room"] = {
        ["events"] = {
            ["ICE_MQ_FINAL_ROOM_CLEAR"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() end,
        },
        ["exits"] = {
            ["Ice Cavern Past Red Ice Before Shiek"] = function () return event('ICE_MQ_FINAL_ROOM_CLEAR') end,
            ["Ice Cavern Ledge After Shiek"] = function () return event('ICE_MQ_FINAL_ROOM_CLEAR') and has_iron_boots() end,
        },
        ["locations"] = {
            ["MQ Ice Cavern Iron Boots"] = function () return event('ICE_MQ_FINAL_ROOM_CLEAR') end,
            ["MQ Ice Cavern Sheik Song"] = function () return event('ICE_MQ_FINAL_ROOM_CLEAR') and soul_npc(SOUL_NPC_SHEIK) end,
        },
        ["age_change"] = true,
    },
    ["Ice Cavern Ledge After Shiek"] = {
        ["exits"] = {
            ["Ice Cavern Room After Entrance Boulder"] = function () return true end,
            ["Ice Cavern Serenade Room"] = function () return has_iron_boots() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Jabu-Jabu Entrance Boulder"] = function () return has_explosives_or_hammer() end,
            ...
            ["MQ Jabu-Jabu Wonder Item Entrance Left Cow"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Entrance Right Cow"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Main Elevator Room Upper"] = {
        ["events"] = {
            ["JABU_MQ_LOWER_BIG_OCTO_PLATFORM"] = function () return climb_anywhere() or longshot_anywhere() end,
        },
        ["exits"] = {
            ["Jabu-Jabu"] = function () return true end,
            ["Jabu-Jabu Vanilla Ruto Falling Room"] = function () return event('JABU_MQ_UNDERWATER_ALCOVE_SWITCH') end,
            ["Jabu-Jabu Main Elevator Room Ledge Before Pre-Boss"] = function () return has_hover_boots() or (can_use_slingshot() and can_hookshot()) or event('JABU_MQ_LOWER_BIG_OCTO_PLATFORM') or climb_anywhere() end,
            ["Jabu-Jabu Main Elevator Room Lower"] = function () return true end,
            ["Jabu-Jabu Main Elevator Room High Ledge"] = function () return climb_anywhere() end,
        },
        ["locations"] = {
    ...
    ["Jabu-Jabu Main Elevator Room Lower"] = {
        ["exits"] = {
            ["Jabu-Jabu Main Elevator Room Upper"] = function () return true end,
            ["Jabu-Jabu Main Elevator Room Lower Switch Alcove"] = function () return is_child() or (is_adult() and can_dive_small()) end,
            ["Jabu Jabu Basement Loop to Main Elevator"] = function () return event('JABU_MQ_WATER_SPOUTS') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Second Room B1 Chest"] = function () return true end,
            ["MQ Jabu-Jabu Wonder Item Elevator Cow"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Main Elevator Room Lower Switch Alcove"] = {
        ["events"] = {
            ["JABU_MQ_UNDERWATER_ALCOVE_SWITCH"] = function () return true end,
        },
        ["exits"] = {
            ["Jabu-Jabu Main Elevator Room Lower"] = function () return is_child() or (is_adult() and can_dive_small()) end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Compass Chest"] = function () return can_hit_triggers_distance() or can_hookshot() or has_bombchu() end,
            ["MQ Jabu-Jabu Pot Underwater Alcove 1"] = function () return true end,
            ["MQ Jabu-Jabu Pot Underwater Alcove 2"] = function () return true end,
            ["MQ Jabu-Jabu Rupee Bottom"] = function () return can_dive_big() end,
    ...
    ["Jabu-Jabu Main Elevator Room Ledge Before Pre-Boss"] = {
        ["exits"] = {
            ["Jabu-Jabu Main Elevator Room Upper"] = function () return has_hover_boots() or event('JABU_MQ_LOWER_BIG_OCTO_PLATFORM') or climb_anywhere() end,
            ["Jabu-Jabu Main Elevator Room Lower"] = function () return true end,
            ["Jabu-Jabu Pre-Boss Room"] = function () return event('JABU_TENTACLE_RED') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Second Room 1F Chest"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Vanilla Ruto Falling Room"] = {
        ["events"] = {
            ["JABU_MQ_BACK_UNLOCK"] = function () return can_use_slingshot() and (has_explosives() or (climb_anywhere() and (can_hammer() or can_use_mask_blast()))) end,
        },
        ["exits"] = {
            ["Jabu-Jabu Main Elevator Room Upper"] = function () return true end,
            ["Jabu-Jabu Vanilla Ruto Falling Room After Wiggling Thing"] = function () return can_boomerang() end,
            ["Jabu-Jabu Ruto Section Bottom Floor"] = function () return true end,
            ["Jabu-Jabu Basement Ledge Before Big Octo"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Grass Main Room Top 1"] = function () return can_cut_grass() and has_explosives_or_hammer() end,
            ["MQ Jabu-Jabu Grass Main Room Top 2"] = function () return can_cut_grass() and has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Vanilla Ruto Falling Room After Wiggling Thing"] = {
        ["exits"] = {
            ["Jabu-Jabu Vanilla Ruto Falling Room"] = function () return can_boomerang() end,
            ["Jabu-Jabu Back Forked Paths"] = function () return event('JABU_MQ_BACK_UNLOCK') end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Ruto Section Bottom Floor"] = {
        ["exits"] = {
            ["Jabu-Jabu Vanilla Ruto Falling Room"] = function () return true end,
            ["Jabu-Jabu Basement Side Room"] = function () return event('JABU_TENTACLE_RED') end,
            ["Jabu Jabu Basement Loop to Main Elevator"] = function () return soul_ruto() or can_play_elegy() end,
            ["Jabu-Jabu Basement Ledge Before Big Octo"] = function () return climb_anywhere() end,
        },
        ["locations"] = {
            ...
            ["MQ Jabu-Jabu Grass Main Room Bottom 1"] = function () return can_cut_grass() end,
            ["MQ Jabu-Jabu Grass Main Room Bottom 2"] = function () return can_cut_grass() end,
            ["MQ Jabu-Jabu Grass Main Room Bottom 3"] = function () return can_cut_grass() end,
            ["MQ Jabu-Jabu Hole Room Boulder Ground"] = function () return has_explosives_or_hammer() end,
            ["MQ Jabu-Jabu Hole Room Boulder Wall"] = function () return has_explosives() end,
        },
        ["age_change"] = true,
    },
    ["Jabu Jabu Basement Loop to Main Elevator"] = {
        ["events"] = {
            ["JABU_MQ_WATER_SPOUTS"] = function () return soul_lizalfos_dinalfos() and soul_like_like() and soul_enemy(SOUL_ENEMY_STINGER) and has_weapon() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Ruto Section Bottom Floor"] = function () return true end,
            ["Jabu-Jabu Main Elevator Room Lower"] = function () return event('JABU_MQ_WATER_SPOUTS') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Boomerang Chest"] = function () return event('JABU_MQ_WATER_SPOUTS') end,
            ["MQ Jabu-Jabu SoT Room Lower Chest"] = function () return true end,
            ["MQ Jabu-Jabu Pot Boomerang Room 1"] = function () return true end,
            ["MQ Jabu-Jabu Pot Boomerang Room 2"] = function () return true end,
            ["MQ Jabu-Jabu GS SoT Block"] = function () return gs() and can_play_time() and can_damage_skull() end,
            ["MQ Jabu-Jabu Grass Boomerang Room"] = function () return can_cut_grass() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Basement Ledge Before Big Octo"] = {
        ["exits"] = {
            ["Jabu-Jabu Ruto Section Bottom Floor"] = function () return true end,
            ["Jabu-Jabu Basement Ledge Before Big Octo After Green Tentacle"] = function () return event('JABU_TENTACLE_GREEN') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Wonder Item Basement Right Cow 1"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Basement Right Cow 2"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Basement Right Cow 3"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Basement Left Cow 1"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Basement Left Cow 2"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Basement Left Cow 3"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Basement Ledge Before Big Octo After Green Tentacle"] = {
        ["exits"] = {
            ["Jabu-Jabu Basement Ledge Before Big Octo"] = function () return event('JABU_TENTACLE_GREEN') end,
            ["Jabu-Jabu Big Octo Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Big Octo Room"] = {
        ["events"] = {
            ["JABU_BIG_OCTO"] = function () return soul_octorok() and (can_use_sticks() or has_weapon()) and soul_ruto() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Basement Ledge Before Big Octo After Green Tentacle"] = function () return true end,
            ["Jabu-Jabu Above Big Octo Fight"] = function () return event('JABU_BIG_OCTO') end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Above Big Octo Fight"] = {
        ["exits"] = {
            ["Jabu-Jabu Room After Above Big Octo"] = function () return can_use_slingshot() end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Grass Big Octo Top 1"] = function () return can_cut_grass() end,
            ["MQ Jabu-Jabu Grass Big Octo Top 2"] = function () return can_cut_grass() end,
            ["MQ Jabu-Jabu Wonder Item After Big Octo"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Room After Above Big Octo"] = {
        ["events"] = {
            ["MQ_JABU_AFTER_ABOVE_BIG_OCTO_SPAWN_COW_AND_CRATES"] = function () return can_use_slingshot() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Above Big Octo Fight"] = function () return true end,
        ...
        ["locations"] = {
            ...
            ["MQ Jabu-Jabu Room After Above Big Octo Small Crate 2"] = function () return event('MQ_JABU_AFTER_ABOVE_BIG_OCTO_SPAWN_COW_AND_CRATES') end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Main Elevator Room High Ledge"] = {
        ["events"] = {
            ["JABU_MQ_LOWER_BIG_OCTO_PLATFORM"] = function () return true end,
        },
        ["exits"] = {
            ["Jabu-Jabu Room After Above Big Octo"] = function () return true end,
            ["Jabu-Jabu Main Elevator Room Upper"] = function () return true end,
            ["Jabu-Jabu Main Elevator Room Lower"] = function () return true end,
        },
        ["age_change"] = true,
    ...
    ["Jabu-Jabu Back Forked Paths"] = {
        ["events"] = {
            ["JABU_MQ_BACK_CENTER_SWITCH"] = function () return has_explosives_or_hammer() end,
            ["JABU_MQ_BACK_FIRE_WEB"] = function () return has_fire() or (can_use_sticks() and event('JABU_MQ_LIKE_LIKE_ROOM_CLEAR')) end,
        },
        ["exits"] = {
            ["Jabu-Jabu Vanilla Ruto Falling Room After Wiggling Thing"] = function () return true end,
            ["Jabu-Jabu Back Forked Paths Left 1"] = function () return event('JABU_TENTACLE_BLUE') end,
            ["Jabu-Jabu Back Forked Paths Left 2"] = function () return event('JABU_TENTACLE_BLUE') end,
            ["Jabu-Jabu Back Forked Paths Center After Rocks"] = function () return has_explosives_or_hammer() or climb_anywhere() end,
            ["Jabu-Jabu Back Forked Paths Right 1"] = function () return true end,
            ["Jabu-Jabu Back Forked Paths After Right 2 Web"] = function () return event('JABU_MQ_BACK_FIRE_WEB') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Back Forked Paths Main Room Small Crate 1"] = function () return true end,
            ["MQ Jabu-Jabu Back Forked Paths Main Room Small Crate 2"] = function () return true end,
            ["MQ Jabu-Jabu Forked Path Boulders"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Back Forked Paths Center After Rocks"] = {
        ["events"] = {
            ["JABU_MQ_BACK_CENTER_SWITCH"] = function () return can_play_elegy() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Back Forked Paths"] = function () return has_explosives_or_hammer() or climb_anywhere() end,
            ["Jabu-Jabu Back Forked Paths Center Room"] = function () return event('JABU_MQ_BACK_CENTER_SWITCH') end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Back Forked Paths Center Room"] = {
        ...
        ["exits"] = {
            ["Jabu-Jabu Back Forked Paths Center After Rocks"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Back Forked Paths Right 1"] = {
        ["events"] = {
            ["JABU_MQ_LIKE_LIKE_ROOM_CLEAR"] = function () return soul_like_like() and can_use_slingshot() end,
        },
        ["exits"] = {
            ["Jabu-Jabu Back Forked Paths"] = function () return event('JABU_MQ_LIKE_LIKE_ROOM_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Back Chest"] = function () return event('JABU_MQ_LIKE_LIKE_ROOM_CLEAR') end,
            ...
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Cow Right 1"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Cow Right 2"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Cow Right 3"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Cow Left 1"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Cow Left 2"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Cow Left 3"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Explosion 1"] = function () return has_explosives() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Explosion 2"] = function () return has_explosives() end,
            ["MQ Jabu-Jabu Wonder Item Falling Like-Likes Explosion 3"] = function () return has_explosives() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Back Forked Paths After Right 2 Web"] = {
        ["exits"] = {
            ["Jabu-Jabu Back Forked Paths"] = function () return true end,
            ["Jabu-Jabu Back Forked Paths Right 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Back Forked Paths Right 2"] = {
        ["events"] = {
            ["JABU_TENTACLE_BLUE"] = function () return can_boomerang() and soul_enemy(SOUL_ENEMY_PARASITE) end,
        },
        ["exits"] = {
            ["Jabu-Jabu Back Forked Paths After Right 2 Web"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Back Forked Paths Left 1"] = {
        ["events"] = {
            ["JABU_TENTACLE_GREEN"] = function () return can_boomerang() and soul_enemy(SOUL_ENEMY_PARASITE) end,
        },
        ["exits"] = {
            ["Jabu-Jabu Back Forked Paths"] = function () return true end,
        },
    ...
    ["Jabu-Jabu Back Forked Paths Left 2"] = {
        ...
        ["locations"] = {
            ["MQ Jabu-Jabu GS Back"] = function () return gs() and (climb_anywhere() and (can_hammer() or can_use_mask_blast()) or (has_explosives() and can_collect_distance())) end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Basement Side Room"] = {
        ["events"] = {
            ["JABU_MQ_BASEMENT_SIDE_PLATFORM"] = function () return has_lens() and soul_keese() and soul_enemy(SOUL_ENEMY_STINGER) and has_ranged_weapon() end,
            ["JABU_MQ_BASEMENT_SIDE_BUTTON"] = function () return true end,
        },
        ["exits"] = {
            ["Jabu-Jabu Ruto Section Bottom Floor"] = function () return event('JABU_MQ_BASEMENT_SIDE_BUTTON') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu GS Basement Side Room"] = function () return gs() and (event('JABU_MQ_BASEMENT_SIDE_PLATFORM') and can_collect_distance() or (has_hover_boots() and can_collect_distance()) or (has_fire_arrows() and can_longshot()) or (climb_anywhere() and can_damage_skull())) end,
            ["MQ Jabu-Jabu Side Room Boulders"] = function () return has_explosives_or_hammer() end,
        },
        ["age_change"] = true,
    },
    ["Jabu-Jabu Pre-Boss Room"] = {
        ...
        ["exits"] = {
            ["Jabu-Jabu Main Elevator Room Ledge Before Pre-Boss"] = function () return true end,
            ["Jabu-Jabu Boss"] = function () return event('JABU_MQ_END') end,
        },
        ["locations"] = {
            ["MQ Jabu-Jabu Pre-Boss Chest"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu GS Pre-Boss"] = function () return gs() and can_collect_distance() end,
            ...
            ["MQ Jabu-Jabu Wonder Item Before Boss Left Cow"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Before Boss Right Cow 1"] = function () return can_use_slingshot() end,
            ["MQ Jabu-Jabu Wonder Item Before Boss Right Cow 2"] = function () return can_use_slingshot() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple"] = {
        ["exits"] = {
            ["Graveyard Upper"] = function () return true end,
            ["Shadow Temple Entrance"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Entrance"] = {
        ["exits"] = {
            ["Shadow Temple"] = function () return true end,
            ["Shadow Temple Truth Spinner"] = function () return has_lens() and (can_hookshot() or has_hover_boots() or glitch_megaflip() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Truth Spinner"] = {
        ["events"] = {
            ["SHADOW_MQ_TRUTH_SPINNER"] = function () return has_lens() end,
            ["SHADOW_MQ_SPINNER_ROOM_ICE_PLATFORM"] = function () return has_fire_arrows() end,
            ["ARROWS"] = function () return true end,
            ["BOMBS_OR_BOMBCHU"] = function () return true end,
            ["MAGIC"] = function () return true end,
        },
        ["exits"] = {
            ["Shadow Temple Entrance"] = function () return has_lens() and (can_hookshot() or has_hover_boots() or glitch_megaflip() or climb_anywhere()) end,
            ["Shadow Temple Spinner Room Hallway to Textured Maze"] = function () return has_lens() and has_explosives() end,
            ["Shadow Temple Spinner Room Platform Before First Beamos Fork"] = function () return event('SHADOW_MQ_SPINNER_ROOM_ICE_PLATFORM') or has_hover_boots() or hookshot_anywhere() or glitch_megaflip() end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Truth Spinner Room Small Crate 1"] = function () return true end,
            ["MQ Shadow Temple Truth Spinner Room Small Crate 2"] = function () return true end,
            ["MQ Shadow Temple Truth Spinner Room Small Crate 3"] = function () return true end,
            ["MQ Shadow Temple Truth Spinner Room Small Crate 4"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Spinner Room Hallway to Textured Maze"] = {
        ["exits"] = {
            ["Shadow Temple Truth Spinner"] = function () return has_lens() and has_explosives() or trick('OOT_PASS_COLLISION') end,
            ["Shadow Temple Textured Maze Room Lens Alcove to Spinner"] = function () return small_keys_shadow(6) end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Textured Maze Room Lens Alcove to Spinner"] = {
        ["exits"] = {
            ["Shadow Temple Spinner Room Hallway to Textured Maze"] = function () return small_keys_shadow(6) end,
            ["Shadow Temple Textured Maze First Half"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Textured Maze First Half"] = {
        ["exits"] = {
            ["Shadow Temple Textured Maze Room Lens Alcove to Spinner"] = function () return has_lens() end,
            ["Shadow Temple Textured Maze Room Lens Alcove to Side Room"] = function () return has_lens() end,
            ["Shadow Temple Textured Maze Second Half"] = function () return has_lens() and can_play_time() end,
        },
        ["locations"] = {
    ...
    ["Shadow Temple Textured Maze Room Lens Alcove to Side Room"] = {
        ["exits"] = {
            ["Shadow Temple Textured Maze First Half"] = function () return has_lens() end,
            ["Shadow Temple Textured Maze First Half Side Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Textured Maze First Half Side Room"] = {
        ["events"] = {
            ["SHADOW_MQ_TEXTURED_MAZE_SIDE_ROOM_CLEAR"] = function () return soul_redead_gibdo() and (can_use_din() or (can_play_sun() and (has_weapon() or can_use_sticks()))) end,
        },
        ["exits"] = {
            ["Shadow Temple Textured Maze Room Lens Alcove to Side Room"] = function () return event('SHADOW_MQ_TEXTURED_MAZE_SIDE_ROOM_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Compass Chest"] = function () return event('SHADOW_MQ_TEXTURED_MAZE_SIDE_ROOM_CLEAR') end,
            ["MQ Shadow Temple Pot Compass Room 1"] = function () return can_use_din() or can_play_sun() end,
            ["MQ Shadow Temple Pot Compass Room 2"] = function () return can_use_din() or can_play_sun() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Textured Maze Second Half"] = {
        ["events"] = {
            ["SHADOW_MQ_TEXTURED_MAZE_LOWER_ICE_BLOCK"] = function () return can_hit_triggers_distance() end,
        },
        ["exits"] = {
            ["Shadow Temple Textured Maze First Half"] = function () return has_lens() and can_play_time() end,
            ["Shadow Temple Textured Maze Room Lens Alcove to Dead Hand Room"] = function () return has_lens() and event('SHADOW_MQ_TEXTURED_MAZE_LOWER_ICE_BLOCK') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Flying Pot Entrance Maze 3"] = function () return soul_flying_pot() end,
            ["MQ Shadow Temple Flying Pot Entrance Maze 4"] = function () return soul_flying_pot() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Textured Maze Room Lens Alcove to Dead Hand Room"] = {
        ["exits"] = {
            ["Shadow Temple Textured Maze Second Half"] = function () return has_lens() and event('SHADOW_MQ_TEXTURED_MAZE_LOWER_ICE_BLOCK') end,
            ["Shadow Temple Dead Hand Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Dead Hand Room"] = {
        ["events"] = {
            ["SHADOW_MQ_TEXTURED_MAZE_DEAD_HAND_ROOM_CLEAR"] = function () return soul_enemy(SOUL_ENEMY_DEAD_HAND) and (has_weapon() or (can_use_sticks() and trick('OOT_DEAD_HAND_STICKS'))) end,
        },
        ["exits"] = {
            ["Shadow Temple Textured Maze Room Lens Alcove to Dead Hand Room"] = function () return event('SHADOW_MQ_TEXTURED_MAZE_DEAD_HAND_ROOM_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Hover Boots Chest"] = function () return event('SHADOW_MQ_TEXTURED_MAZE_DEAD_HAND_ROOM_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Spinner Room Platform Before First Beamos Fork"] = {
        ["events"] = {
            ["SHADOW_MQ_SPINNER_ROOM_ICE_PLATFORM"] = function () return has_fire() end,
        },
        ["exits"] = {
            ["Shadow Temple Truth Spinner"] = function () return event('SHADOW_MQ_SPINNER_ROOM_ICE_PLATFORM') or has_hover_boots() or hookshot_anywhere() or glitch_megaflip() end,
            ["Shadow Temple First Beamos Fork"] = function () return event('SHADOW_MQ_TRUTH_SPINNER') end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple First Beamos Fork"] = {
        ["exits"] = {
            ["Shadow Temple Spinner Room Platform Before First Beamos Fork"] = function () return event('SHADOW_MQ_TRUTH_SPINNER') end,
            ["Shadow Temple First Beamos Fork Room Lens Alcove to Gibdo Room"] = function () return has_lens() end,
            ["Shadow Temple First Beamos Fork Room Lens Alcove to Scythe and Shortcut Room"] = function () return has_lens() end,
            ["Shadow Temple First Beamos Fork Room Solid Wall Alcove to Guillotine Hallway"] = function () return has_explosives() end,
        },
        ["locations"] = {
    ...
    ["Shadow Temple First Beamos Fork Room Lens Alcove to Gibdo Room"] = {
        ["exits"] = {
            ["Shadow Temple First Beamos Fork"] = function () return has_lens() end,
            ["Shadow Temple First Beamos Fork Gibdo Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple First Beamos Fork Gibdo Room"] = {
        ["events"] = {
            ["SHADOW_MQ_FIRST_BEAMOS_FORK_GIBDOS_CLEAR"] = function () return soul_redead_gibdo() and (has_weapon() or can_use_sticks() or can_use_din()) end,
        },
        ["exits"] = {
            ["Shadow Temple First Beamos Fork Room Lens Alcove to Gibdo Room"] = function () return event('SHADOW_MQ_FIRST_BEAMOS_FORK_GIBDOS_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple First Gibdos Chest"] = function () return event('SHADOW_MQ_FIRST_BEAMOS_FORK_GIBDOS_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ...
    ["Shadow Temple Scythe Room"] = {
        ...
        ["exits"] = {
            ...
            ["Shadow Temple Enclosed Shortcut Area"] = function () return has_lens() and get_past_skultullas() end,
        },
        ["locations"] = {
            ["MQ Shadow Temple SR Scythe 1"] = function () return true end,
            ["MQ Shadow Temple SR Scythe 2"] = function () return can_hookshot() or (is_adult() and has_hover_boots()) or climb_anywhere() end,
    ...
    ["Shadow Temple Enclosed Shortcut Area"] = {
        ["exits"] = {
            ["Shadow Temple Scythe Room"] = function () return get_past_skultullas() end,
            ["Shadow Temple Area Before Boat Ride"] = function () return event('SHADOW_MQ_SHORTCUT_BLOCK_PUSHED') end,
            ["Shadow Temple Area Above Shortcut Enclosure"] = function () return climb_anywhere() and trick('OOT_SHADOW_BOAT_EARLY') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Boat Passage Chest"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple First Beamos Fork Room Solid Wall Alcove to Guillotine Hallway"] = {
        ["exits"] = {
            ["Shadow Temple First Beamos Fork"] = function () return has_explosives() or trick('OOT_PASS_COLLISION') end,
            ["Shadow Temple Upper Guillotine Hallway Before Upper Huge Pit"] = function () return small_keys_shadow(2) end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Upper Guillotine Hallway Before Upper Huge Pit"] = {
        ["exits"] = {
            ["Shadow Temple First Beamos Fork Room Solid Wall Alcove to Guillotine Hallway"] = function () return small_keys_shadow(2) end,
            ["Shadow Temple Lower Guillotine Hallway Before Upper Huge Pit"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Lower Guillotine Hallway Before Upper Huge Pit"] = {
        ["exits"] = {
            ["Shadow Temple Upper Guillotine Hallway Before Upper Huge Pit"] = function () return can_hookshot() or climb_anywhere() end,
            ["Shadow Temple Upper Huge Pit Guillotine Section Before Drop"] = function () return true end,
            ["Shadow Temple Lower Huge Pit Ledge Before Invisible Spike Floors"] = function () return climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Upper Huge Pit Guillotine Section Before Drop"] = {
        ["exits"] = {
            ["Shadow Temple Lower Guillotine Hallway Before Upper Huge Pit"] = function () return true end,
            ["Shadow Temple Upper Huge Pit After Drop"] = function () return true end,
            ["Shadow Temple Lower Huge Pit"] = function () return longshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Upper Huge Pit After Drop"] = {
        ["events"] = {
            ["SHADOW_MQ_UPPER_HUGE_PIT_FROZEN_EYE"] = function () return has_fire() end,
        },
        ["exits"] = {
            ["Shadow Temple Upper Huge Pit Guillotine Section Before Drop"] = function () return hookshot_anywhere() end,
            ["Shadow Temple Upper Huge Pit East Ledge Before Invisible Scythe Room"] = function () return has_lens() end,
            ["Shadow Temple Lower Huge Pit"] = function () return event('SHADOW_MQ_UPPER_HUGE_PIT_FROZEN_EYE') or has_hover_boots() or hookshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Upper Huge Pit East Ledge Before Invisible Scythe Room"] = {
        ["exits"] = {
            ["Shadow Temple Upper Huge Pit After Drop"] = function () return has_lens() end,
            ["Shadow Temple Invisible Scythe Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Scythe Room"] = {
        ["exits"] = {
            ["Shadow Temple Upper Huge Pit East Ledge Before Invisible Scythe Room"] = function () return true end,
        },
        ["locations"] = {
    ...
    ["Shadow Temple Lower Huge Pit"] = {
        ["events"] = {
            ["SHADOW_MQ_LOWER_HUGE_PIT_FALLING_SPIKES_GATE_SWITCH"] = function () return has_weapon() or has_bombs() or can_use_mask_blast() or has_bombchu() or can_use_sticks() or can_hammer() end,
        },
        ["exits"] = {
            ["Shadow Temple Upper Huge Pit After Drop"] = function () return event('SHADOW_MQ_UPPER_HUGE_PIT_FROZEN_EYE') or hookshot_anywhere() end,
            ["Shadow Temple Falling Spikes Room Entrance"] = function () return event('SHADOW_MQ_LOWER_HUGE_PIT_FALLING_SPIKES_GATE_SWITCH') end,
            ["Shadow Temple Lower Huge Pit Ledge Before Invisible Spike Floors"] = function () return has_lens() and (has_hover_boots() or climb_anywhere() or hookshot_anywhere()) end,
    ...
    ["Shadow Temple Falling Spikes Room Entrance"] = {
        ["events"] = {
            ["SHADOW_MQ_LOWER_HUGE_PIT_FALLING_SPIKES_GATE_SWITCH"] = function () return has_weapon() or has_bombs() or can_use_mask_blast() or has_bombchu() or can_use_bow() or can_use_slingshot() or can_hookshot() or can_use_sticks() or can_hammer() or can_boomerang() end,
        },
        ["exits"] = {
            ["Shadow Temple Lower Huge Pit"] = function () return event('SHADOW_MQ_LOWER_HUGE_PIT_FALLING_SPIKES_GATE_SWITCH') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Spike Curtain Ground Chest"] = function () return true end,
            ["MQ Shadow Temple Spike Curtain Upper Cage Chest"] = function () return has_lens() and has_goron_bracelet() and is_adult() or climb_anywhere() or hookshot_anywhere() end,
            ["MQ Shadow Temple Spike Curtain Upper Switch Chest"] = function () return has_lens() and has_goron_bracelet() and is_adult() or climb_anywhere() or hookshot_anywhere() end,
            ["MQ Shadow Temple GS Spike Curtain"] = function () return gs() and collect_gs_on_walls() end,
            ...
            ["MQ Shadow Temple Pot Guillotines Room Upper 1"] = function () return has_lens() and has_goron_bracelet() and is_adult() or climb_anywhere() or hookshot_anywhere() end,
            ["MQ Shadow Temple Pot Guillotines Room Upper 2"] = function () return has_lens() and has_goron_bracelet() and is_adult() or climb_anywhere() or hookshot_anywhere() end,
    ...
    ["Shadow Temple Lower Huge Pit Ledge Before Invisible Spike Floors"] = {
        ["exits"] = {
            ["Shadow Temple Lower Huge Pit"] = function () return has_lens() and (has_hover_boots() or hookshot_anywhere()) end,
            ["Shadow Temple Invisible Spike Floors Room"] = function () return small_keys_shadow(3) end,
            ["Shadow Temple Lower Guillotine Hallway Before Upper Huge Pit"] = function () return climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Spike Floors Room"] = {
        ...
        ["exits"] = {
            ...
            ["Shadow Temple Invisible Spike Floors Room Upper Ledge Before Door"] = function () return hookshot_anywhere() or climb_anywhere() or ((is_adult() and can_hookshot() and (event('SHADOW_MQ_INVISIBLE_SPIKE_FLOORS_ICE_PLATFORMS') or event('SHADOW_MQ_INVISIBLE_SPIKE_FLOORS_REDEADS_CLEAR')) or can_longshot()) and (glitch_megaflip() or has_hover_boots() or can_jump_slash())) end,
            ["Shadow Temple Dual Staircase Stalfos Room"] = function () return event('SHADOW_MQ_INVISIBLE_SPIKE_FLOORS_ICE_PLATFORMS') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Invisible Spike Floor Chest"] = function () return event('SHADOW_MQ_INVISIBLE_SPIKE_FLOORS_REDEADS_CLEAR') end,
            ["MQ Shadow Temple SR Spikes Northwest Corner"] = function () return has_lens() and can_hookshot() or hookshot_anywhere() or climb_anywhere() end,
            ["MQ Shadow Temple SR Spikes Southwest Wall"] = function () return can_hookshot() or climb_anywhere() end,
            ["MQ Shadow Temple SR Spikes West Midair"] = function () return can_hookshot() and has_lens() or longshot_anywhere() end,
            ["MQ Shadow Temple SR Spikes Ceiling"] = function () return is_adult() and ((has_lens() and can_hookshot()) and (event('SHADOW_MQ_INVISIBLE_SPIKE_FLOORS_ICE_PLATFORMS') or event('SHADOW_MQ_INVISIBLE_SPIKE_FLOORS_REDEADS_CLEAR')) or hookshot_anywhere()) or can_longshot() end,
            ["MQ Shadow Temple SR Spikes Center Ground"] = function () return true end,
            ["MQ Shadow Temple SR Spikes Center Midair"] = function () return is_adult() and can_hookshot() or can_longshot() end,
            ["MQ Shadow Temple SR Spikes South Midair"] = function () return is_adult() and can_hookshot() or can_longshot() end,
            ["MQ Shadow Temple SR Spikes East Ground"] = function () return true end,
            ["MQ Shadow Temple SR Spikes Northeast Wall"] = function () return hookshot_anywhere() or (has_lens() and can_longshot()) end,
            ["MQ Shadow Temple SR Spikes East Wall"] = function () return can_hookshot() or climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Dual Staircase Stalfos Room"] = {
        ["events"] = {
            ["SHADOW_MQ_DUAL_STAIRCASE_STALFOS_CLEAR"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() end,
        },
        ["exits"] = {
            ["Shadow Temple Invisible Spike Floors Room"] = function () return event('SHADOW_MQ_DUAL_STAIRCASE_STALFOS_CLEAR') end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Stalfos Room Chest"] = function () return event('SHADOW_MQ_DUAL_STAIRCASE_STALFOS_CLEAR') end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Spike Floors Room Upper Ledge Before Door"] = {
        ["exits"] = {
            ["Shadow Temple Invisible Spike Floors Room"] = function () return true end,
            ["Shadow Temple Wind Tunnel First Part"] = function () return small_keys_shadow(4) end,
        },
        ["locations"] = {
            ["MQ Shadow Temple SR Spikes West Midair"] = function () return true end,
            ["MQ Shadow Temple SR Spikes Ceiling"] = function () return has_lens() and can_hookshot() or hookshot_anywhere() end,
            ["MQ Shadow Temple SR Spikes Center Midair"] = function () return can_hookshot() end,
            ["MQ Shadow Temple SR Spikes South Midair"] = function () return can_hookshot() and event('SHADOW_MQ_INVISIBLE_SPIKE_FLOORS_ICE_PLATFORMS') end,
            ["MQ Shadow Temple SR Spikes Northeast Wall"] = function () return has_hover_boots() or can_jump_slash() or glitch_megaflip() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Wind Tunnel First Part"] = {
        ["exits"] = {
            ["Shadow Temple Invisible Spike Floors Room Upper Ledge Before Door"] = function () return small_keys_shadow(4) end,
            ["Shadow Temple Wind Tunnel After Pit Before Drop"] = function () return get_past_skultullas() and (has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Wind Tunnel After Pit Before Drop"] = {
        ["exits"] = {
            ["Shadow Temple Wind Tunnel First Part"] = function () return true end,
            ["Shadow Temple Wind Tunnel After Drop"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Wind Tunnel After Drop"] = {
        ["exits"] = {
            ["Shadow Temple Wind Tunnel After Pit Before Drop"] = function () return can_hookshot() or climb_anywhere() end,
            ["Shadow Temple Wind Tunnel Hint Room"] = function () return true end,
            ["Shadow Temple Wind Tunnel Lens Alcove"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Wind Tunnel Hint Room"] = {
        ["events"] = {
            ["SHADOW_MQ_WIND_TUNNEL_HINT_ROOM_REDEADS_CLEAR"] = function () return soul_redead_gibdo() and (can_use_din() or (can_play_sun() and (has_weapon() or can_use_sticks()))) end,
        },
        ["exits"] = {
            ["Shadow Temple Wind Tunnel After Drop"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Wind Hint Chest"] = function () return event('SHADOW_MQ_WIND_TUNNEL_HINT_ROOM_REDEADS_CLEAR') end,
            ["MQ Shadow Temple GS Wind Hint"] = function () return gs() and collect_gs_on_walls() end,
    ...
    ["Shadow Temple Wind Tunnel Lens Alcove"] = {
        ["exits"] = {
            ["Shadow Temple Wind Tunnel After Drop"] = function () return has_lens() and (is_adult() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["Shadow Temple After Wind Tunnel Gibdos Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple After Wind Tunnel Gibdos Room"] = {
        ["events"] = {
            ["SHADOW_MQ_AFTER_WIND_TUNNEL_GIBDOS_CLEAR"] = function () return soul_redead_gibdo() and (has_weapon() or can_use_sticks() or can_use_din()) end,
        },
        ["exits"] = {
            ["Shadow Temple Wind Tunnel Lens Alcove"] = function () return true end,
            ["Shadow Temple Area Before Boat Ride"] = function () return small_keys_shadow(5) end,
        },
        ["locations"] = {
            ["MQ Shadow Temple After Wind Gibdos Chest"] = function () return event('SHADOW_MQ_AFTER_WIND_TUNNEL_GIBDOS_CLEAR') end,
            ["MQ Shadow Temple After Wind Bomb Chest"] = function () return has_explosives() and has_lens() end,
            ["MQ Shadow Temple GS After Wind Bomb"] = function () return gs() and has_explosives() end,
            ["MQ Shadow Temple Pot Room Before Boat 1"] = function () return true end,
            ["MQ Shadow Temple Pot Room Before Boat 2"] = function () return true end,
            ["MQ Shadow Temple Flying Pot Room Before Boat 1"] = function () return soul_flying_pot() end,
            ["MQ Shadow Temple Flying Pot Room Before Boat 2"] = function () return soul_flying_pot() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Area Before Boat Ride"] = {
        ["events"] = {
            ["SHADOW_MQ_SHORTCUT_BLOCK_PUSHED"] = function () return has_goron_bracelet() end,
        },
        ["exits"] = {
            ["Shadow Temple After Wind Tunnel Gibdos Room"] = function () return small_keys_shadow(5) end,
            ["Shadow Temple Enclosed Shortcut Area"] = function () return event('SHADOW_MQ_SHORTCUT_BLOCK_PUSHED') end,
            ["Shadow Temple Boarding Boat Landing Area"] = function () return event('SHADOW_MQ_SHORTCUT_BLOCK_PUSHED') and is_adult() or climb_anywhere() or can_hookshot() end,
            ["Shadow Temple Area Above Shortcut Enclosure"] = function () return hookshot_anywhere() or climb_anywhere() end,
            ["Shadow Temple On The Boat"] = function () return hookshot_anywhere() or climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Area Above Shortcut Enclosure"] = {
        ["exits"] = {
            ["Shadow Temple Area Before Boat Ride"] = function () return true end,
            ["Shadow Temple Boarding Boat Landing Area"] = function () return can_longshot() end,
            ["Shadow Temple On The Boat"] = function () return hookshot_anywhere() end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Heart Shortcut 1"] = function () return true end,
            ["MQ Shadow Temple Heart Shortcut 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Boarding Boat Landing Area"] = {
        ["exits"] = {
            ["Shadow Temple Area Before Boat Ride"] = function () return true end,
            ["Shadow Temple On The Boat"] = function () return true end,
            ["Shadow Temple Area Above Shortcut Enclosure"] = function () return scarecrow_longshot() or longshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple On The Boat"] = {
        ["events"] = {
            ["SHADOW_MQ_ACTIVATE_BOAT_RIDE"] = function () return can_play_zelda() end,
        },
        ["exits"] = {
            ["Shadow Temple Area Before Boat Ride"] = function () return true end,
            ["Shadow Temple Boarding Boat Landing Area"] = function () return true end,
            ["Shadow Temple Area Above Shortcut Enclosure"] = function () return hookshot_anywhere() or scarecrow_longshot() end,
            ["Shadow Temple Area After Boat Initial Landing"] = function () return event('SHADOW_MQ_ACTIVATE_BOAT_RIDE') end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Area After Boat Initial Landing"] = {
        ["events"] = {
            ["SHADOW_MQ_AFTER_BOAT_BRIDGE_FALL"] = function () return can_use_bow() end,
        },
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Corridor To After Boat"] = function () return event('SHADOW_MQ_AREA_AFTER_BOAT_AFTER_BRIDGE_HIGH_LEDGE_SWITCH') end,
            ["Shadow Temple Area After Boat After Bridge"] = function () return event('SHADOW_MQ_AFTER_BOAT_BRIDGE_FALL') or (can_longshot() and event('SHADOW_MQ_AFTER_BOAT_AFTER_BRIDGE_EYE_SWITCH')) or longshot_anywhere() or climb_anywhere() end,
            ["Shadow Temple Area After Boat After Bridge High Ledge"] = function () return longshot_anywhere() or climb_anywhere() end,
        },
        ["locations"] = {
            ["MQ Shadow Temple GS After Boat"] = function () return gs() and (can_hookshot() or can_use_bow() or can_use_slingshot() or can_use_din() or has_explosives() or (can_boomerang() and event('SHADOW_MQ_AFTER_BOAT_BRIDGE_FALL'))) end,
    ...
    ["Shadow Temple Area After Boat After Bridge"] = {
        ["events"] = {
            ["SHADOW_MQ_AFTER_BOAT_BRIDGE_FALL"] = function () return can_use_bow() or has_bombflowers() or can_use_din() end,
            ["SHADOW_MQ_AFTER_BOAT_AFTER_BRIDGE_EYE_SWITCH"] = function () return can_play_time() and can_hit_triggers_distance() end,
        },
        ["exits"] = {
            ["Shadow Temple Area After Boat Initial Landing"] = function () return event('SHADOW_MQ_AFTER_BOAT_BRIDGE_FALL') and (is_adult() or has_hover_boots() or hookshot_anywhere()) or climb_anywhere() or longshot_anywhere() end,
            ["Shadow Temple Area After Boat After Bridge High Ledge"] = function () return event('SHADOW_MQ_AFTER_BOAT_AFTER_BRIDGE_EYE_SWITCH') and can_longshot() or hookshot_anywhere() or climb_anywhere() end,
            ["Shadow Temple Pre-Boss Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Shadow Temple GS After Boat"] = function () return gs() and (event('SHADOW_MQ_AFTER_BOAT_BRIDGE_FALL') and (can_boomerang() or can_hookshot()) or can_longshot()) end,
            ["MQ Shadow Temple Pot After Boat After Bridge 1"] = function () return true end,
            ["MQ Shadow Temple Pot After Boat After Bridge 2"] = function () return true end,
            ["MQ Shadow Temple Heart Pre-Boss Lower"] = function () return is_adult() or hookshot_anywhere() or climb_anywhere() or (can_hookshot() and event('SHADOW_MQ_AFTER_BOAT_AFTER_BRIDGE_EYE_SWITCH')) end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Area After Boat After Bridge High Ledge"] = {
        ["events"] = {
            ["SHADOW_MQ_AREA_AFTER_BOAT_AFTER_BRIDGE_HIGH_LEDGE_SWITCH"] = function () return true end,
        },
        ["exits"] = {
            ["Shadow Temple Area After Boat After Bridge"] = function () return true end,
            ["Shadow Temple Area After Boat Initial Landing"] = function () return climb_anywhere() or longshot_anywhere() end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Heart Pre-Boss Upper 1"] = function () return true end,
            ["MQ Shadow Temple Heart Pre-Boss Upper 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Wall Maze Corridor To After Boat"] = {
        ["exits"] = {
            ["Shadow Temple Area After Boat Initial Landing"] = function () return true end,
            ["Shadow Temple Invisible Wall Maze Navigation"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Wall Maze Navigation"] = {
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Corridor To After Boat"] = function () return has_lens() end,
            ["Shadow Temple Invisible Wall Maze Corridor To Dead Hand Bomb Flower Room"] = function () return has_lens() end,
            ["Shadow Temple Invisible Wall Maze Corridor To Triple Spinning Pots Room"] = function () return has_lens() end,
            ["Shadow Temple Invisible Wall Maze Corridor To Crushing Walls Room"] = function () return has_lens() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Wall Maze Corridor To Dead Hand Bomb Flower Room"] = {
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Navigation"] = function () return has_lens() end,
            ["Shadow Temple Invisible Wall Maze Dead Hand Bomb Flower Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Wall Maze Dead Hand Bomb Flower Room"] = {
        ["events"] = {
            ["SHADOW_MQ_INVISIBLE_WALL_MAZE_DEAD_HAND_CLEAR"] = function () return has_lens() and has_bombflowers() and soul_enemy(SOUL_ENEMY_DEAD_HAND) and (has_weapon() or (can_use_sticks() and trick('OOT_DEAD_HAND_STICKS'))) end,
        },
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Corridor To Dead Hand Bomb Flower Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Hidden Dead Hand Chest"] = function () return event('SHADOW_MQ_INVISIBLE_WALL_MAZE_DEAD_HAND_CLEAR') end,
    ...
    ["Shadow Temple Invisible Wall Maze Corridor To Triple Spinning Pots Room"] = {
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Navigation"] = function () return has_lens() end,
            ["Shadow Temple Invisible Wall Maze Triple Spinning Pots Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Wall Maze Triple Spinning Pots Room"] = {
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Corridor To Triple Spinning Pots Room"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Triple Pot Key"] = function () return true end,
            ["MQ Shadow Temple Wonder Item"] = function () return can_use_bow() end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Wall Maze Corridor To Crushing Walls Room"] = {
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Navigation"] = function () return has_lens() end,
            ["Shadow Temple Invisible Wall Maze Crushing Walls Room"] = function () return small_keys_shadow(6) end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Invisible Wall Maze Crushing Walls Room"] = {
        ["events"] = {
            ["SHADOW_MQ_CRUSHING_WALLS_BURNT"] = function () return can_use_din() end,
        },
        ["exits"] = {
            ["Shadow Temple Invisible Wall Maze Corridor To Crushing Walls Room"] = function () return small_keys_shadow(6) end,
        },
        ["locations"] = {
            ["MQ Shadow Temple Crushing Wall Left Chest"] = function () return event('SHADOW_MQ_CRUSHING_WALLS_BURNT') end,
            ["MQ Shadow Temple Boss Key Chest"] = function () return event('SHADOW_MQ_CRUSHING_WALLS_BURNT') end,
            ["MQ Shadow Temple Pot Boss Key Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Pre-Boss Room"] = {
        ["exits"] = {
            ["Shadow Temple Area After Boat After Bridge"] = function () return true end,
            ["Shadow Temple Pre-Boss"] = function () return has_lens() and (has_hover_boots() or climb_anywhere() or hookshot_anywhere() or glitch_megaflip()) end,
        },
        ["locations"] = {
            ["MQ Shadow Temple GS Pre-Boss"] = function () return gs() and has_lens() and (hookshot_anywhere() or glitch_megaflip() or ((climb_anywhere() or has_hover_boots()) and (has_ranged_weapon() or has_explosives() or can_use_din()))) end,
        },
        ["age_change"] = true,
    },
    ["Shadow Temple Pre-Boss"] = {
    ...
    ["Spirit Temple"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Spirit Temple Entrance Boulders"] = function () return has_explosives_or_hammer() end,
    ...
    ["Spirit Temple Child Side"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Spirit Temple Child Crawl Boulders"] = function () return has_bombchu() end,
            ["MQ Spirit Temple Child Bridge Boulders"] = function () return has_explosives_or_hammer() end,
    ...
    ["Spirit Temple Child Upper"] = {
        ...
        ["locations"] = {
            ["MQ Spirit Temple Child Upper Ground Chest"] = function () return soul_like_like() and soul_beamos() and soul_enemy(SOUL_ENEMY_BABY_DODONGO) and has_explosives() end,
    ...
    ["Spirit Temple Statue"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Spirit Temple Giant Statue Room Large Crate 1"] = function () return true end,
            ["MQ Spirit Temple Giant Statue Room Large Crate 2"] = function () return true end,
            ["MQ Spirit Temple Giant Statue Room Small Crate"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ...
    ["Spirit Temple Adult Lower"] = {
        ["locations"] = {
            ["MQ Spirit Temple Purple Leever Chest"] = function () return can_longshot() and soul_leever() and has_weapon() end,
            ...
            ["MQ Spirit Temple GS Leever Room"] = function () return gs() and can_collect_distance() end,
            ...
            ["MQ Spirit Temple Boulder after Water"] = function () return has_explosives_or_hammer() end,
    ...
    ["Spirit Temple Adult Upper"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Spirit Temple Beamos Room Small Crate"] = function () return can_play_time() end,
        },
        ["age_change"] = true,
    },
    ["Spirit Temple Adult Upper 2"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Spirit Temple Wonder Item Chest Hammer"] = function () return can_hammer() end,
            ["MQ Spirit Temple Wonder Item Chest Slash"] = function () return can_use_sword_or_sticks() end,
        },
        ["age_change"] = true,
    },
    ["Spirit Temple Adult Hand"] = {
    ...
    ["Spirit Temple Top Floor"] = {
        ...
        ["locations"] = {
            ...
            ["MQ Spirit Temple Top Near Lowering Platform Large Crate 1"] = function () return can_play_zelda() end,
            ["MQ Spirit Temple Top Near Lowering Platform Large Crate 2"] = function () return can_play_zelda() end,
            ["MQ Spirit Temple Top Near Lowering Platform Large Crate 3"] = function () return can_play_zelda() end,
            ["MQ Spirit Temple Top Near Lowering Platform Large Crate 4"] = function () return can_play_zelda() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple"] = {
        ["exits"] = {
            ["Lake Hylia Near Water Temple"] = function () return true end,
            ["Water Temple Entrance"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Entrance"] = {
        ["exits"] = {
            ["Water Temple"] = function () return true end,
            ["Water Temple Main Room Top Floor"] = function () return event('WATER_LEVEL_HIGH') or has_hover_boots() or hookshot_anywhere() end,
            ["Water Temple Main Room Middle Floor"] = function () return event('WATER_LEVEL_MID') or (has_iron_boots() and has_tunic_zora()) or (event('WATER_LEVEL_LOW') and hookshot_anywhere()) end,
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_LEVEL_LOW') or (has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple High Water Level Ledge"] = function () return longshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Main Room Top Floor"] = {
        ["exits"] = {
            ["Water Temple Entrance"] = function () return event('WATER_LEVEL_HIGH') or has_hover_boots() or hookshot_anywhere() or can_longshot() end,
            ["Water Temple Main Room Middle Floor"] = function () return event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora() or event('WATER_LEVEL_MID') or (event('WATER_LEVEL_LOW') and longshot_anywhere()) end,
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_LEVEL_LOW') or (has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple High Water Level Ledge"] = function () return can_hookshot() or (is_adult() and has_hover_boots()) end,
            ["Water Temple Hookshot Staircase from Main"] = function () return small_keys_water(1) and event('WATER_LEVEL_HIGH') end,
            ["Water Temple Main Room Antichamber Ledge"] = function () return can_longshot() and event('WATER_LEVEL_HIGH') or longshot_anywhere() or climb_anywhere() end,
            ["Water Temple Storage Room"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and (can_hookshot() or climb_anywhere()) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and can_hookshot() and has_tunic_zora()) end,
            ["Water Temple Lizalfos Hallway"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (has_iron_boots() and has_tunic_zora_strict() and event('WATER_LEVEL_HIGH')) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Main Room Middle Floor"] = {
        ["exits"] = {
            ["Water Temple Entrance"] = function () return event('WATER_LEVEL_HIGH') or longshot_anywhere() or (hookshot_anywhere() and climb_anywhere()) end,
            ["Water Temple High Water Level Ledge"] = function () return can_longshot() or hookshot_anywhere() end,
            ["Water Temple Main Room Top Floor"] = function () return event('WATER_LEVEL_HIGH') end,
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_LEVEL_LOW') or ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple Storage Room"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and (can_hookshot() or climb_anywhere()) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and can_hookshot() and has_tunic_zora()) end,
            ["Water Temple Blue Switch Path"] = function () return event('WATER_LEVEL_MID') and (can_hookshot() or can_play_elegy()) end,
            ["Water Temple Central Tower Middle Door"] = function () return event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID') or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple Lizalfos Hallway"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and (hookshot_anywhere() or has_hover_boots()) or (has_iron_boots() and has_tunic_zora_strict() and event('WATER_LEVEL_HIGH')) end,
            ["Water Temple Main Room Antichamber Ledge"] = function () return event('WATER_LEVEL_HIGH') and can_longshot() or longshot_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Main Room Bottom Floor"] = {
        ["exits"] = {
            ["Water Temple Main Room Top Floor"] = function () return event('WATER_LEVEL_HIGH') or (event('WATER_LEVEL_LOW') and has_iron_boots() and has_tunic_zora() and longshot_anywhere()) end,
            ["Water Temple Main Room Middle Floor"] = function () return event('WATER_LEVEL_MID') or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) or (event('WATER_LEVEL_LOW') and hookshot_anywhere()) end,
            ["Water Temple Central Tower Lower Door"] = function () return event('WATER_LEVEL_LOW') end,
            ["Water Temple Ruto Column Bottom"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["Water Temple Three Torch Room"] = function () return event('WATER_GATES') and (event('WATER_LEVEL_LOW') and (has_iron_boots() or hookshot_anywhere() or can_dive_big()) or ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora_strict())) end,
            ["Water Temple Boss Key Room Switch Side"] = function () return event('WATER_GATES') and (can_dive_big() and event('WATER_LEVEL_LOW') and climb_anywhere() or (event('WATER_LEVEL_LOW') and hookshot_anywhere() and climb_anywhere()) or ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora() and (hookshot_anywhere() or climb_anywhere()))) end,
            ["Water Temple Leading To Side Loop Before Spikes"] = function () return event('WATER_GATES') and ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora() or (event('WATER_LEVEL_LOW') and (has_iron_boots() or hookshot_anywhere() or can_dive_small()))) end,
            ["Water Temple Storage Room"] = function () return event('WATER_LEVEL_MID') or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) or (event('WATER_LEVEL_LOW') and longshot_anywhere()) end,
            ["Water Temple Lizalfos Hallway"] = function () return event('WATER_LEVEL_MID') or (event('WATER_LEVEL_LOW') and longshot_anywhere()) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Ruto Column Bottom"] = {
        ["exits"] = {
            ["Water Temple Main Room Bottom Floor"] = function () return true end,
            ["Water Temple Ruto Column Middle"] = function () return event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora() or (event('WATER_LEVEL_MID') and has_iron_boots() and has_tunic_zora()) or (event('WATER_LEVEL_LOW') and ((longshot_anywhere() or (hookshot_anywhere() and climb_anywhere())) or (event('RUTO_COLUMN_HOOKSHOTS') and can_hookshot()))) end,
            ["Water Temple Ruto Column Top"] = function () return event('WATER_LEVEL_HIGH') end,
            ["Water Temple Compass Room"] = function () return event('WATER_LEVEL_LOW') and event('WATER_LEVEL_HIGH') and (can_use_bow() or has_fire()) end,
        },
        ["locations"] = {
            ["MQ Water Temple Pot Ruto 1"] = function () return can_hookshot() or event('WATER_LEVEL_LOW') end,
            ["MQ Water Temple Pot Ruto 2"] = function () return can_hookshot() or event('WATER_LEVEL_LOW') end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Compass Room"] = {
        ["events"] = {
            ["RUTO_COLUMN_HOOKSHOTS"] = function () return soul_enemy(SOUL_ENEMY_SPIKE) and soul_lizalfos_dinalfos() and can_damage() end,
        },
        ["exits"] = {
            ["Water Temple Ruto Column Bottom"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Water Temple Compass Chest"] = function () return event('RUTO_COLUMN_HOOKSHOTS') end,
            ["MQ Water Temple Wonder Item Lizalfos Room"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Ruto Column Middle"] = {
        ["exits"] = {
            ["Water Temple Ruto Column Bottom"] = function () return event('WATER_LEVEL_LOW') or ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple Ruto Column Top"] = function () return event('WATER_LEVEL_HIGH') or ((event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and (hookshot_anywhere() and climb_anywhere() or longshot_anywhere())) end,
        },
        ["locations"] = {
            ["MQ Water Temple Longshot Chest"] = function () return event('WATER_LEVEL_LOW') and can_hookshot() and can_damage() end,
            ["MQ Water Temple Wonder Item Longshot Room"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Ruto Column Top"] = {
        ...
        ["exits"] = {
            ["Water Temple Ruto Column Bottom"] = function () return has_iron_boots() and has_tunic_zora() or event('WATER_LEVEL_LOW') end,
            ["Water Temple Ruto Column Middle"] = function () return event('WATER_LEVEL_MID') or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) or (event('WATER_LEVEL_LOW') and (longshot_anywhere() or (hookshot_anywhere() and climb_anywhere()))) end,
            ["Water Temple Map Chest Room"] = function () return has_fire() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Map Chest Room"] = {
        ["exits"] = {
            ["Water Temple Ruto Column Top"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() end,
        },
        ["locations"] = {
            ["MQ Water Temple Map Chest"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Stalfos Room"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Storage Room"] = {
        ["exits"] = {
            ["Water Temple Main Room Middle Floor"] = function () return (event('WATER_LEVEL_MID') or event('WATER_LEVEL_LOW')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple Main Room Top Floor"] = function () return can_hookshot() and (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (has_iron_boots() and has_tunic_zora() and event('WATER_LEVEL_HIGH')) end,
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_LEVEL_LOW') or (has_iron_boots() and has_tunic_zora() and (event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH'))) end,
        },
        ["locations"] = {
            ["MQ Water Temple Pot Storage Room 1"] = function () return true end,
            ["MQ Water Temple Pot Storage Room 2"] = function () return true end,
            ["MQ Water Temple Pot Storage Room 3"] = function () return true end,
            ["MQ Water Temple Storage Room Large Crate 1"] = function () return true end,
            ["MQ Water Temple Storage Room Large Crate 2"] = function () return true end,
            ["MQ Water Temple Storage Room Large Crate 3"] = function () return true end,
            ["MQ Water Temple Storage Room Large Crate 4"] = function () return true end,
            ["MQ Water Temple Storage Room Large Crate 5"] = function () return true end,
            ["MQ Water Temple Storage Room Large Crate 6"] = function () return true end,
            ["MQ Water Temple Storage Room Large Crate 7"] = function () return true end,
            ["MQ Water Temple Storage Room Small Crate 1"] = function () return true end,
            ["MQ Water Temple Storage Room Small Crate 2"] = function () return true end,
            ["MQ Water Temple Storage Room Small Crate 3"] = function () return true end,
            ["MQ Water Temple Storage Room Small Crate 4"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Hookshot Staircase from Main"] = {
        ["exits"] = {
            ["Water Temple Main Room Top Floor"] = function () return small_keys_water(1) and event('WATER_LEVEL_HIGH') end,
            ["Water Temple Hookshot Staircase Before Dark Link"] = function () return can_longshot() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Hookshot Staircase Before Dark Link"] = {
        ["exits"] = {
            ["Water Temple Hookshot Staircase from Main"] = function () return can_longshot() end,
            ["Water Temple Room Before Dark Link Lower"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Water Temple Wonder Item Hookshot Staircase Right 1"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Hookshot Staircase Right 2"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Hookshot Staircase Right 3"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Hookshot Staircase Left 1"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Hookshot Staircase Left 2"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Hookshot Staircase Left 3"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Room Before Dark Link Lower"] = {
        ["exits"] = {
            ["Water Temple Room Before Dark Link Upper"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() and can_hookshot() or hookshot_anywhere() or climb_anywhere() end,
            ["Water Temple Hookshot Staircase Before Dark Link"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Water Temple Pot Before Dark Link Ledge 1"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() and can_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere() end,
            ["MQ Water Temple Pot Before Dark Link Ledge 2"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() and can_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere() end,
            ["MQ Water Temple Pot Before Dark Link Ledge 3"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() and can_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere() end,
            ["MQ Water Temple Big Fairy Before Dark Link Ledge"] = function () return can_play_sun() and (soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() and can_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Room Before Dark Link Upper"] = {
        ["exits"] = {
            ["Water Temple Room Before Dark Link Lower"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() and can_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere() end,
            ["Water Temple Dark Link"] = function () return can_hookshot() or climb_anywhere() end,
        },
        ["locations"] = {
            ["MQ Water Temple Pot Before Dark Link Near Door 1"] = function () return true end,
            ["MQ Water Temple Pot Before Dark Link Near Door 2"] = function () return true end,
            ["MQ Water Temple Big Fairy Before Dark Link Near Door 1"] = function () return can_play_sun() end,
            ["MQ Water Temple Big Fairy Before Dark Link Near Door 2"] = function () return can_play_storms() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Dark Link"] = {
        ["exits"] = {
            ["Water Temple Room Before Dark Link Upper"] = function () return soul_enemy(SOUL_ENEMY_DARK_LINK) and has_weapon() end,
            ["Water Temple After Dark Link"] = function () return soul_enemy(SOUL_ENEMY_DARK_LINK) and has_weapon() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple After Dark Link"] = {
        ["exits"] = {
            ["Water Temple Dark Link"] = function () return true end,
            ["Water Temple River Start"] = function () return can_hookshot() end,
        },
        ["locations"] = {
            ["MQ Water Temple Wonder Item After Dark Link"] = function () return can_hookshot() end,
            ["MQ Water Temple Pot After Dark Link 1"] = function () return true end,
            ["MQ Water Temple Pot After Dark Link 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple River Start"] = {
        ["exits"] = {
            ["Water Temple River End"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Water Temple GS River"] = function () return gs() and can_collect_distance() end,
            ["MQ Water Temple Pot River 1"] = function () return true end,
            ["MQ Water Temple Pot River 2"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple River End"] = {
        ["exits"] = {
            ["Water Temple River Start"] = function () return has_iron_boots() and can_hookshot() end,
            ["Water Temple Dragon Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Dragon Room"] = {
        ["exits"] = {
            ["Water Temple River End"] = function () return can_longshot() or climb_anywhere() end,
            ["Water Temple Boss Key Room Dragon Side"] = function () return has_fire() and can_dive_small() and has_tunic_zora() end,
        },
        ["locations"] = {
            ["MQ Water Temple Wonder Item Dragon Room Eyes 1"] = function () return has_iron_boots() and can_hookshot() end,
            ["MQ Water Temple Wonder Item Dragon Room Eyes 2"] = function () return has_iron_boots() and can_hookshot() end,
            ["MQ Water Temple Wonder Item Dragon Room Portrait"] = function () return can_dive_small() and has_tunic_zora() and can_hookshot() end,
            ["MQ Water Temple Dragon Room At Door Large Crate 1"] = function () return true end,
            ["MQ Water Temple Dragon Room At Door Large Crate 2"] = function () return true end,
            ["MQ Water Temple Dragon Room Underwater Large Crate 1"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room Underwater Large Crate 2"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room Underwater Large Crate 3"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room Underwater Large Crate 4"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room At Torches Large Crate 1"] = function () return can_dive_small() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room At Torches Large Crate 2"] = function () return can_dive_small() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room At Torches Small Crate 1"] = function () return can_dive_small() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room At Torches Small Crate 2"] = function () return can_dive_small() and has_tunic_zora() end,
            ["MQ Water Temple Dragon Room At Torches Small Crate 3"] = function () return can_dive_small() and has_tunic_zora() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Boss Key Room Dragon Side"] = {
        ["exits"] = {
            ["Water Temple Dragon Room"] = function () return true end,
            ["Water Temple Boss Key Room Switch Side"] = function () return (has_explosives_or_hammer() or can_hit_triggers_distance() or can_use_sword_or_sticks()) and can_use_din() end,
        },
        ["locations"] = {
            ["MQ Water Temple Pot Boss Key Room"] = function () return true end,
            ["MQ Water Temple Boss Key Room By Switch Large Crate"] = function () return true end,
            ["MQ Water Temple Boss Key Room In Center Large Crate 1"] = function () return true end,
            ["MQ Water Temple Boss Key Room In Center Large Crate 2"] = function () return true end,
            ["MQ Water Temple Boss Key Room In Center Large Crate 3"] = function () return true end,
            ["MQ Water Temple Boss Key Room In Center Large Crate 4"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Boss Key Room Switch Side"] = {
        ["events"] = {
            ["WATER_GATES"] = function () return has_iron_boots() and has_tunic_zora() end,
        },
        ["exits"] = {
            ["Water Temple Boss Key Room Dragon Side"] = function () return can_use_din() and (can_hit_triggers_distance() or climb_anywhere() or can_hookshot()) end,
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_GATES') end,
        },
        ["locations"] = {
            ["MQ Water Temple Boss Key Chest"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Three Torch Room"] = {
        ["events"] = {
            ["WATER_MQ_THREE_TORCH_ROOM_GATE"] = function () return has_fire_arrows() end,
        },
        ["exits"] = {
            ["Water Temple Main Room Bottom Floor"] = function () return has_iron_boots() and has_tunic_zora_strict() or (longshot_anywhere() and has_tunic_zora_strict()) end,
        },
        ["locations"] = {
            ["MQ Water Temple GS Three Torch"] = function () return gs() and event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["MQ Water Temple Pot Skull Cage 1"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["MQ Water Temple Pot Skull Cage 2"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["MQ Water Temple Pot Skull Cage 3"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["MQ Water Temple Pot Skull Cage 4"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["MQ Water Temple Wonder Item Triple Torches"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) and (can_hookshot() or can_use_bow()) end,
            ["MQ Water Temple Three Torch Room Large Crate 1"] = function () return true end,
            ["MQ Water Temple Three Torch Room Large Crate 2"] = function () return true end,
            ["MQ Water Temple Three Torch Room Large Crate 3"] = function () return true end,
            ["MQ Water Temple Three Torch Room Large Crate 4"] = function () return true end,
            ["MQ Water Temple Three Torch Room Large Crate 5"] = function () return true end,
            ["MQ Water Temple Three Torch Room Large Crate 6"] = function () return true end,
            ["MQ Water Temple Three Torch Room Behind Gate Large Crate 1"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["MQ Water Temple Three Torch Room Behind Gate Large Crate 2"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
            ["MQ Water Temple Three Torch Room Behind Gate Large Crate 3"] = function () return event('WATER_MQ_THREE_TORCH_ROOM_GATE') and (scarecrow_hookshot() or has_hover_boots() or hookshot_anywhere() or climb_anywhere()) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Leading To Side Loop Before Spikes"] = {
        ["exits"] = {
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_GATES') and has_iron_boots() and has_tunic_zora() end,
            ["Water Temple Leading To Side Loop After Spikes"] = function () return can_longshot() or hookshot_anywhere() or climb_anywhere() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Leading To Side Loop After Spikes"] = {
        ["exits"] = {
            ["Water Temple Side Loop Gate Room Main"] = function () return true end,
            ["Water Temple Leading To Side Loop Before Spikes"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Gate Room Main"] = {
        ["exits"] = {
            ["Water Temple Leading To Side Loop After Spikes"] = function () return true end,
            ["Water Temple Side Loop Scarecrow Alcove"] = function () return scarecrow_hookshot() or (has_hover_boots() and is_adult()) or hookshot_anywhere() or climb_anywhere() end,
            ["Water Temple Side Loop Gate Room Past Gate"] = function () return has_iron_boots() and hookshot_anywhere() or (climb_anywhere() and can_jump_slash()) end,
        },
        ["locations"] = {
            ["MQ Water Temple Side Loop First Room At Door Large Crate 1"] = function () return true end,
            ["MQ Water Temple Side Loop First Room At Door Large Crate 2"] = function () return true end,
            ["MQ Water Temple Side Loop First Room Water Large Crate 1"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Side Loop First Room Water Large Crate 2"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Side Loop First Room Water Large Crate 3"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Side Loop First Room Water Large Crate 4"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Side Loop First Room Water Large Crate 5"] = function () return has_iron_boots() and has_tunic_zora() end,
            ["MQ Water Temple Side Loop First Room Water Large Crate 6"] = function () return has_iron_boots() and has_tunic_zora() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Scarecrow Alcove"] = {
        ["exits"] = {
            ["Water Temple Side Loop Gate Room Main"] = function () return true end,
            ["Water Temple Side Loop Room After Locked Door Switch Side"] = function () return small_keys_water(2) end,
            ["Water Temple Side Loop Scarecrow Alcove Underwater Path"] = function () return has_iron_boots() and has_tunic_zora() or (hookshot_anywhere() and has_tunic_zora()) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Room After Locked Door Switch Side"] = {
        ["exits"] = {
            ["Water Temple Side Loop Scarecrow Alcove"] = function () return small_keys_water(2) end,
            ["Water Temple Side Loop Room After Locked Door Target Side"] = function () return (has_explosives_or_hammer() or can_hit_triggers_distance() or can_use_sword_or_sticks()) or can_hookshot() or climb_anywhere() end,
        },
        ["locations"] = {
            ["MQ Water Temple Wonder Item Water Sprouts 1"] = function () return can_hookshot() or (has_explosives_or_hammer() or can_hit_triggers_distance() or can_use_sword_or_sticks()) end,
            ["MQ Water Temple Wonder Item Water Sprouts 2"] = function () return can_hookshot() or (has_explosives_or_hammer() or can_hit_triggers_distance() or can_use_sword_or_sticks()) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Room After Locked Door Target Side"] = {
        ["exits"] = {
            ["Water Temple Side Loop Room After Locked Door Switch Side"] = function () return can_hit_triggers_distance() or can_hookshot() or climb_anywhere() end,
            ["Water Temple Side Loop Dodongo Room"] = function () return can_hit_triggers_distance() and has_fire() end,
        },
        ["locations"] = {
            ["MQ Water Temple Wonder Item Water Sprouts 1"] = function () return (can_hit_triggers_distance() or can_longshot()) or (climb_anywhere() and (has_explosives_or_hammer() or can_hit_triggers_distance() or can_use_sword_or_sticks())) end,
            ["MQ Water Temple Wonder Item Water Sprouts 2"] = function () return (can_hit_triggers_distance() or can_longshot()) or (climb_anywhere() and (has_explosives_or_hammer() or can_hit_triggers_distance() or can_use_sword_or_sticks())) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Dodongo Room"] = {
        ["exits"] = {
            ["Water Temple Side Loop Room After Locked Door Target Side"] = function () return soul_dodongo() and (has_explosives_or_hammer() or has_weapon()) and (can_hookshot() or climb_anywhere()) end,
            ["Water Temple Side Loop Gate Room Past Gate"] = function () return soul_dodongo() and (has_explosives_or_hammer() or has_weapon()) and (can_hookshot() or climb_anywhere()) end,
        },
        ["locations"] = {
            ["MQ Water Temple Pot Loop 1"] = function () return true end,
            ["MQ Water Temple Pot Loop 2"] = function () return true end,
            ["MQ Water Temple Dodongo Room Large Crate 1"] = function () return true end,
            ["MQ Water Temple Dodongo Room Large Crate 2"] = function () return true end,
            ["MQ Water Temple Dodongo Room Large Crate 3"] = function () return true end,
            ["MQ Water Temple Dodongo Room Large Crate 4"] = function () return true end,
            ["MQ Water Temple Dodongo Room Large Crate 5"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Gate Room Past Gate"] = {
        ["exits"] = {
            ["Water Temple Side Loop Dodongo Room"] = function () return true end,
            ["Water Temple Side Loop Gate Room Main"] = function () return hookshot_anywhere() end,
        },
        ["locations"] = {
            ["MQ Water Temple GS Side Loop"] = function () return gs() and can_damage_skull() end,
            ["MQ Water Temple Side Loop First Room In Gate Large Crate 1"] = function () return true end,
            ["MQ Water Temple Side Loop First Room In Gate Large Crate 2"] = function () return true end,
            ["MQ Water Temple Side Loop First Room In Gate Large Crate 3"] = function () return true end,
            ["MQ Water Temple Side Loop First Room In Gate Large Crate 4"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Scarecrow Alcove Underwater Path"] = {
        ["exits"] = {
            ["Water Temple Side Loop Scarecrow Alcove"] = function () return has_iron_boots() and has_tunic_zora() or (hookshot_anywhere() and has_tunic_zora()) or can_dive_small() end,
            ["Water Temple Side Loop Stalfos Room"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Side Loop Stalfos Room"] = {
        ["exits"] = {
            ["Water Temple Side Loop Scarecrow Alcove Underwater Path"] = function () return soul_enemy(SOUL_ENEMY_STALFOS) and has_weapon() end,
        },
        ["locations"] = {
            ...
            ["MQ Water Temple Wonder Item Freestanding Room"] = function () return can_hookshot() end,
            ["MQ Water Temple Side Loop Stalfos Room Large Crate 1"] = function () return true end,
            ["MQ Water Temple Side Loop Stalfos Room Large Crate 2"] = function () return true end,
            ["MQ Water Temple Side Loop Stalfos Room Large Crate 3"] = function () return true end,
            ["MQ Water Temple Side Loop Stalfos Room Large Crate 4"] = function () return true end,
            ["MQ Water Temple Side Loop Stalfos Room Large Crate 5"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Main Room Antichamber Ledge"] = {
        ["exits"] = {
            ["Water Temple Main Room Top Floor"] = function () return can_longshot() or event('WATER_LEVEL_HIGH') end,
            ["Water Temple Antichamber Room"] = function () return true end,
            ["Water Temple Main Room Middle Floor"] = function () return can_longshot() and ((event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora())) end,
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_LEVEL_LOW') or ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora()) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Antichamber Room"] = {
        ["exits"] = {
            ["Water Temple Main Room Antichamber Ledge"] = function () return true end,
            ["Water Temple Boss"] = function () return boss_key(BOSS_KEY_WATER) and (can_longshot() or climb_anywhere()) end,
        },
        ["locations"] = {
            ["MQ Water Temple Wonder Item Before Boss 1"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Before Boss 2"] = function () return can_hookshot() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Central Tower Lower Door"] = {
        ["exits"] = {
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_LEVEL_LOW') or (event('WATER_LEVEL_MID') and ((can_hookshot() or is_adult()) or climb_anywhere())) end,
            ["Water Temple Central Tower Middle Door"] = function () return event('WATER_LEVEL_MID') and is_adult() or can_longshot() or hookshot_anywhere() or climb_anywhere() end,
            ["Water Temple Central Tower Under Gate"] = function () return event('WATER_CENTRAL_GATE') and (event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora_strict() end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Central Tower Middle Door"] = {
        ["events"] = {
            ["WATER_LEVEL_MID"] = function () return event('WATER_LEVEL_LOW') and can_play_zelda() end,
            ["MOVE_WATER_TIME_BLOCK"] = function () return can_play_time() and (can_longshot() or hookshot_anywhere() or climb_anywhere()) end,
            ["WATER_CENTRAL_GATE"] = function () return event('MOVE_WATER_TIME_BLOCK') and can_use_din() and (event('WATER_LEVEL_HIGH') or ((event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and hookshot_anywhere())) end,
        },
        ["exits"] = {
            ["Water Temple Main Room Middle Floor"] = function () return event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID') or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple Central Tower Lower Door"] = function () return event('WATER_LEVEL_LOW') or ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora()) end,
        },
        ["locations"] = {
            ["MQ Water Temple Central Tower Top Large Crate 1"] = function () return event('WATER_LEVEL_HIGH') and (has_iron_boots() or has_explosives_or_hammer()) or ((event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and (can_longshot() or hookshot_anywhere() or climb_anywhere())) end,
            ["MQ Water Temple Central Tower Top Large Crate 2"] = function () return event('WATER_LEVEL_HIGH') and (has_iron_boots() or has_explosives_or_hammer()) or ((event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and (can_longshot() or hookshot_anywhere() or climb_anywhere())) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Central Tower Under Gate"] = {
        ["exits"] = {
            ["Water Temple Central Tower Lower Door"] = function () return event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH') end,
        },
        ["locations"] = {
            ["MQ Water Temple Central Pillar Chest"] = function () return can_hookshot() end,
            ["MQ Water Temple Wonder Item Under Pillar Room"] = function () return can_hookshot() end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 1"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 2"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 3"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 4"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 5"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 6"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 7"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 8"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 9"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 10"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 11"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 12"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 13"] = function () return true end,
            ["MQ Water Temple Central Tower Under Gate Large Crate 14"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Lizalfos Hallway"] = {
        ["exits"] = {
            ["Water Temple Main Room Top Floor"] = function () return can_hookshot() or climb_anywhere() end,
        },
        ["locations"] = {
            ["MQ Water Temple GS Lizalfos Hallway"] = function () return gs() and (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and can_use_din() end,
            ["MQ Water Temple Pot Twisted Room Entrance"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora() and can_hookshot()) end,
            ["MQ Water Temple Pot Twisted Room Right 1"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora() and can_hookshot()) end,
            ["MQ Water Temple Pot Twisted Room Right 2"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora() and can_hookshot()) end,
            ["MQ Water Temple Pot Twisted Room Cage 1"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and can_use_din() end,
            ["MQ Water Temple Pot Twisted Room Cage 2"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and can_use_din() end,
            ["MQ Water Temple Wonder Item Lizalfos Hallway"] = function () return can_hookshot() end,
            ["MQ Water Temple Lizalfos Hallway Behind Gate Large Crate 1"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and can_use_din() end,
            ["MQ Water Temple Lizalfos Hallway Behind Gate Large Crate 2"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) and can_use_din() end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 1"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 2"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 3"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 4"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 5"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 6"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 7"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["MQ Water Temple Lizalfos Hallway Large Crate 8"] = function () return (event('WATER_LEVEL_LOW') or event('WATER_LEVEL_MID')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
        },
        ["age_change"] = true,
    },
    ["Water Temple High Water Level Ledge"] = {
        ["events"] = {
            ["WATER_LEVEL_HIGH"] = function () return can_play_zelda() end,
        },
        ["exits"] = {
            ["Water Temple Main Room Top Floor"] = function () return true end,
            ["Water Temple Entrance"] = function () return longshot_anywhere() end,
            ["Water Temple Main Room Middle Floor"] = function () return (event('WATER_LEVEL_MID') or event('WATER_LEVEL_LOW')) or (event('WATER_LEVEL_HIGH') and has_iron_boots() and has_tunic_zora()) end,
            ["Water Temple Main Room Bottom Floor"] = function () return event('WATER_LEVEL_LOW') or ((event('WATER_LEVEL_MID') or event('WATER_LEVEL_HIGH')) and has_iron_boots() and has_tunic_zora()) end,
        },
        ["age_change"] = true,
    ...
    ["Water Temple Blue Switch Path"] = {
        ["exits"] = {
            ["Water Temple Main Room Middle Floor"] = function () return true end,
            ["Water Temple Room Before Water Raise"] = function () return can_longshot() or (hookshot_anywhere() and climb_anywhere()) end,
        },
        ["locations"] = {
            ["MQ Water Temple Pot Room Before High Water 1"] = function () return true end,
            ["MQ Water Temple Pot Room Before High Water 2"] = function () return true end,
            ["MQ Water Temple Pot Room Before High Water 3"] = function () return true end,
            ["MQ Water Temple Wonder Item Above 2F West Room"] = function () return can_hookshot() end,
            ["MQ Water Temple Room Before High Water Lower Large Crate 1"] = function () return true end,
            ["MQ Water Temple Room Before High Water Lower Large Crate 2"] = function () return true end,
            ["MQ Water Temple Room Before High Water Lower Large Crate 3"] = function () return true end,
            ["MQ Water Temple Room Before High Water Lower Large Crate 4"] = function () return true end,
            ["MQ Water Temple Room Before High Water Lower Large Crate 5"] = function () return true end,
            ["MQ Water Temple Room Before High Water Lower Large Crate 6"] = function () return true end,
            ["MQ Water Temple Room Before High Water Lower Small Crate"] = function () return true end,
        },
        ["age_change"] = true,
    },
    ["Water Temple Room Before Water Raise"] = {
        ["exits"] = {
            ["Water Temple Blue Switch Path"] = function () return true end,
            ["Water Temple High Water Level Ledge"] = function () return true end,
        },
        ["locations"] = {
            ["MQ Water Temple GS High Water Changer"] = function () return gs() and can_damage_skull() end,
            ["MQ Water Temple Room Before High Water Upper Large Crate 1"] = function () return true end,
            ["MQ Water Temple Room Before High Water Upper Large Crate 2"] = function () return true end,
